...
/// Map a Z.AI HTTP status + error body to the shared taxonomy.
...
pub fn classify(status: Option<u16>, message: &str) -> ErrorKind {
    if let Ok(v) = serde_json::from_str::<Value>(message) {
        if let Some(kind) = classify_zai_value(&v, status) {
            return kind;
        }
    }
    match status {
...
/// Z.AI sends OpenAI-style envelopes `{ "error": { "message", "code" } }`
/// with numeric-string business codes (e.g. "1113"); a custom
...
fn classify_zai_value(v: &Value, status: Option<u16>) -> Option<ErrorKind> {
    let err = v.get("error")?;
    ...
    match code {
        ...
        // Z.AI 1113 "insufficient balance / no resource package" is a billing
        // wall (docs.z.ai devpack/faq) — retry/backoff cannot fix it.
        "1113" => return Some(ErrorKind::Permanent),
        _ => {}
    }
    match err_type {
...
fn is_context_overflow_message(message: &str) -> bool {
    ...
    m.contains("context length")
        ...
        // Z.AI phrasing: "This model's maximum prompt length is 256000 but the
...
mod tests {
    ...
    #[test]
    fn zai_grpc_style_prompt_length_overflow_is_context_overflow() {
