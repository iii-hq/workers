...
pub struct PartialState {
    text: String,
    /// Accumulated `delta.reasoning_content` (Z.AI reasoning models).
    thinking: String,
    ...
    for fc in &state.function_calls {
        ...
        let arguments = if fc.args_json.is_empty() {
            ...
            serde_json::from_str(&fc.args_json)
                .unwrap_or_else(|_| llm_router::types::messages::degraded_arguments(&fc.args_json))
        };
...
/// Last-wins merge: Z.AI reports usage natively on the final SSE chunk;
/// OpenAI-compatible servers honoring `stream_options.include_usage` report
/// once at the end or cumulatively per chunk — overwriting is correct for
...
pub fn handle_chunk(
    ...
    if let Some(delta) = choice.get("delta") {
        // Z.AI reasoning models stream chain-of-thought as `reasoning_content`
    ...
    if let Some(finish) = choice.get("finish_reason").and_then(Value::as_str) {
        ...
        if finish == "content_filter" {
            state
                .warnings
                .push("zai filtered the completion (finish_reason: content_filter)".to_string());
        }
        close_open_block(state, model, &mut events);
    }
    events
...
mod tests {
    ...
    fn run(chunks: &[Value]) -> (PartialState, Vec<AssistantMessageEvent>) {
        ...
        let mut events = Vec::new();
        for c in chunks {
            events.extend(handle_chunk(c, &mut state, "glm-test"));
        }
        (state, events)
    }
    ...
    fn text_stream_produces_start_delta_end_and_final_content() {
        ...
        let final_msg = build_final(&state, "glm-test");
        assert_eq!(
            final_msg.content,
    ...
    fn synthetic_error_event_shape() {
        let ev = synthetic_error_event("boom", "glm-test", ErrorKind::RateLimited);
        match ev {
            AssistantMessageEvent::Error { error } => {
                assert_eq!(error.error_kind, Some(ErrorKind::RateLimited));
                assert_eq!(error.stop_reason, StopReason::Error);
                assert_eq!(error.provider, "zai");
            }
