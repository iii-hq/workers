//! AgentMessage[] → Z.AI Chat Completions wire shape. Port of the TS
...
/// (Z.AI rejects assistant `tool_calls` without a tool message per id).
...
pub fn to_wire_messages(messages: &[AgentMessage], system_prompt: &str) -> Vec<Value> {
    ...
    // call: Z.AI rejects a user row between tool_calls and its tool rows.
    ...
    // placeholder so Z.AI never sees an unanswered tool_call_id.
    ...
    for m in messages {
        match m {
            ...
            AgentMessage::Assistant(a) => {
                ...
                // row — exactly where Z.AI expects the tool messages.
...
mod tests {
    ...
    fn user_message_between_call_and_result_keeps_tool_row_adjacent() {
        ...
        // transcript. Z.AI rejects a user row between the assistant tool_calls
        // row and its tool rows.
