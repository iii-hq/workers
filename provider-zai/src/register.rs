...
pub fn declaration() -> ProviderDeclaration {
    ProviderDeclaration {
        ...
        // Z.AI has no models-listing endpoint, but "listing" here gates the
        ...
        worker_id: Some("provider-zai".into()),
    }
}
...
async fn persist_registration_token(iii: &IIIClient, token: &str) -> Result<(), Error> {
    ...
    for attempt in 0..5 {
        match state::store_token(iii, token).await {
            ...
            Err(e) if attempt < 4 => {
                eprintln!(
                    "[provider-zai] store registration_token failed ({e}); retrying in {delay:?}"
                );
...
pub async fn declare_with_backoff(iii: IIIClient) {
    ...
    loop {
        match declare_once(&iii).await {
            Ok(()) => {
                println!("[provider-zai] registered with llm-router");
                return;
            }
            Err(e) => {
                eprintln!("[provider-zai] register failed ({e}); retrying in {delay:?}");
            }
        }
        tokio::time::sleep(delay).await;
