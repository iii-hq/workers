//! z.ai Chat Completions streaming via provider-base.
...
pub struct ZaiConfig {
    ...
    /// <value>`. Populated via [`ZaiConfig::with_credential`] in P5; the
    /// legacy [`ZaiConfig::from_env`] constructor still reads the env var
    /// directly.
    pub api_key: String,
    pub model: String,
    pub max_tokens: u32,
...
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<ZaiConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| ZaiConfig::with_credential(model, cred),
        stream,
    );
    Ok(())
}
...
mod tests {
    ...
    /// so splitting these caused a race on `ZAI_API_KEY`. Coverage is identical
    /// when sequenced.
    #[test]
    fn config_env_resolution() {
        let prev = std::env::var("ZAI_API_KEY").ok();

        std::env::remove_var("ZAI_API_KEY");
        assert!(ZaiConfig::from_env("test-model").is_err());

        std::env::set_var("ZAI_API_KEY", "test-key");
        let cfg = ZaiConfig::from_env("test-model").expect("ok");
    ...
    fn with_credential_api_key() {
        ...
        let cfg = ZaiConfig::with_credential("the-model", &cred).unwrap();
