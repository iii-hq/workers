    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ConfigError::NotConfigured => f.write_str(
                "provider zai not configured (no api_key in the llm-router entry \
                 and ZAI_API_KEY unset)",
            ),
            ConfigError::InvalidApiUrl(u) => write!(
                f,
                "provider zai has an invalid endpoint url: {u:?} \
...
/// must agree on it. Z.AI takes `Authorization: Bearer` for both shapes.
