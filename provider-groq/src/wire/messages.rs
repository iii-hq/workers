...
//!     dead tokens.
//!
//! Cache invariant: Groq's automatic prompt cache hits on shared request
...
const IMAGE_PLACEHOLDER: &str = "[image omitted: Groq takes text input only]";
...
/// Flatten content blocks to the plain string Groq accepts: text verbatim,
...
mod tests {
    ...
    /// Cache pin (module doc § Cache invariant): Groq's automatic prompt
    ...
    fn images_degrade_to_a_marker_and_content_stays_a_plain_string() {
        // Groq documents no multimodal content-part array; sending
        ...
        assert_eq!(
            content,
            "what is this\n[image omitted: Groq takes text input only]"
        );
