...
pub fn declaration() -> ProviderDeclaration {
    ProviderDeclaration {
        ...
        // `GET /models` exists upstream; this also gates the router's
        ...
        worker_id: Some("provider-groq".into()),
    }
}
...
async fn persist_registration_token(iii: &IIIClient, token: &str) -> Result<(), Error> {
    ...
    for attempt in 0..5 {
        match state::store_token(iii, token).await {
            ...
            Err(e) if attempt < 4 => {
                eprintln!(
                    "[provider-groq] store registration_token failed ({e}); retrying in {delay:?}"
                );
...
pub async fn declare_with_backoff(iii: IIIClient) {
    ...
    loop {
        match declare_once(&iii).await {
            Ok(()) => {
                println!("[provider-groq] registered with llm-router");
                return;
            }
            Err(e) => {
                eprintln!("[provider-groq] register failed ({e}); retrying in {delay:?}");
            }
        }
        tokio::time::sleep(delay).await;
...
/// Register, then discover the catalog. The declaration carries no models, so
/// the slice is empty until this refresh lands; failures are logged and left
/// to the next config-change refresh.
pub async fn declare_and_refresh(iii: IIIClient, http: reqwest::Client) {
    ...
    match refresh_models(&iii, &http).await {
        Ok(count) => println!("[provider-groq] catalog refreshed: {count} models"),
        Err(e) => eprintln!("[provider-groq] post-register refresh failed ({e})"),
    }
}
...
pub async fn register_provider(iii: IIIClient) -> Result<(), Error> {
    ...
    // Groq holds an overloaded request open with `: keep-alive` SSE
    ...
    iii.register_function(
        ...
        RegisterFunction::new_async(|req: crate::count_tokens::CountTokensRequest| async move {
            crate::count_tokens::handle(req).await
        })
...
mod tests {
    ...
    fn declaration_uses_credential_env_var_const() {
        assert_eq!(super::CREDENTIAL_ENV_VAR, "GROQ_API_KEY");
        assert_eq!(
    ...
    #[test]
    fn declaration_defaults_point_at_the_documented_endpoint() {
        let defaults = declaration().defaults.expect("declared defaults");
        assert_eq!(
        ...
        assert!(declaration().models.is_none(), "catalog comes from refresh");
    }
}
