...
/// Groq-only finish reason: the server ran out of capacity part-way
/// through generation (api-docs.groq.com, create-chat-completion). The
...
pub fn handle_chunk(
    ...
    if let Some(finish) = choice.get("finish_reason").and_then(Value::as_str) {
        ...
        if finish == "content_filter" {
            state
                .warnings
                .push("groq filtered the completion (finish_reason: content_filter)".to_string());
        }
        close_open_block(state, model, &mut events);
        if finish == INSUFFICIENT_RESOURCE {
            ...
            let msg = "groq ran out of system resources mid-generation \
...
mod tests {
    ...
    fn run(chunks: &[Value]) -> (PartialState, Vec<AssistantMessageEvent>) {
        ...
        let mut events = Vec::new();
        for c in chunks {
            events.extend(handle_chunk(c, &mut state, "groq-test"));
        }
        (state, events)
    }
    ...
    fn text_stream_produces_start_delta_end_and_final_content() {
        ...
        let final_msg = build_final(&state, "groq-test");
        assert_eq!(
            final_msg.content,
    ...
    fn tool_calls_sit_in_arrival_order_between_thinking_and_text() {
        ...
        let kinds: Vec<&str> = build_final(&state, "groq-test")
            .content
            .iter()
            .map(|b| match b {
    ...
    fn resumed_tool_call_arguments_reopen_the_owning_segment() {
        ...
        let content = build_final(&state, "groq-test").content;
        assert_eq!(
            content.len(),
            2,
    ...
    fn input_is_the_cache_miss_slice_so_cached_tokens_are_billed_once() {
        // Groq-native: the split is reported directly.
        ...
        // Both spellings present: Groq's own fields win.
        let mut usage = Usage::default();
        merge_usage(
    ...
    fn synthetic_error_event_shape() {
        let ev = synthetic_error_event("boom", "groq-test", ErrorKind::RateLimited);
        match ev {
            AssistantMessageEvent::Error { error } => {
