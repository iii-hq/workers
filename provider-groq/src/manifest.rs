//! Registry-publish manifest emitted by `provider-groq --manifest`
//! (binary-worker.md § manifest; same shape as provider-anthropic/src/manifest.rs).
use serde::Serialize;

const DESCRIPTION: &str = "Groq Chat Completions provider worker behind llm-router.";
...
mod tests {
    ...
    fn json_roundtrip_has_required_fields() {
        ...
        assert_eq!(parsed["name"], "provider-groq");
