...
async fn run_upstream(
        ...
        Err(e) => {
            let _ = tx
                .send(synthetic_error_event(
                    &format!("groq fetch failed: {}", error_chain(&e)),
                    &args.model,
                    ErrorKind::Transient,
                ))
                .await;
    ...
    // so a multi-byte character (Groq output is frequently CJK) can split
...
mod tests {
    ...
    async fn happy_stream_yields_start_through_stop_and_done() {
        ...
        match events.last() {
            Some(AssistantMessageEvent::Done { message }) => {
                ...
                // prompt_tokens 12 = 8 miss + 4 hit; `input` is the miss slice
        ...
        // exactly one terminal — and the `: keep-alive` comment block produced
        // no frames of its own
        assert_eq!(events.iter().filter(|e| e.is_terminal()).count(), 1);
    }

    #[tokio::test(flavor = "multi_thread")]
    async fn http_402_out_of_balance_is_a_permanent_error_frame() {
        let url = stub(
    ...
    async fn builder_error_surfaces_its_source_not_just_builder_error() {
        ...
        match &events[0] {
            AssistantMessageEvent::Error { error } => {
                ...
                assert!(msg.starts_with("groq fetch failed: "), "got {msg:?}");
                // The source chain was appended past the bare "builder error".
                assert_ne!(msg, "groq fetch failed: builder error", "source dropped");
    ...
    async fn connect_failure_yields_transient_error_frame() {
        ...
        let dead = {
            ...
            format!("http://{}/chat/completions", l.local_addr().unwrap())
        };
