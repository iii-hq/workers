//! Groq Chat Completions streaming via provider-base.
...
pub struct GroqConfig {
    ...
    /// <value>`. Populated via [`GroqConfig::with_credential`] in P5; the
    /// legacy [`GroqConfig::from_env`] constructor still reads the env
    /// var directly.
    pub api_key: String,
    pub model: String,
    pub max_tokens: u32,
...
/// Register `provider::groq::stream` on the iii bus.
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<GroqConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| GroqConfig::with_credential(model, cred),
        stream,
    );
    Ok(())
}
...
mod tests {
    ...
    fn config_from_env_behavior() {
        ...
        assert!(GroqConfig::from_env("test-model").is_err());
        std::env::set_var(ENV_VAR, "test-key");
        let cfg = GroqConfig::from_env("test-model").unwrap();
    ...
    fn with_credential_api_key() {
        ...
        let cfg = GroqConfig::with_credential("the-model", &cred).unwrap();
