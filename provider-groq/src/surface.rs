//! Wire-surface catalog for the `provider::groq::*` functions — the single
...
pub const STREAM_ID: &str = "provider::groq::stream";
pub const STREAM_DESC: &str =
    "Stream a Groq chat completion: resolve credentials, call the upstream Chat \
     Completions API, and relay AssistantMessageEvent frames to writer_ref.";

pub const ABORT_ID: &str = "provider::groq::abort";
...
pub const REFRESH_MODELS_ID: &str = "provider::groq::refresh_models";
pub const REFRESH_MODELS_DESC: &str =
    "Reconcile the Groq catalog slice through the router: list the upstream models, \
     enrich each with local metadata, and return the model count written.";

pub const COUNT_TOKENS_ID: &str = "provider::groq::count_tokens";
pub const COUNT_TOKENS_DESC: &str =
    "Count prompt tokens for {model, system_prompt?, tools?, messages} locally with \
     Groq's own published vocabulary; never runs the model and costs nothing.";

pub const ON_ROUTER_READY_ID: &str = "provider::groq::on_router_ready";
