//! Wire-schema snapshots for the `provider::groq::*` functions.
//!
//! `provider_groq::surface::catalog()` is the single source of truth for
...
use provider_groq::surface::{catalog, FunctionSpec};
...
fn catalog_lists_all_functions_in_registration_order() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "provider::groq::abort",
            "provider::groq::refresh_models",
            "provider::groq::on_router_ready",
            "provider::groq::count_tokens",
        ]
    );
}
