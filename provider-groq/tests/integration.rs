...
async fn spawn_engine() -> Option<Engine> {
    let bin = engine_bin()?;
    let port = free_port();
    let dir = std::env::temp_dir().join(format!("provider-groq-it-{}", uuid::Uuid::new_v4()));
...
/// Serves Groq's two endpoints: `GET /models` for discovery and
...
async fn boot_stack(engine_url: &str) -> (IIIClient, IIIClient) {
    ...
    loop {
        ...
        let registered = list["providers"]
            .as_array()
            .is_some_and(|p| p.iter().any(|x| x["id"] == "groq"));
        if registered {
            break;
        }
        assert!(
...
async fn refresh_and_wait(router_iii: &IIIClient, provider_iii: &IIIClient, expect_id: &str) {
    let res = call(provider_iii, "provider::groq::refresh_models", json!({}))
        .await
...
async fn provider_registers_with_persisted_token_and_credential_gated_catalog() {
    ...
    loop {
        let token = call(
            &provider_iii,
            "state::get",
            json!({ "scope": "provider-groq", "key": "registration_token" }),
        )
        .await
        .unwrap();
...
async fn chat_streams_end_to_end_with_cost_fill() {
    ...
    refresh_and_wait(&router_iii, &provider_iii, "llama-3.3-70b-versatile").await;
    ...
    // Groq's own cache-split spelling survived the whole path: the hit
...
async fn refresh_models_discovers_the_live_catalog() {
    ...
    // known rows carry the hand-maintained metadata, and the window comes
    // from the live listing rather than the local snapshot
    let known = models
        .iter()
