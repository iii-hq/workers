...
#[command(
    ...
    about = "Sarvam AI provider worker behind llm-router: chat, speech to text, text to speech."
)]
struct Cli {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.manifest {
        println!(
            "{}",
            serde_json::to_string_pretty(&provider_sarvam::manifest::build_manifest())?
        );
        return Ok(());
    }
    ...
    if let Ok(contents) = std::fs::read_to_string(&cli.config) {
        if has_config_keys(&contents) {
            tracing::warn!(
                path = %cli.config,
                "provider-sarvam takes no file-based config; configure the provider \
    ...
    let iii = register_worker(
        ...
        InitOptions {
            metadata: Some(WorkerMetadata {
                ...
                name: "provider-sarvam".to_string(),
    ...
    tracing::info!(url = %cli.url, "provider-sarvam registered");
