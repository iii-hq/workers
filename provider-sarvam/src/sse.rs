...
pub struct PartialState {
    text: String,
    /// Accumulated `delta.reasoning_content` (Sarvam AI reasoning models).
    thinking: String,
...
/// Last-wins merge: Sarvam AI reports usage natively on the final SSE chunk;
...
pub fn handle_chunk(
    ...
    if let Some(finish) = choice.get("finish_reason").and_then(Value::as_str) {
        ...
        if finish == "content_filter" {
            state
                .warnings
                .push("sarvam filtered the completion (finish_reason: content_filter)".to_string());
        }
        close_open_block(state, model, &mut events);
    }
    events
    ...
    fn synthetic_error_event_shape() {
        ...
        match ev {
            AssistantMessageEvent::Error { error } => {
                ...
                assert_eq!(error.stop_reason, StopReason::Error);
                assert_eq!(error.provider, "sarvam");
            }
