//! Catalog reconcile. Sarvam exposes no models-listing endpoint, so the
//! curated table (curated.rs) is the source of truth for the id list;
//! refresh pushes it through the router's single write path. The configured
