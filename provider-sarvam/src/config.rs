    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ConfigError::NotConfigured => f.write_str(
                "provider sarvam not configured (no api_key in the llm-router entry \
                 and SARVAM_API_KEY unset)",
            ),
            ConfigError::InvalidApiUrl(u) => write!(
                f,
                "provider sarvam has an invalid endpoint url: {u:?} \
...
/// must agree on it. Sarvam takes the same key as a bearer token and as api-subscription-key.
...
) -> Result<SarvamConfig, ConfigError> {
