        ...
        Err(e) => {
            let _ = tx
                .send(synthetic_error_event(
                    &format!("sarvam fetch failed: {}", error_chain(&e)),
                    &args.model,
                    ErrorKind::Transient,
                ))
                .await;
    ...
    async fn builder_error_surfaces_its_source_not_just_builder_error() {
        ...
        match &events[0] {
            AssistantMessageEvent::Error { error } => {
                ...
                assert!(msg.starts_with("sarvam fetch failed: "), "got {msg:?}");
                assert_ne!(msg, "sarvam fetch failed: builder error", "source dropped");
