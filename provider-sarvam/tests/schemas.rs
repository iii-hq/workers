//! Wire-schema snapshots for the `provider::sarvam::*` functions.
//!
//! `provider_sarvam::surface::catalog()` is the single source of truth for each
...
use provider_sarvam::surface::{catalog, FunctionSpec};
...
fn catalog_lists_all_functions_in_registration_order() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "provider::sarvam::abort",
            "provider::sarvam::refresh_models",
            "provider::sarvam::count_tokens",
            "provider::sarvam::transcribe",
            "provider::sarvam::speak",
            "provider::sarvam::on_router_ready",
        ]
    );
}
