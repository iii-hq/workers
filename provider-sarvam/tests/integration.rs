...
async fn boot_stack(engine_url: &str) -> (IIIClient, IIIClient) {
    ...
    loop {
        ...
        let registered = list["providers"]
            .as_array()
            .is_some_and(|p| p.iter().any(|x| x["id"] == "sarvam"));
        if registered {
            break;
        }
        assert!(
...
async fn refresh_and_wait(router_iii: &IIIClient, provider_iii: &IIIClient, expect_id: &str) {
    let res = call(provider_iii, "provider::sarvam::refresh_models", json!({}))
        .await
...
async fn provider_registers_with_persisted_token_and_credential_gated_catalog() {
    ...
    loop {
        let token = call(
            &provider_iii,
            "state::get",
            json!({ "scope": "provider-sarvam", "key": "registration_token" }),
        )
        .await
        .unwrap();
...
async fn refresh_models_reconciles_curated_catalog() {
    ...
    let curated: Vec<String> = provider_sarvam::curated::models()
        .into_iter()
        .map(|m| m.id)
        .collect();
    for id in &curated {
    ...
    let flagship = models.iter().find(|m| m["id"] == "sarvam-105b").unwrap();
    assert_eq!(flagship["context_window"], 128_000);
    assert_eq!(flagship["supports_structured_output"], false);
    assert_eq!(flagship["supports_xhigh"], false);
    assert!(flagship["pricing"].is_null());
