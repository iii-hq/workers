...
use crate::codex::events_types::{CommandExecutionItem, McpToolCallItem, ThreadItemDetails};
...
pub fn function_id(details: &ThreadItemDetails) -> String {
    match details {
        ThreadItemDetails::CommandExecution(_) => "codex::shell".to_string(),
        ThreadItemDetails::FileChange(_) => "codex::apply_patch".to_string(),
        ThreadItemDetails::WebSearch(_) => "codex::web_search".to_string(),
        ThreadItemDetails::McpToolCall(m) => format!("{}::{}", server_or(m, "mcp"), tool_or(m)),
        _ => "codex::item".to_string(),
    }
}
...
pub fn map_usage(u: &crate::codex::events_types::Usage) -> WireUsage {
    WireUsage {
