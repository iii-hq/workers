...
/// session_id -> live run. The single handle codex::stop targets.
...
fn codex_bin(cfg: &Config) -> String {
    if cfg.codex_executable.is_empty() {
        "codex".to_string()
    } else {
        cfg.codex_executable.clone()
    }
}
...
pub async fn run(iii: IIIClient, cfg: Arc<Config>, req: RunRequest) -> Value {
    ...
    // output_schema → temp file (codex takes a path, not inline). Held until
    // the child exits.
    let schema_file = match &req.output_schema {
    ...
    let argv = args::build_args(&opts, prior_thread.as_deref(), schema_path.as_deref());
    ...
    let mut child = match Command::new(codex_bin(&cfg))
        .args(&argv)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
...
async fn stream_turn(
    ...
    let stdout = match child.stdout.take() {
        ...
        None => {
            ...
            outcome.result_text = "codex produced no stdout".to_string();
            return outcome;
        }
    };
...
fn write_schema(schema: &Value) -> anyhow::Result<tempfile::NamedTempFile> {
    ...
    f.write_all(serde_json::to_string(schema)?.as_bytes())?;
    f.flush()?;
    Ok(f)
}
