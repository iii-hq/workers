...
def load_yaml(path):
    """Parse a versioned YAML file for structural assertions."""
...
class TemplateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        """Load the shared Compose inventory once for the structural suite."""
    ...
    def test_local_stack_matches_harness_and_adds_browser(self):
        """Every declared worker must build from the expected sibling source."""
    ...
    def test_dependencies_are_declared_before_consumers(self):
        """Compose dependency ordering must be valid and Harness must start last."""
        seen = set()
        for name, container in self.containers.items():
    ...
    def test_default_env_file_exists_and_has_no_credentials(self):
        """The tracked environment example must be available and contain no keys."""
        for name, container in self.containers.items():
            self.assertEqual(
                container["env_file"],
    ...
    def test_configuration_seeds_are_loaded_from_compose_directory(self):
        """Worker commands must load seeds from the project, not worker sources."""
    ...
    def test_local_skills_are_protected_from_registry_downloads(self):
        """Downloaded worker skills must not replace the local Harness namespace."""
    ...
    def test_console_defaults_to_loopback(self):
        """The committed Console seed must not expose the UI to the network."""
    ...
    def test_agent_profiles_resolve_all_preloaded_skills(self):
        """Each versioned profile must reference real skills and valid function IDs."""
    ...
    def test_generated_snapshot_matches_sync_manifest(self):
        """Versioned generated bytes must match provenance without including config."""
    ...
    def test_all_upstream_skill_documents_are_present(self):
        """The imported skill set must contain nonempty regular Markdown files."""
    ...
    def test_git_tracks_seeds_and_instructions_but_ignores_runtime_files(self):
        """Git rules must distinguish deliverables from local credentials and data."""
        tracked = [
