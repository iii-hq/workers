//! Registry-publish manifest emitted by `provider-llamacpp --manifest`
//! (binary-worker.md § manifest; same shape as provider-anthropic/src/manifest.rs).
use serde::Serialize;

const DESCRIPTION: &str =
    "llama.cpp server (llama-server) Chat Completions provider worker behind llm-router.";
...
mod tests {
    ...
    fn json_roundtrip_has_required_fields() {
        ...
        assert_eq!(parsed["name"], "provider-llamacpp");
