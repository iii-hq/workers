//! AgentMessage[] → llama.cpp Chat Completions wire shape. Port of the TS
...
/// (most Chat Completions servers, llama.cpp included, expect a tool message per id).
...
pub fn to_wire_messages(messages: &[AgentMessage], system_prompt: &str) -> Vec<Value> {
    ...
    // call: a user row between tool_calls and its tool rows breaks that shape.
    ...
    // placeholder so llama.cpp never sees an unanswered tool_call_id.
    ...
    for m in messages {
        match m {
            ...
            AgentMessage::Assistant(a) => {
                ...
                // row — exactly where the Chat Completions shape expects the tool messages.
...
mod tests {
    ...
    fn user_message_between_call_and_result_keeps_tool_row_adjacent() {
        ...
        // transcript. A user row must never land between the assistant
