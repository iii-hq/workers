//! `provider-llamacpp` binary entry.
//!
...
#[command(
    ...
    about = "llama.cpp server (llama-server) Chat Completions provider worker behind llm-router."
)]
struct Cli {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.manifest {
        println!(
            "{}",
            serde_json::to_string_pretty(&provider_llamacpp::manifest::build_manifest())?
        );
        return Ok(());
    }
    ...
    if let Ok(contents) = std::fs::read_to_string(&cli.config) {
        if has_config_keys(&contents) {
            tracing::warn!(
                path = %cli.config,
                "provider-llamacpp takes no file-based config; configure the provider \
    ...
    let iii = register_worker(
        ...
        InitOptions {
            metadata: Some(WorkerMetadata {
                ...
                name: "provider-llamacpp".to_string(),
    ...
    tracing::info!(url = %cli.url, "provider-llamacpp registered");
