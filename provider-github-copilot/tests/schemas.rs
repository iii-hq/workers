//! Wire-schema snapshots for the `provider::github-copilot::*` functions.
//!
//! `provider_github_copilot::surface::catalog()` is the single source of truth for each
...
use provider_github_copilot::surface::{catalog, FunctionSpec};
...
fn catalog_lists_all_functions_in_registration_order() {
    ...
    assert_eq!(
        ids,
        vec![
            "provider::github-copilot::stream",
            "provider::github-copilot::abort",
            "provider::github-copilot::refresh_models",
            "provider::github-copilot::on_router_ready",
            "provider::github-copilot::login::start",
            "provider::github-copilot::login::poll",
        ]
    );
}
