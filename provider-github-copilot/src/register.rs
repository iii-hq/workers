...
async fn persist_registration_token(iii: &IIIClient, token: &str) -> Result<(), Error> {
    ...
    for attempt in 0..5 {
        match state::store_token(iii, token).await {
            ...
            Err(e) if attempt < 4 => {
                eprintln!(
                    "[provider-github-copilot] store registration_token failed ({e}); retrying in {delay:?}"
                );
...
pub async fn declare_with_backoff(iii: IIIClient) {
    ...
    loop {
        match declare_once(&iii).await {
            Ok(()) => {
                println!("[provider-github-copilot] registered with llm-router");
                return;
            }
            Err(e) => {
                eprintln!("[provider-github-copilot] register failed ({e}); retrying in {delay:?}");
            }
        }
        tokio::time::sleep(delay).await;
...
/// carries no models, so the slice is empty until this refresh lands (and
/// stays empty until a login exists); failures are logged and left to the
/// next refresh.
pub async fn declare_and_refresh(iii: IIIClient, http: reqwest::Client, cache: BearerCache) {
...
pub async fn register_provider(iii: IIIClient) -> Result<(), Error> {
    ...
    // Copilot bearer cache shared by streaming and discovery.
    ...
    iii.register_function(
        ...
        RegisterFunction::new_async(make_login_start(http.clone()))
            .description(surface::LOGIN_START_DESC),
    );
    // login::poll refreshes the catalog on success so the picker fills the
    // moment the operator finishes signing in.
    {
        let poll = make_login_poll(iii.clone(), http.clone());
        let (iii_poll, http_poll, cache_poll) = (iii.clone(), http.clone(), cache.clone());
        iii.register_function(
            surface::LOGIN_POLL_ID,
            RegisterFunction::new_async(move |req: crate::login::LoginPollRequest| {
                let out = poll(req);
                ...
                async move {
                    ...
                    if resp.status == "ok" {
                        tokio::spawn(async move {
                            match refresh_models(&iii, &http, &cache).await {
                                Ok(count) => println!(
                                    "[provider-github-copilot] signed in; catalog refreshed: {count} models"
                                ),
                                Err(e) => eprintln!(
                                    "[provider-github-copilot] post-login refresh failed ({e})"
                                ),
                            }
                        });
                    }
    ...
    // Re-declare when the router restarts: bind to the router::ready trigger type.
    {
        ...
        iii.register_function(
            ...
            RegisterFunction::new_async(move |_event: RouterReadyEvent| {
                let (iii, http, cache) =
                    (iii_ready.clone(), http_ready.clone(), cache_ready.clone());
                async move {
                    tokio::spawn(declare_and_refresh(iii, http, cache));
