...
//!
//! Thinking models on this wire stream reasoning as a `reasoning` string on
//! the delta; some OpenAI-compatible gateways reachable via `api_url` use
//! the older `reasoning_content` field. Both are surfaced as a Thinking
//! block (ThinkingStart/Delta/End) preceding the answer's Text block.
...
/// Upper bound on parallel tool calls in one assistant turn; the gateway
/// relays several vendors' raw chunks, so the `index` field is untrusted input.
...
fn build_content(state: &PartialState) -> Vec<ContentBlock> {
    let mut out = Vec::new();
    // Thinking precedes the answer text (reasoning streams first). This wire
    // carries no replay signature, so it stays None.
...
pub fn merge_usage(raw: &Value, into: &mut Usage) {
    ...
    // Usage accounting: the actual billed USD, cache discounts included —
    // strictly better than a catalog-pricing estimate, so it wins downstream.
    if let Some(v) = raw.get("cost").and_then(Value::as_f64) {
        into.cost_usd = Some(v);
    }
}
...
/// The reasoning text on a delta: the normalized `reasoning` string, falling
/// back to the older `reasoning_content` used by some gateways.
...
pub fn handle_chunk(
    ...
    // Mid-stream error envelope (the gateway relays vendor failures as an
    // {"error": {...}} chunk): terminal error frame carrying the partial
    // content.
    ...
    if let Some(finish) = choice.get("finish_reason").and_then(Value::as_str) {
        ...
        if finish == "content_filter" {
            state.warnings.push(
                "copilot filtered the completion (finish_reason: content_filter)".to_string(),
            );
        }
        close_open_block(state, model, &mut events);
    }
...
mod tests {
    ...
    fn run(chunks: &[Value]) -> (PartialState, Vec<AssistantMessageEvent>) {
        ...
        let mut events = Vec::new();
        for c in chunks {
            events.extend(handle_chunk(c, &mut state, "copilot/model-x"));
        }
        (state, events)
    }
    ...
    fn text_stream_produces_start_delta_end_and_final_content() {
        ...
        let final_msg = build_final(&state, "copilot/model-x");
        assert_eq!(
            final_msg.content,
    ...
    #[test]
    fn normalized_reasoning_streams_as_thinking_then_text_closes_it() {
        let (state, events) = run(&[
            json!({"choices":[{"index":0,"delta":{"role":"assistant","reasoning":"Let me"}}]}),
            json!({"choices":[{"index":0,"delta":{"reasoning":" think"}}]}),
    ...
    #[test]
    fn legacy_reasoning_content_field_is_honored_too() {
        let (state, events) = run(&[
    ...
    fn mid_stream_error_chunk_is_terminal_with_partial_content() {
        let (_, events) = run(&[
            json!({"choices":[{"index":0,"delta":{"content":"par"}}]}),
            json!({"error":{"code":502,"message":"Provider returned error"}}),
        ]);
    ...
    #[test]
    fn billed_cost_lands_on_cost_usd() {
        let (state, events) = run(&[
    ...
    fn synthetic_error_event_shape() {
        let ev = synthetic_error_event("boom", "copilot/model-x", ErrorKind::RateLimited);
        match ev {
            AssistantMessageEvent::Error { error } => {
                assert_eq!(error.error_kind, Some(ErrorKind::RateLimited));
                assert_eq!(error.stop_reason, StopReason::Error);
                assert_eq!(error.provider, "github-copilot");
            }
