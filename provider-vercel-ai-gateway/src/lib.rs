...
pub struct VercelAiGatewayConfig {
    ...
    /// <value>`. Populated via [`VercelAiGatewayConfig::with_credential`]
    /// in P5; the legacy [`VercelAiGatewayConfig::from_env`] constructor
...
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<VercelAiGatewayConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| {
            VercelAiGatewayConfig::with_credential(model, cred)
        },
        stream,
    );
    Ok(())
...
mod tests {
    ...
    fn with_credential_api_key() {
        ...
        let cfg = VercelAiGatewayConfig::with_credential("the-model", &cred).unwrap();
