...
| **Cap** | The unconditional safety ceiling: any single function result over `max_result_tokens` tokens (default 20000; 0 disables it) is rewritten to a `[…result capped: was ~N tokens; middle omitted; re-call {function_id} with narrower arguments if the omitted middle is needed]` head+marker+tail view. No exemptions except a `details` payload carrying `"status": "denied"`, which keeps its details. |
