...
| **Cap** | The unconditional safety ceiling: any single function result over `max_result_tokens` tokens (default 20000; 0 disables it) is rewritten to a `[…result capped: was ~N tokens; middle omitted; re-call {function_id} for the full data]` head+marker+tail view. No exemptions except a `details` payload carrying `"status": "denied"`, which keeps its details. |
