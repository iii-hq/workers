//! Embed and register the judge console configuration form.
use iii_console_ui::ConsoleUi;
use iii_sdk::IIIClient;
use std::sync::Arc;

pub const PAGE_PATH: &str = "judge/page.js";
pub const STYLES_PATH: &str = "judge/styles.css";
...
mod tests {
    ...
    fn embedded_styles_are_scoped() {
        assert!(
            STYLES_CSS.contains(r#"[data-iii-ui="judge"]"#)
                || STYLES_CSS.contains("[data-iii-ui=judge]"),
            "styles.css must be scoped to the judge worker"
        );
    }
}
