...
pub struct JudgeConfig {
    ...
    pub provider: String,
    /// Keep every local provider's model loaded (judge-semif, judge-laya), not
    /// only the default provider's, so requests can name any of them at run
    /// time. Off, a local provider loads its model only while it is the
    /// default. Hosted providers are unaffected. Off is not stored.
