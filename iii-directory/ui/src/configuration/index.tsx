...
function JudgeQuestionField({
  ...
  const hint =
    'How the judge is asked about each capability. Choice asks one question over the 16-function Hybrid shortlist and lets the functions compete (16× fewer questions than yes/no). A judge that advertises a context window under 4096 tokens (laya) is handled automatically under Choice: it plays a tournament over the whole catalog, with compact options (function id and the first eight words of its description), because the shortlist\'s near-duplicates confuse it. SemIf and hosted judges (JEV) keep the shortlist. Tournament skips the shortlist for any judge: compact rounds over the whole catalog (groups of 128, or 16 for small-window judges and fewer when the judge advertises a lower option limit, three survivors each), then a final Choice over the survivors with their full descriptions; it needs no local semantic model.'
  const presentation = fieldPresentation(field, hint, errors)
  return (
    <SettingsRow
      data-field={field}
