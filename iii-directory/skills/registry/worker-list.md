...
Use `registry::worker-list` to search the public workers registry
(`api.workers.iii.dev`) by free-text term and get back a list of
...
| Question                                          | Use this              |
|---------------------------------------------------|-----------------------|
| What workers are connected to MY engine right now? | `directory::worker-list` |
| What workers exist in the public registry?         | `registry::worker-list`  |
...
`search` MUST be non-empty. (Local `directory::worker-list` allows
empty `search` since it has access to all connected workers locally.)
...
with `directory::worker-list` rows.
...
- `registry::worker-info` — full registry detail for one worker.
- `directory::worker-list` — same row shape against connected workers.
- `skills::download` — install a worker's skill bundle by name.
