...
pub fn search_catalog() -> Vec<FunctionSpec> {
    vec![
        spec::<SearchFunctionsRequest, SearchFunctionsResponse>(
            ...
            "Search available functions for the unmet external capabilities of a step (usually one to six, at most eighteen per call); returns compact function-id candidates grouped by worker, plus matching installed skill documents under `skills` and registered trigger bindings under `triggers`.",
        ),
