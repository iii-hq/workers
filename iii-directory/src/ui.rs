//! Injectable console UI for the iii-directory worker
...
//!   rule scoped under `[data-iii-ui="iii-directory"]`.
//!
//! The registration machinery (content function `iii-directory::ui-content`,
//! one Message-path trigger per asset, `III_III_DIRECTORY_UI_WATCH`
//! hot-reload watcher) lives in the shared `iii-console-ui` crate
//! (path-linked from `workers/crates/console-ui`); this module only names
...
//! For the dev loop, set `III_III_DIRECTORY_UI_WATCH` to the build output
...
pub const PAGE_PATH: &str = "iii-directory/page.js";
pub const STYLES_PATH: &str = "iii-directory/styles.css";
...
fn console_ui() -> ConsoleUi {
    ConsoleUi::new("iii-directory")
        .script(PAGE_PATH, PAGE_JS)
        .style(STYLES_PATH, STYLES_CSS)
}

/// Register the directory worker's console UI. Call after
...
mod tests {
    ...
    fn embedded_styles_are_scoped() {
        ...
        assert!(
            STYLES_CSS.contains(r#"[data-iii-ui="iii-directory"]"#)
                || STYLES_CSS.contains("[data-iii-ui=iii-directory]"),
