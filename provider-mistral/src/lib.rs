...
pub struct MistralConfig {
    ...
    /// <value>`. Populated via [`MistralConfig::with_credential`] in
    /// P5; the legacy [`MistralConfig::from_env`] constructor still
...
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<MistralConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| {
            MistralConfig::with_credential(model, cred)
        },
        stream,
    );
    Ok(())
...
mod tests {
    ...
    fn with_credential_api_key() {
        ...
        let cfg = MistralConfig::with_credential("the-model", &cred).unwrap();
