"""iii runtime context prepended to the prompt when ``iii_context`` is enabled.

Same engine-grounded rules as the harness identity prompts, retargeted to the
``iii`` CLI the agent reaches through its own shell / execute_code tool. A
caller-supplied prompt is the only input; this block is prepended on a fresh
session and skipped on resume (already in the conversation history).
"""
...
- `iii trigger engine::functions::list --json '{"search":"<term>"}'` -- every function across
  all workers; optional filters `prefix` / `search` / `worker`. Use it to FIND a function id.
