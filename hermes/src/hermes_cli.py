...
async def run_turn(hermes: str, prompt: str, *, cwd: str = "", model: str = "") -> tuple[str, str]:
    ...
    """
    code, out, err = await _run([hermes, "-z", prompt], cwd=cwd, env=_model_env(model), stdin=None)
    if code != 0:
