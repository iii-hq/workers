//! Integration with the `configuration` worker — register the github config
//! schema, fetch the live value, and hot-reload it on change. Every field is a
//! runtime tuning knob (gh binary path, GH_TOKEN credential, timeout/output
//! caps), so a change hot-swaps the whole snapshot.
...
pub const DEFAULT_CONFIG_ID: &str = "github";
...
const CONFIG_FN_ID: &str = "github::on-config-change";
const CONFIG_TIMEOUT_MS: u64 = 5_000;
const CONFIG_RETRIES: u32 = 3;

/// Register the `github` configuration schema. When `seed` is present its value
...
/// Read the live `github` configuration; built-in default when none stored.
pub async fn fetch_config(iii: &IIIClient) -> Result<Config, String> {
    match try_get_value(iii).await? {
        ...
        _ => {
            tracing::info!("no github configuration value found; using built-in defaults");
            Ok(Config::default())
        }
    }
}
...
/// The trigger payload is intentionally ignored — `github::on-config-change` is
...
async fn on_config_change(iii: &IIIClient, cell: &ConfigCell) {
    match fetch_config(iii).await {
        Ok(cfg) => {
            ...
            tracing::info!("github configuration reloaded");
        }
        Err(e) => {
