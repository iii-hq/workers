//! Registry-publish manifest emitted by `provider-anthropic --manifest`
//! (binary-worker.md § manifest; same shape as llm-router/src/manifest.rs).
use serde::Serialize;

const DESCRIPTION: &str = "Anthropic Messages API provider worker behind llm-router.";
...
mod tests {
    ...
    fn json_roundtrip_has_required_fields() {
        ...
        assert_eq!(parsed["name"], "provider-anthropic");
