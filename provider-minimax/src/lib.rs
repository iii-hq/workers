//! MiniMax Chat Completions streaming via provider-base.
...
const API_URL: &str = "https://api.minimax.chat/v1/text/chatcompletion_v2";
...
pub struct MiniMaxConfig {
    ...
    /// <value>`. Populated via [`MiniMaxConfig::with_credential`] in
    /// P5; the legacy [`MiniMaxConfig::from_env`] constructor still
...
/// Register `provider::minimax::stream` on the iii bus.
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<MiniMaxConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| {
            MiniMaxConfig::with_credential(model, cred)
        },
        stream,
    );
    Ok(())
...
mod tests {
    ...
    fn with_credential_api_key() {
        ...
        let cfg = MiniMaxConfig::with_credential("the-model", &cred).unwrap();
