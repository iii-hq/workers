...
async fn boot_stack(engine_url: &str) -> (IIIClient, IIIClient) {
    ...
    loop {
        ...
        let registered = list["providers"]
            .as_array()
            .is_some_and(|p| p.iter().any(|x| x["id"] == "merge-gateway"));
        if registered {
            break;
        }
        assert!(
...
async fn configure_stub_key(router_iii: &IIIClient, stub_url: &str) {
    call(
        ...
        json!({ "id": "llm-router", "value": { "providers": {
            "merge-gateway": { "api_key": "sk-test", "api_url": stub_url }
        } } }),
    )
    .await
    .expect("config set");
...
async fn provider_registers_with_persisted_token_and_live_only_catalog() {
    ...
    let token = call(
        &provider_iii,
        "state::get",
        json!({ "scope": "provider-merge-gateway", "key": "registration_token" }),
    )
    .await
    .unwrap();
    assert!(
...
async fn chat_streams_end_to_end_with_cost_fill() {
    ...
    assert_eq!(res["provider"], "merge-gateway");
...
async fn responses_streams_end_to_end() {
    ...
    assert_eq!(response["provider"], "merge-gateway");
