...
pub const DEFAULT_API_URL: &str = "https://api-gateway.merge.dev/v1/openai/chat/completions";
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ConfigError::NotConfigured => f.write_str(
                "provider merge-gateway not configured (no api_key in the llm-router entry \
                 and MERGE_GATEWAY_API_KEY unset)",
            ),
            ConfigError::InvalidApiUrl(u) => write!(
                f,
                "provider merge-gateway has an invalid endpoint url: {u:?} \
...
) -> Result<MergeGatewayConfig, ConfigError> {
...
mod tests {
    ...
    fn endpoint_path_selects_transport() {
        assert_eq!(
            ApiMode::from_url("https://api-gateway.merge.dev/v1/openai/responses"),
            ApiMode::Responses
        );
        assert_eq!(
