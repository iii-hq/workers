//! `provider::merge-gateway::count_tokens` — local prompt token estimation with the
...
//! itself is shared with `provider-merge-gateway-codex` in
...
pub struct CountTokensRequest {
    ...
    /// Wire agent messages, the same shape `provider::merge-gateway::stream`
