//! `provider-merge-gateway` binary entry.
//!
...
use provider_merge_gateway::register::register_provider;

#[derive(Parser, Debug)]
#[command(
    name = "provider-merge-gateway",
    about = "Merge Gateway provider worker behind llm-router."
)]
struct Cli {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.manifest {
        println!(
            "{}",
            serde_json::to_string_pretty(&provider_merge_gateway::manifest::build_manifest())?
        );
        return Ok(());
    }
    ...
    if let Ok(contents) = std::fs::read_to_string(&cli.config) {
        if has_config_keys(&contents) {
            tracing::warn!(
                path = %cli.config,
                "provider-merge-gateway takes no file-based config; configure the provider \
    ...
    let iii = register_worker(
        ...
        InitOptions {
            metadata: Some(WorkerMetadata {
                ...
                name: "provider-merge-gateway".to_string(),
    ...
    tracing::info!(url = %cli.url, "provider-merge-gateway registered");
