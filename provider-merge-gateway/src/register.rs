...
pub const CREDENTIAL_ENV_VAR: &str = "MERGE_GATEWAY_API_KEY";

pub fn declaration() -> ProviderDeclaration {
    ProviderDeclaration {
        id: PROVIDER_ID.into(),
        display_name: Some("Merge Gateway".into()),
        ...
        worker_id: Some("provider-merge-gateway".into()),
    }
}
...
async fn persist_registration_token(iii: &IIIClient, token: &str) -> Result<(), Error> {
    ...
    for attempt in 0..5 {
        match state::store_token(iii, token).await {
            ...
            Err(e) if attempt < 4 => {
                eprintln!(
                    "[provider-merge-gateway] store registration_token failed ({e}); retrying in {delay:?}"
                );
...
pub async fn declare_with_backoff(iii: IIIClient) {
    ...
    loop {
        match declare_once(&iii).await {
            Ok(()) => {
                println!("[provider-merge-gateway] registered with llm-router");
                return;
            }
            Err(e) => {
                eprintln!("[provider-merge-gateway] register failed ({e}); retrying in {delay:?}");
            }
        }
        tokio::time::sleep(delay).await;
...
pub async fn declare_and_refresh(iii: IIIClient, http: reqwest::Client) {
    ...
    match refresh_models(&iii, &http).await {
        Ok(count) => println!("[provider-merge-gateway] catalog refreshed: {count} models"),
        Err(e) => eprintln!("[provider-merge-gateway] post-register refresh failed ({e})"),
    }
}
...
mod tests {
    ...
    fn declaration_uses_credential_env_var_const() {
        assert_eq!(super::CREDENTIAL_ENV_VAR, "MERGE_GATEWAY_API_KEY");
        assert_eq!(
