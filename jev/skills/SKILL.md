...
name: jev
description: >-
  Evaluate JSON state with TypeSafe Noul, Choice or Score questions for ticket
  triage, classification and graded judgments, list provider models, or cancel
  an active JEV request owned by the caller.
---

# jev

`jev::evaluate` evaluates supplied state with mixed Noul, Choice and Score
questions. `jev::models::list` lists provider models using the same credentials
and transport. `jev::cancel` signals cancellation of an identified evaluation or
listing owned by the same caller. Install JEV on the engine and configure `api_key`
in the active JEV entry under Console Settings → Workers, or supply
`TYPESAFE_API_KEY` to the JEV worker process. The entry ID is `jev` by default,
or the worker's `III_CONFIG_NAME` (for example, `jev-prod`). The default model
is `jev-1.13.0`; callers may override it.

## Invocation

1. Read the registered contract with
   `iii trigger engine::functions::info --json '{"function_id":"jev::evaluate"}'`.
   Use `jev::models::list` or `jev::cancel` as `function_id` for those contracts.
...
To list models, invoke `iii trigger jev::models::list --json '{}'`. This uses
...
2. While it is active, invoke `jev::cancel` from that same caller with
...
Multiple JEV replicas require routing affinity so start and cancel reach the same
process. Ordinary evaluation/listing calls without IDs remain valid.

## Result and operational boundaries

- JEV evaluates supplied evidence; callers own tool discovery and fallback
  policies.
...
- Credentials stay in JEV configuration or its environment. Neither function
  accepts caller-supplied keys or provider URLs.
...
The environment fallback reads `TYPESAFE_API_KEY` from the JEV process; restart
JEV after changing that environment. A configured key takes precedence and
clearing it restores fallback. A configuration value `${TYPESAFE_API_KEY}` instead
expands in the configuration service's environment. For an existing key supplied
to the shell, standalone invocation is `jev --url ws://127.0.0.1:49134`. For YAML
seeding, hot reload and clearing limit overrides, read [Configuration](../reference.md#configuration).
