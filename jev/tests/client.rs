use jev::{ExecutionLimits, JevClient};
use jev_contract::{
...
fn request(count: usize) -> EvaluateRequest {
    EvaluateRequest {
        options: jev_contract::RequestOptions {
            retry: jev_contract::RetryPolicy {
                max_retries: 0,
                ..Default::default()
            },
            ..Default::default()
...
fn ok(
    ...
    String,
    BTreeMap<String, jev_contract::EvaluationResult>,
    Stats,
) {
    match response {
        EvaluateResponse::Ok {
...
fn models_ok(response: ModelsResponse) -> (Vec<jev_contract::ModelCard>, Stats) {
    match response {
...
async fn cancelled_call_aborts_http_and_releases_its_id() {
    ...
    assert!(matches!(
        wrong,
        jev_contract::CancelResponse::Ok { cancelled: false }
    ));
    let duplicate = error(
    ...
    assert!(matches!(
        ...
        jev_contract::CancelResponse::Ok { cancelled: true }
    ));
    let stats = error(
...
async fn cancel_interrupts_backoff_without_a_new_attempt() {
    ...
    input.options.retry = jev_contract::RetryPolicy {
    ...
    assert!(matches!(
        client.cancel(jev_contract::CancelRequest {
...
async fn default_retry_policy_recovers_rate_limits_for_evaluation_and_models() {
    for listing in [false, true] {
        ...
        .await;
        let client = server.client();
        let mut options = jev_contract::RequestOptions::default();
