//! Exercise the real binary against an isolated configuration/engine websocket.
...
#[tokio::test]
async fn boot_without_key_registers_config_schemas_reload_and_optional_console_assets() {
...
async fn check_boot(shutdown_signal: Option<&str>) {
    ...
    let server = tokio::spawn(async move {
        ...
        while let Some(Ok(frame)) = socket.next().await {
            ...
            if value["type"] == "registerfunction" && value["id"] == "jev::evaluate" {
                socket.send(Message::Text(json!({"type":"invokefunction","invocation_id":"00000000-0000-0000-0000-000000000009","function_id":"jev::evaluate","data":{"timeout_ms":1000,"evaluations":[{"id":"ticket","state":{},"questions":{"urgent":{"type":"noul","instructions":"Is this urgent?"}}}]}}).to_string().into())).await.unwrap();
            }
            if value["type"] == "registerfunction" && value["id"] == "jev::configuration-id" {
                socket.send(Message::Text(json!({"type":"invokefunction","invocation_id":"00000000-0000-0000-0000-000000000010","function_id":"jev::configuration-id","data":{"_caller_worker_id":"console-test"}}).to_string().into())).await.unwrap();
            }
        }
    });
    let mut worker = Worker(
        Command::new(env!("CARGO_BIN_EXE_jev"))
            .env("III_URL", url)
            .env("III_CONFIG_NAME", "jev-boot-test")
            .env_remove("TYPESAFE_API_KEY")
            .env_remove("III_JEV_UI_WATCH")
            .env("RUST_LOG", "info")
    ...
    timeout(Duration::from_secs(10), async {
        let mut expected_functions = BTreeSet::from([
            "jev::evaluate".to_owned(),
            "jev::models::list".to_owned(),
            "jev::cancel".to_owned(),
            "jev::on-config-change".to_owned(),
        ]);
        if console_ui_enabled {
            expected_functions.insert("jev::ui-content".to_owned());
            expected_functions.insert("jev::configuration-id".to_owned());
        }
        let mut functions = BTreeSet::new();
        let mut identity = !console_ui_enabled;
        let (mut config, mut reload, mut script, mut style, mut missing_key) = (
            false,
            false,
            !console_ui_enabled,
            !console_ui_enabled,
            ...
            && missing_key)
        {
            ...
            match value["type"].as_str().unwrap() {
                "invokefunction" if value["function_id"] == "configuration::ensure" => {
                    ...
                    assert_eq!(value["data"]["metadata"]["ui_form"], "jev");
                ...
                "registerfunction" => {
                    ...
                    if matches!(
                        id,
                        "jev::on-config-change" | "jev::ui-content" | "jev::configuration-id"
                    ) {
                ...
                "registertrigger" if value["trigger_type"] == "configuration" => {
                    assert_eq!(value["function_id"], "jev::on-config-change");
                    assert_eq!(
                        value["config"],
                        json!({
                            "configuration_id": "jev-boot-test",
                ...
                "registertrigger" if value["trigger_type"] == "console:script" => {
                    ...
                    assert_eq!(value["function_id"], "jev::ui-content");
                    assert_eq!(value["config"]["path"], "jev/page.js");
                    script = true;
                }
                "registertrigger" if value["trigger_type"] == "console:style" => {
                    assert!(console_ui_enabled);
                    assert_eq!(value["function_id"], "jev::ui-content");
                    assert_eq!(value["config"]["path"], "jev/styles.css");
                    style = true;
                }
                "invocationresult" if value["function_id"] == "jev::evaluate" => {
                    assert_eq!(value["result"]["code"], "missing_key");
                    assert!(value.get("error").is_none());
                    missing_key = true;
                }
                "invocationresult" if value["function_id"] == "jev::configuration-id" => {
                    assert!(console_ui_enabled);
                    assert_eq!(value["result"], json!({"id":"jev-boot-test"}));
                    assert!(value.get("error").is_none());
                    identity = true;
                }
                _ => {}
    ...
    .await
    .expect("boot is ready without provider credentials");

    if let Some(signal) = shutdown_signal {
        timeout(Duration::from_secs(10), async {
            while let Some(line) = logs.recv().await {
                if line.contains("JEV worker ready") {
                    return;
                }
            }
