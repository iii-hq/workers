...
async fn invoke(payload: Value) -> (Value, Value) {
    invoke_function("jev::evaluate", payload).await
}
...
async fn invoke_function(function_id: &'static str, payload: Value) -> (Value, Value) {
    invoke_client(
        function_id,
        payload,
        jev::configuration::new_cell(JevConfig::default()),
        JevClient::new(None),
    )
    .await
}
...
async fn invoke_client(
    ...
    jev::register(&iii, config, client);
...
async fn models_registration_rejects_unknown_fields_and_malformed_timeouts() {
    for payload in [
        ...
        let (_, response) = invoke_function("jev::models::list", payload).await;
        assert!(response.get("error").is_none());
...
async fn registered_handlers_apply_current_operator_limits() {
    ...
    for (function_id, payload) in [
        ("jev::evaluate", ticket()),
        ("jev::models::list", json!({"timeout_ms":1000})),
    ] {
        assert!(
        ...
        assert_eq!(response["result"]["code"], "invalid_request");
        assert_eq!(response["result"]["stats"]["attempts"], 0);
        assert!(jev::configuration::apply_config(&cell, JevConfig::default()).await);
    ...
    let (_, response) = invoke_client("jev::evaluate", ticket(), cell, client).await;
...
async fn engine_caller_metadata_is_removed_only_at_registration_boundary() {
    ...
    assert!(serde_json::from_value::<jev_contract::EvaluateRequest>(payload.clone()).is_err());
...
async fn cancel_rejects_missing_identity_invalid_ids_and_unknown_fields() {
    for payload in [
        ...
        json!({"request_id":"x".repeat(129), "_caller_worker_id":"owner"}),
        ...
        let (_, response) = invoke_function("jev::cancel", payload).await;
        assert!(response.get("error").is_none());
        assert_eq!(
            response["result"],
...
impl LiveBus {
    async fn start(config: jev::SharedConfig, client: JevClient) -> Self {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let address = format!("ws://{}", listener.local_addr().unwrap());
        ...
        assert_eq!(
            registered,
            ["jev::evaluate", "jev::models::list", "jev::cancel"]
                .map(str::to_owned)
                .into_iter()
                .collect()
        );
...
fn identified_payload(function_id: &str, caller: &str, request_id: &str) -> Value {
    let mut payload = if function_id == "jev::evaluate" {
        ticket()
    } else {
        json!({})
    };
...
async fn bus_cancellation_shares_ids_between_evaluate_and_models_and_releases_them() {
    ...
    let config = jev::configuration::new_cell(JevConfig {
    ...
    for (function_id, other) in [
        ("jev::evaluate", "jev::models::list"),
        ("jev::models::list", "jev::evaluate"),
    ] {
...
async fn bus_callers_with_the_same_id_can_only_cancel_their_own_call() {
    ...
    let first = bus.send(
        "jev::evaluate",
        identified_payload("jev::evaluate", "first", "shared"),
    );
    provider.started().await;
    let second = bus.send(
        "jev::models::list",
        identified_payload("jev::models::list", "second", "shared"),
    );
    provider.started().await;
    assert_eq!(
