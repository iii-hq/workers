...
mod tests {
    ...
    fn guidance_covers_this_worker_s_surface() {
        ...
        for needle in [
            ...
            "request_format",
            // Observed in dogfooding: an agent read the schema in
            // functions::info and assumed the boundary enforces it. It does
            // not — nothing on this bus does — and guidance that shows a
            // schema without saying so invites handlers with no input checks.
            "not call-time validation",
            // The other dogfooding finding: agents mix the two return
            // conventions and learn only from the first silent null. The
            // guidance was a JavaScript-only document until this clause.
            "assign `result = x`",
        ] {
            assert!(
