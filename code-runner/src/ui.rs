...
//!
//! - `code-runner/page.js` (`console:script`) — the function-trigger renderers
//!   its `setup(host)` registers, so `code-runner::run`, `register_function`
//!   and `teardown` render as purpose-built cards in chat and traces instead
//!   of raw JSON.
...
//!
//! The registration machinery (content function `code-runner::ui-content`, one
//! Message-path trigger per asset, `III_CODE_RUNNER_UI_WATCH` hot-reload
...
/// The content function `ConsoleUi` publishes to serve the two assets.
///
/// Declared here and passed EXPLICITLY below rather than left to
/// `ConsoleUi`'s `<worker>::ui-content` default, because this id must also be
/// SEEDED into the runtime manager's id registry (see
/// `functions::seeded_ids`). A const that merely happened to match the default
/// would silently stop matching if either side changed, and the failure mode is
/// a process abort — so the two reads come from one place.
pub const CONTENT_FUNCTION_ID: &str = "code-runner::ui-content";
...
fn console_ui() -> ConsoleUi {
    ConsoleUi::new("code-runner")
        .content_function_id(CONTENT_FUNCTION_ID)
...
mod tests {
    ...
    /// The completion value is what this worker's wire adds over
    /// `sandbox-code-runner`'s, and the `run` card is the only place a caller
    /// sees it. A stylesheet that lost the block would render it unstyled.
    #[test]
    fn the_result_block_is_styled_and_rendered() {
        assert!(
            STYLES_CSS.contains("cr-ui-result-body"),
            "styles.css lost the completion-value block"
        );
        assert!(
            PAGE_JS.contains("cr-ui-result-body"),
            "page.js never renders the completion value"
        );
    }

    /// This worker has no `network` field on any request — neither engine has
    /// any network at all — so a chip claiming one must not survive the port
    /// from sandbox-code-runner's UI, where it is a real create-time flag.
    #[test]
    fn nothing_claims_a_network_capability() {
        assert!(
            !PAGE_JS.contains("cr-run-net"),
            "the network chip was ported from sandbox-code-runner, but this \
             worker's wire has no `network` field — the chip would claim a knob \
             that does not exist"
        );
    }
}
