 ...
 *     share it. code-runner claims ids for BOTH engines in ONE registry
 *     (`RuntimeManager::ids()`), so `app::greet` cannot exist in node and
 *     python at once — that is the common surprise, and it gets its own line
 *     rather than being a substring of a blob.
 *   - whether the source actually defines `handler(payload)`. Both engines
 *     WRAP what you send: node's `wrap_register` evaluates the source and
 *     then looks for `handler`, and python's registration execs the source
 *     and then asserts `callable(handler)`. So the source DEFINES the
 *     handler; it does not register itself.
 *
 * ONE function per call: this request carries a single `function_id` +
 * `source` pair.
 *
 * NO `runtime_id` ON THIS WIRE: the runtime backing a namespace is an
 * implementation detail this call never sees or names — `lang` (required)
 * decides which engine it needs, and code-runner creates or reuses that
 * namespace's runtime itself. A python namespace's interpreter is PINNED
 * (exempt from the idle sweep), so `teardown namespace=` is the only way to
 * reclaim it.
 *
 * A python registration that leaves no callable `handler` fails the
 * REGISTRATION rather than the first call — the id never reaches the catalog,
 * and a corrected redeploy of the same id works. That is why the handler
 * advisory below matters before you approve, not after.
 *
 * No capability lives on this request, but a caller is free to name a
 * function, description, or source after a runtime id it holds from elsewhere
 * — every free-text field still goes through `redactRuntimeIds` as
...
/** The free-text fields `RegisterRequest` declares — `lang` is excluded: it is
 * a closed enum, not free text, checked separately below. */
...
interface RegisterRequest {
  ...
  lang?: Lang
  /** Fields the request carried with a non-string value — surfaced, not
   * dropped. */
  malformed: string[]
}
...
function parseRequest(input: unknown): RegisterRequest {
  ...
  return {
    ...
    malformed: STRING_FIELDS.filter((k) => k in obj && obj[k] !== null && typeof obj[k] !== 'string'),
  }
}
 ...
 * id has no `::` or nothing after it — exactly the two shapes the worker
 * refuses, so the card can say so before the call lands.
 */
 ...
 *
 * This is a HINT, never a claim. `def` is checked first because it is the one
 * marker JavaScript cannot produce.
 */
function guessPrism(source: string): string | undefined {
  if (/^[ \t]*(async[ \t]+)?def[ \t]+\w/m.test(source)) return 'python'
  if (/\bexport\s+function\b|\bfunction\s+\w|=>/.test(source)) return 'javascript'
  return undefined
}
...
function Head({
  ...
  req: RegisterRequest
  /**
   * The response's `function_id` — the id actually live on the bus. Falls back
   * into the display when the request carried none, so a card can never say
   * "no function_id in the request" beside a green `registered` badge while
   * hiding the id that is actually callable. `undefined` in the
  ...
  // covers that. `resId` only fills the gap when the request had nothing.
    ...
    <div className="cr-ui-section">
      ...
      {displayId ? (
        <div className={`cr-register-function-ns${ns ? '' : ' cr-ui-warn'}`}>
          {ns ? (
            <>
              claims <code>{redactRuntimeIds(ns)}</code> — the first registration takes the namespace, and ids are
              claimed across BOTH engines, so this id cannot also exist in the other language
            </>
          ) : (
            <>
              no namespace — an id must look like <code>app::name</code>, which this one does not
            </>
          )}
        </div>
      ) : null}
      <div className={`cr-register-function-desc${req.description ? '' : ' cr-ui-warn'}`}>
        {req.description
 ...
 * arithmetic.
 */
function SourceSection({ source, lang, clipped }: { source: string; lang?: Lang; clipped: boolean }) {
  const [expanded, setExpanded] = useState(false)
  const safe = redactRuntimeIds(source)
  const lines = safe.split('\n')
  const long = lines.length > COLLAPSE_AFTER || safe.length > SOURCE_CHAR_CAP
  const collapsed = long && !expanded
  const shown = collapsed ? lines.slice(0, COLLAPSE_AFTER).join('\n').slice(0, SOURCE_CHAR_CAP) : safe
    ...
    <div className="cr-ui-section">
      ...
      {guessed ? (
        <div className="cr-register-function-lang">
          highlighted as {guessed} — guessed from the source; this request's lang field is missing or invalid, so the
          engine it will actually run under is unconfirmed
        </div>
      ) : null}
    </div>
  )
...
/**
 * The source has to DEFINE `handler(payload)` — both engines wrap it and then
 * look the name up. Quiet, because a substring check is not a parser: it is an
 * advisory, never a verdict. Suppressed on a clipped excerpt, where the
 * definition may simply be past the cut.
 *
 * On python the consequence is sharper than on node, and the text says so: the
 * worker asserts `callable(handler)` in the same turn as the exec, so a source
 * without one fails the registration outright instead of registering and then
 * failing on every call.
 */
function HandlerAdvisory({ source, lang, clipped }: { source: string; lang?: Lang; clipped: boolean }) {
  if (clipped || /\bhandler\b/.test(source)) return null
  return (
    <div className="cr-ui-msg-note">
      · nothing named `handler` in this source —{' '}
      {lang === 'python'
        ? 'the worker execs the source and then checks callable(handler), so this registration will be REFUSED'
        : 'the worker evaluates the source and then looks for handler, so a source that never defines it cannot be registered'}
    </div>
  )
}

/** Source block plus its advisories, or the placeholder when there is none. */
function SourceBlock({ req, clipped }: { req: RegisterRequest; clipped: boolean }) {
  ...
  return (
    <>
      ...
      <HandlerAdvisory source={req.source} lang={req.lang} clipped={clipped} />
    </>
  )
}
...
function SettledView({ message }: { message: FunctionTriggerMessage }) {
  ...
  const registered = typeof res.registered === 'boolean' ? res.registered : undefined
  const resId = typeof res.function_id === 'string' ? res.function_id : undefined
  const mismatch = resId !== undefined && req.functionId !== undefined && resId !== req.functionId

  return (
    <CardShell op={opName(message.functionId)} chips={<LangChip req={req} />}>
      <Head req={req} resId={resId} status={registered === undefined ? undefined : registered ? 'live' : 'refused'} />
 ...
 */
function PendingView({ message, running }: { message: FunctionTriggerMessage; running: boolean }) {
  const req = parseRequest(message.input)
  const clipped = looksClipped(req.source)
  const anyClipped = clipped || looksClipped(req.description) || looksClipped(req.functionId)
...
export function createRegisterFunctionRenderer(host: Host): FunctionTriggerRenderer {
  ...
  const render = (message: FunctionTriggerMessage, running: boolean): React.ReactNode | null => {
    ...
    if (denied) {
      return <DeniedCard op={opName(message.functionId)} reason={denied.reason} deniedBy={denied.deniedBy} />
    }
    ...
    // redactRuntimeIds either way — belt-and-braces over trusting the default
    // view.
  ...
  return {
    ...
    tryRenderPreview: (message) =>
      isMatch(message.functionId) && message.pendingApproval && asRecord(message.input) ? (
