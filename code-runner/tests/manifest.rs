...
fn manifest_subcommand_emits_valid_json() {
    // Hyphen, not underscore: Cargo uses [[bin]].name verbatim here.
    let bin = env!("CARGO_BIN_EXE_code-runner");
    let output = Command::new(bin)
        .arg("--manifest")
        .output()
        .expect("spawn code-runner --manifest");
    ...
    assert_eq!(manifest["name"], "code-runner");
