...
class ShadowContractTest(unittest.TestCase):
    ...
    def test_ephemeral_runner_keeps_the_bootstrapped_target_binary_and_checks_runtime_identity(self):
        ...
        self.assertIn('add_with_retry "stack-$worker" "$worker@$version"', runner)
        self.assertIn('add_with_retry "repin-$worker" "$worker@$version"', runner)
        self.assertNotIn('add_with_retry "stack-$worker" "$worker@$version" --force', runner)
        self.assertNotIn('add_with_retry "repin-$worker" "$worker@$version" --force', runner)
