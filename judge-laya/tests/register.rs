//! Bus contract of the laya provider over a mocked engine socket.
#[path = "../../judge-typesafe/tests/support/fake_engine.rs"]
mod fake_engine;
mod support;
use iii_sdk::{register_worker, InitOptions};
use judge_laya::{LayaClient, LayaConfig};
use serde_json::{json, Value};
use std::{sync::Arc, time::Duration};
use tokio::{sync::mpsc, time::timeout};

async fn invoke(payload: Value) -> (Value, Value) {
    invoke_function("judge-laya::evaluate", payload).await
}
...
async fn invoke_function(function_id: &'static str, payload: Value) -> (Value, Value) {
    invoke_client(
        function_id,
        payload,
        judge_laya::configuration::new_cell(LayaConfig::default()),
        support::tiny_client(),
    )
    .await
}
...
async fn invoke_client(
    ...
    payload: Value,
    config: judge_laya::SharedConfig,
    client: LayaClient,
) -> (Value, Value) {
    ...
    judge_laya::register(&iii, config, client);
...
async fn models_and_cancel_are_registered_with_the_provider_ids() {
    let (registration, response) =
        invoke_function("judge-laya::models::list", json!({"_caller_worker_id":"w"})).await;
    assert!(registration["request_format"]["properties"]["timeout_ms"].is_object());
    assert_eq!(response["result"]["models"][0]["name"], "laya");
    ...
    let (_, response) =
        invoke_function("judge-laya::cancel", json!({"request_id":"no-caller"})).await;
...
fn provider_ids_follow_the_hub_convention() {
    ...
    use judge_laya::{CANCEL_ID, EVALUATE_ID, MODELS_ID, PROVIDER};
