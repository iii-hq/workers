//! The provider over a real engine, with the tiny checkpoint. Run with
...
use judge_laya::{CANCEL_ID, EVALUATE_ID, MODELS_ID};
use serde_json::json;

#[tokio::test]
#[ignore = "requires III_ENGINE_BIN; starts an isolated real engine, no network"]
async fn tiny_checkpoint_answers_through_a_real_engine() {
    ...
    assert_eq!(evaluated["model"], "laya");
    ...
    let listed = invoke(&consumer, MODELS_ID, json!({"timeout_ms": 3000}))
        .await
        .unwrap();
    assert_eq!(listed["models"][0]["name"], "laya", "{listed}");
