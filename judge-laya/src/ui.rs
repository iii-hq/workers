//! Embed and register the laya console configuration form.
use iii_console_ui::ConsoleUi;
use iii_sdk::IIIClient;
use std::sync::Arc;

pub const PAGE_PATH: &str = "judge-laya/page.js";
pub const STYLES_PATH: &str = "judge-laya/styles.css";
...
mod tests {
    ...
    fn embedded_styles_are_scoped() {
        assert!(
            STYLES_CSS.contains(r#"[data-iii-ui="judge-laya"]"#)
                || STYLES_CSS.contains("[data-iii-ui=judge-laya]"),
            "styles.css must be scoped to the laya worker"
        );
    }
}
