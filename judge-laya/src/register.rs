...
pub fn register(iii: &IIIClient, config: SharedConfig, slot: Arc<ModelSlot<LayaClient>>) {
    ...
    iii.register_function(crate::MODELS_ID, registration.request_format(request_schema).description("Describe the laya checkpoints, loading them if needed (name, encoder, revision, context window); the first card is the default. Performs no inference.").metadata(json!({ "internal": true })));
