...
| `sentinel` | yes | Built on the shared surface throughout: `useWorkerLive` (two worker trigger types), `useContainerNarrow`, `PageShell`/`PageHeader`/`PageMain`, `Toolbar`, `SegmentedControl`, `SearchField`, `Select`, `Table` family, `Badge`/`Chip`/`StatusDot`, `Tabs`, `Card`, `MetaRow`, `Eyebrow`, `EmptyState`/`Skeleton`/`StatusPanel`, `DropdownMenu`, `Settings*` + `DirectoryPicker` for the config form, `CodeHighlight`/`Markdown`, `lucide-react`, `errorMessage`/`formatRelative`; the 24-hour sparkline is CSS bars, not inline `<svg>` | Error groups, frozen evidence, and the investigation session beside the page |
