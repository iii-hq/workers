...
| [`injectable-console-ui.md`](injectable-console-ui.md) | Shipping worker UI into the running console at runtime — `console:script`/`console:style` triggers, the content function, `setup(host)` slots (pages, function and trigger-activity renderers, config forms), scoped CSS, hot reload. The standalone, registry-published versions live in [`console/skills/`](../../console/skills/) |
