# Contributing to judge-typesafe
...
```bash
pnpm --filter '@iii-workers/judge-typesafe-ui...' install --frozen-lockfile
pnpm --dir judge-typesafe/ui build
cargo build --manifest-path judge-typesafe/Cargo.toml --locked --release
```

The binary is `judge-typesafe/target/release/judge-typesafe` unless `CARGO_TARGET_DIR` overrides the
output directory. To build without the Console form or its Node/pnpm dependency:

```bash
cargo build --manifest-path judge-typesafe/Cargo.toml --locked --release --no-default-features
```
...
The UI registers the `judge-typesafe` configuration form. The standard Console builder
emits `ui/dist/page.js` and `ui/dist/styles.css` for Rust embedding.

```bash
pnpm --dir judge-typesafe/ui test
pnpm --dir judge-typesafe/ui build
```
...
```bash
pnpm --dir judge-typesafe/ui watch
III_JEV_UI_WATCH=judge-typesafe/ui/dist cargo run --manifest-path judge-typesafe/Cargo.toml --locked -- --url ws://127.0.0.1:49134
```
...
apply when starting from `judge-typesafe/`. Without the watch environment, rebuild the binary
...
cargo clippy --manifest-path judge-typesafe/Cargo.toml --locked --all-targets --no-default-features -- -D warnings
cargo test --manifest-path judge-typesafe/Cargo.toml --locked --no-default-features
```
...
cargo test --manifest-path judge-typesafe/Cargo.toml --locked --test register
cargo test --manifest-path judge-typesafe/Cargo.toml --locked --test boot
```
...
and source recipes here. Repository links outside `judge-typesafe/` must use absolute
