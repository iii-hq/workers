...
async fn check_boot(shutdown_signal: Option<&str>) {
    ...
    let server = tokio::spawn(async move {
        ...
        while let Some(Ok(frame)) = socket.next().await {
            ...
            if value["type"] == "registerfunction" && value["id"] == "judge-typesafe::evaluate" {
                socket.send(Message::Text(json!({"type":"invokefunction","invocation_id":"00000000-0000-0000-0000-000000000009","function_id":"judge-typesafe::evaluate","data":{"timeout_ms":1000,"evaluations":[{"id":"ticket","state":{},"questions":{"urgent":{"type":"noul","instructions":"Is this urgent?"}}}]}}).to_string().into())).await.unwrap();
            }
            if value["type"] == "registerfunction"
                && value["id"] == "judge-typesafe::configuration-id"
            {
                socket.send(Message::Text(json!({"type":"invokefunction","invocation_id":"00000000-0000-0000-0000-000000000010","function_id":"judge-typesafe::configuration-id","data":{"_caller_worker_id":"console-test"}}).to_string().into())).await.unwrap();
            }
        }
    });
    let mut worker = Worker(
        Command::new(env!("CARGO_BIN_EXE_judge-typesafe"))
            .env("III_URL", url)
    ...
    timeout(Duration::from_secs(10), async {
        let mut expected_functions = BTreeSet::from([
            "judge-typesafe::evaluate".to_owned(),
        ...
        if console_ui_enabled {
            ...
            expected_functions.insert("judge-typesafe::configuration-id".to_owned());
        }
            ...
            && missing_key)
        {
            ...
            match value["type"].as_str().unwrap() {
                ...
                "registertrigger" if value["trigger_type"] == "configuration" => {
                    assert_eq!(value["function_id"], "judge-typesafe::on-config-change");
                    assert_eq!(
                        value["config"],
                        json!({
                ...
                "registertrigger" if value["trigger_type"] == "console:script" => {
                    ...
                    assert_eq!(value["function_id"], "judge-typesafe::ui-content");
                    assert_eq!(value["config"]["path"], "judge-typesafe/page.js");
                    script = true;
                }
                "registertrigger" if value["trigger_type"] == "console:style" => {
                    assert!(console_ui_enabled);
                    assert_eq!(value["function_id"], "judge-typesafe::ui-content");
                    assert_eq!(value["config"]["path"], "judge-typesafe/styles.css");
                    style = true;
                }
                "invocationresult" if value["function_id"] == "judge-typesafe::evaluate" => {
                    assert_eq!(value["result"]["code"], "missing_key");
                    assert!(value.get("error").is_none());
                    missing_key = true;
                }
                "invocationresult"
                    if value["function_id"] == "judge-typesafe::configuration-id" =>
                {
                    assert!(console_ui_enabled);
