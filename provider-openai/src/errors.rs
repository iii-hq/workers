...
/// Map an OpenAI HTTP status + error body to the shared taxonomy.
...
pub fn classify(status: Option<u16>, message: &str) -> ErrorKind {
    if let Ok(v) = serde_json::from_str::<Value>(message) {
        if let Some(kind) = classify_openai_value(&v, status) {
            return kind;
        }
    }
    match status {
...
fn classify_openai_value(v: &Value, status: Option<u16>) -> Option<ErrorKind> {
    let err = v.get("error")?;
...
mod tests {
    ...
    #[test]
    fn openai_envelope_codes_are_honored() {
