//! Build script for the `canvas` worker.
//!
...
//!    `ui/dist/page.js`, `ui/dist/styles.css`, `ui/dist/mermaid.js` and
//!    `ui/dist/freeform.js` via `include_str!`, so if any is missing or stale
//!    we run `pnpm install && pnpm build` inside `ui/` first (the state
//!    worker's precedent). Set `SKIP_UI_BUILD=1` to use the existing
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=ui/mermaid-entry.ts");
    println!("cargo:rerun-if-changed=ui/freeform-entry.ts");
    ...
    let dist_assets = [
        ...
        ui_dir.join("dist").join("mermaid.js"),
        ui_dir.join("dist").join("freeform.js"),
    ];
...
fn dist_is_fresh(dist_asset: &Path, ui_dir: &Path) -> bool {
    ...
    let watched_files = [
        ...
        ui_dir.join("mermaid-entry.ts"),
        ui_dir.join("freeform-entry.ts"),
