//! Standalone `llm-budget` worker. Registers `budget::*` (caps, alerts,
//! forecast, period rollover) on the iii engine and runs until Ctrl-C.
...
fn parse_args(args: Vec<String>) -> Result<String> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!("iii-llm-budget [--engine-url <ws>]");
                std::process::exit(0);
            }
            "--version" | "-V" => {
                println!("iii-llm-budget {}", env!("CARGO_PKG_VERSION"));
                std::process::exit(0);
            }
...
async fn main() -> Result<()> {
    ...
    log::info!("registered: llm-budget (14 budget::* fns)");

    log::info!("llm-budget ready — waiting for requests (Ctrl-C to exit)");
