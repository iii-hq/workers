...
// daemon; S404 and S500-S503 are REST-specific extensions.
export const S = {
...
export function mapHttpStatus(status: number, body: string): SandboxError {
  if (status === 401 || status === 403) return new SandboxError(S.AUTH_INVALID, 'auth invalid or expired')
  if (status === 402) return new SandboxError(S.QUOTA_EXHAUSTED, 'quota exhausted')
  if (status === 429) return new SandboxError(S.RATE_LIMITED, 'rate limited by provider')
  if (status >= 500) return new SandboxError(S.PROVIDER_UNAVAILABLE, `status ${status}: ${body}`)
