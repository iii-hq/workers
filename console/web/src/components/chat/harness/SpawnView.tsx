...
/**
 * `harness::spawn` — the direct sub-agent call. The request is the child's
 * task plus its policy (model, mode, turn budget, output contract, function
 * globs). The call is fire-and-forget: it seeds the child session and
 * answers at once with `{ child_session_id, child_turn_id }`. Historical
 * transcripts (spawn as a pending trigger) instead carry the child's final
 * result — a markdown string for a text contract, a structured value for a
 * json contract — and keep rendering through the same result pane.
 */
  ...
  if (running) {
    return (
      <div className="border-t border-rule-2 bg-bg">
        <MetaRow>
          <StatusPill label="spawning…" variant="default" />
          <SpawnChips req={req} />
        </MetaRow>
...
/**
 * One badge per lifecycle state, and no success claim anywhere: the harness
 * only knows whether the child is still working or has ended its turn.
 * Whether the task itself came out right is for the reader to judge from
 * the child's chat, so nothing here is green or says "completed".
 */
const SUBAGENT_STATUS_COPY: Record<SubagentActivityKind, SubagentStatusCopy> = {
  queued: { label: 'Queued', icon: Activity, tone: 'warning', motion: 'none' },
  waiting: {
    label: 'Waiting',
    icon: Activity,
    tone: 'warning',
...
/** Terminal states read as a moment in the past; live ones as a duration. */
function subagentStatusDetail(
  status: SubagentActivityKind,
  label: string,
  activityAge: string | null,
): string | null {
  if (activityAge == null) return null
  const terminal = status === 'ended' || status === 'stopped'
  if (activityAge === 'just now') {
    return terminal ? `${label} just now` : `${label} now`
  }
  return terminal
...
function SubagentStatus({
  ...
  const copy = SUBAGENT_STATUS_COPY[status]
  return (
    <ActivityStatus
      label={copy.label}
      detail={subagentStatusDetail(status, copy.label, activityAge)}
      icon={copy.icon}
      tone={copy.tone}
      motion={copy.motion}
    />
