/**
 * System-prompt assembly for console-driven harness turns.
 *
 * The Rust harness ships no built-in identity prompt — `harness::send`
 * forwards `options.system_prompt` verbatim and otherwise sends the raw
 * history with the model's own defaults. The console therefore supplies the
 * agent identity itself, the way any consumer building on the harness must.
 *
 * `MODE_PARAGRAPHS` and `BASE_IDENTITY_PROMPT` are ported from the legacy
 * turn-orchestrator (`harness/src/turn-orchestrator/system-prompt.ts` and
 * `prompt/default.ts`). The identity prompt is provider-agnostic; the
 * `provider` parameter is reserved so a future revision can select a
 * provider-specific family without changing call sites.
 */
...
/**
 * Provider-agnostic identity prompt. Teaches the model the single
 * `agent_trigger` tool, the discover → contract → call loop, payload/error
 * rules, and the search-before-build ladder. Ported verbatim from the legacy
 * orchestrator's `PROMPT_DEFAULT`.
 */
const BASE_IDENTITY_PROMPT = `You are an iii agent worker.
...
/**
 * Build the `options.system_prompt` for a console turn: the mode paragraph
 * (plan / ask / agent) prepended to the shared identity prompt. `provider` is
 * accepted for forward-compatibility with provider-specific prompt families
 * but is not yet consulted.
 */
export function buildModeSystemPrompt(mode: Mode, _provider?: string): string {
  return `${MODE_PARAGRAPHS[mode]}\n\n${BASE_IDENTITY_PROMPT}`
}
