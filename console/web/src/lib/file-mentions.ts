 ...
 * path.
 *
 * The composer inserts `#file(…)` tokens (FileMentionNode pills). A mention
 * is a REFERENCE, not an attachment: the token stays in the message text and
 * the agent reads the file (or lists the folder) on demand through its own
 * function calls, so a whole file never rides along with the message.
 *
 * The one exception is a line window (`#file(src/a.ts:12-40)`, what
 * "Reference in chat" from a code selection produces): the person pointed at
 * specific lines, so at send time the console reads exactly those lines
 * through one `coder::read-file` batch call scoped to the conversation's
 * working directory and appends one `<attached-file …>` text block per
 * window to the outgoing user message. Failures never block the send: the
 * failed window becomes a self-closing placeholder block the model can see,
 * plus a `failures` entry the caller surfaces as a chat notice.
 */
...
export interface ExpandedMentions {
  /** One `<attached-file …>` text block per line-window mention, request order. */
  blocks: string[]
...
export interface AttachedFileHeader {
  path: string
  /** `12-40` — the attached line window. Every mention block carries one. */
  lines?: string
  size?: number
  totalLines?: number
  /** Set by the document and PDF expansions when the text was cut short. */
  truncated?: boolean
  error?: string
}

/** Unique `#file(...)` references in first-appearance order. */
...
/** A mention that names a line window — the only kind read at send time. */
type WindowedRef = FileMentionRef & { range: LineRange }

function hasRange(ref: FileMentionRef): ref is WindowedRef {
  return ref.range !== undefined
}

/** A batch entry: the line window `coder::read-file` reads. */
interface ReadTargetWire {
  path: string
  line_from: number
  line_to: number
}

function readTarget(ref: WindowedRef): ReadTargetWire {
  return { path: ref.path, line_from: ref.range.from, line_to: ref.range.to }
}

/**
 * Read every mentioned line window in one jail-validated batch call and
 * format the attachment blocks. Batch results come back in request order
 * (the wire contract), so blocks are labeled with the caller's references.
 *
 * Whole-file and folder mentions attach nothing: the `#file(path)` token
 * stays in the message text and the agent reads the file or lists the
 * folder on demand. A mention without a window therefore never costs a
 * read here — a mistyped path surfaces when the agent tries it, not as an
 * up-front probe.
 */
export async function expandFileMentions(
  ...
  const windowRefs = refs.filter(hasRange)
  if (windowRefs.length === 0) {
    return { blocks: [], attachments: [], failures: [] }
  }
  const labels = windowRefs.map(formatFileMentionInner)
  ...
  try {
    const res = (await call(READ_FILE_FUNCTION_ID, {
      paths: windowRefs.map(readTarget),
  ...
  for (const [i, ref] of windowRefs.entries()) {
    ...
    attachments.push({ path: label, size: entry.content.length })
  }
  return { blocks, attachments, failures }
}

function contentBlock(ref: WindowedRef, entry: ReadEntryResultWire): string {
  const attrs = [
    `path="${escapeAttr(ref.path)}"`,
    `lines="${formatLineRange(ref.range)}"`,
  ]
