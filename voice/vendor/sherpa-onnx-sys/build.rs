// Vendored from the sherpa-onnx-sys 1.13.7 crate (Apache-2.0, k2-fsa/sherpa-onnx).
// Local change: the static link list omits espeak-ng, piper_phonemize and ucd
// (GPL-3.0) and the prebuilt archives are the `-no-tts` variants that ship
// without them. Text-to-speech in this worker never links these libraries.
use std::env;
use std::error::Error;
use std::ffi::OsStr;
use std::fs;
...
fn archive_name(
    ...
    let name = match (link_mode, target_os, target_arch) {
        ...
        (LinkMode::Static, "linux", "aarch64") => {
            format!("sherpa-onnx-v{version}-linux-aarch64-static-no-tts-lib.tar.bz2")
        }
        ...
        (LinkMode::Static, "windows", "aarch64") => {
            format!("sherpa-onnx-v{version}-win-arm64-static-MT-Release-no-tts-lib.tar.bz2")
        }
