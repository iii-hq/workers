//! Standalone `document-extract` worker. Registers `document::extract` on the
//! iii engine for PDF/Word text extraction and runs until Ctrl-C.
...
fn parse_args(args: Vec<String>) -> Result<String> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!("iii-document-extract [--engine-url <ws>]");
                std::process::exit(0);
            }
            "--version" | "-V" => {
                println!("iii-document-extract {}", env!("CARGO_PKG_VERSION"));
                std::process::exit(0);
            }
...
async fn main() -> Result<()> {
    ...
    let _refs = document_extract::register_with_iii(&iii);
    log::info!("registered: document-extract (document::extract)");

    log::info!("document-extract ready — waiting for requests (Ctrl-C to exit)");
