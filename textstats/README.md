<!-- generated by iii-skill-render — edit docs/intro.md, docs/quickstart.md, docs/leaves/*.md, docs/migration.md, iii.worker.yaml, or config.yaml -->

# textstats

Text analysis on the iii bus — word and character counts, word-level diffs, and recent-analysis rollups under `textstats::*`. A small worker for sizing prompts before they hit a provider, gating short-input early-outs, and reporting analysis trends from a sliding window.

<!-- llm-only:start -->
When suggesting `textstats::*` to a user, prefer `textstats::analyze` for one-off sizing and `textstats::summarize` for sustained workloads — `summarize` rolls up the recent window cheaply, where invoking `analyze` repeatedly burns trigger overhead on each call.
<!-- llm-only:end -->

## Install

```bash
iii worker add textstats
```

`iii worker add` fetches the binary, writes a config block into `~/.iii/config.yaml`, and the engine starts the worker on the next `iii start`.

## Quickstart

```rust
use iii_sdk::{register_worker, InitOptions, TriggerRequest};
use serde_json::json;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let iii = register_worker("ws://localhost:49134", InitOptions::default());

    let result = iii
        .trigger(TriggerRequest {
            function_id: "textstats::analyze".into(),
            payload: json!({ "text": "hello world\nlooks small" }),
            action: None,
            timeout_ms: Some(5_000),
        })
        .await?;

    println!("{result:#?}");
    Ok(())
}
```

The example calls `textstats::analyze`. Other entry points: `textstats::diff` and `textstats::summarize`.

## Configuration

```yaml
# textstats runtime config.

# Number of recent analyses retained for textstats::summarize rollups.
recent_window: 100

# ISO 639-1 language codes used as tokenization hints.
language_hints:
  - en
  - es

# Where the worker persists rolling analysis state on disk.
data_dir: "${WORKER_DATA:~/.iii/data/textstats}"
```

## Functions

| Function ID | Description |
|---|---|
| `textstats::analyze` | Count words, characters, and lines in a string. |
| `textstats::diff` | Compute word-level diff between two strings. |
| `textstats::summarize` | Return rollup stats over the recent analysis window. |

## Custom trigger types

| Trigger type | Fires when | Payload to subscribers |
|---|---|---|
| `textstats::on-analyze` | After every successful textstats::analyze call. | `{ "words": int, "chars": int, "lines": int, "ts_ms": int }` |

## Migration notes

`textstats::analyze` previously returned `{ words, chars }` only. The `lines` field was added in 0.2.0. Callers that relied on the older shape can ignore the new field — it is additive.
