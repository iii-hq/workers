//! Standalone `models-catalog` worker. Registers `models::*` (list, get,
//! supports, register) on the iii engine and runs until Ctrl-C. Seeds an
//! embedded baseline catalog into iii state on first boot when state is empty.
...
fn parse_args(args: Vec<String>) -> Result<String> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!("iii-models-catalog [--engine-url <ws>]");
                std::process::exit(0);
            }
            "--version" | "-V" => {
                println!("iii-models-catalog {}", env!("CARGO_PKG_VERSION"));
                std::process::exit(0);
            }
...
async fn main() -> Result<()> {
    ...
    log::info!("registered: models-catalog (4 models::* fns)");

    log::info!("models-catalog ready — waiting for requests (Ctrl-C to exit)");
