...
function record(session_id: string): SessionRecord {
  return {
    session_id,
    pi_session_id: `cs-${session_id}`,
    session_file: `/sessions/${session_id}.jsonl`,
    cwd: '/repo',
    model: '',
    status: 'done',
    turns: 1,
...
describe('session state', () => {
  it('round-trips a record through scope pi_sessions', async () => {
    const fake = fakeIii();
    await saveSession(fake.iii, record('s1'));
    const set = fake.calls.find((c) => c.function_id === 'state::set');
    expect(set?.payload).toMatchObject({ scope: 'pi_sessions', key: 's1' });
