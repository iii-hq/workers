//! Shared test support.
//!
//! - [`engine`] boots a real iii engine and registers this worker against it.
//! - The golden-file helpers below back `tests/schemas.rs`.
//!
//! Hand-rolled golden harness (deliberately no snapshot dependency). Goldens
//! live under `tests/golden/` and are committed; any wire-surface change must
//! show up as an explicit, reviewed diff.
//!
//! Workflow:
//! - `cargo test` compares actual output against the committed goldens.
//! - `UPDATE_GOLDENS=1 cargo test` regenerates the files; review the git diff,
//!   then commit the new goldens alongside the change that caused them.
...
/// Compare `actual` against the golden file at `tests/golden/<rel>`. Returns
/// `Err(readable diff hint)` on mismatch or missing golden; with
/// `UPDATE_GOLDENS=1` the file is (re)written and the check passes.
    ...
    let expected = fs::read_to_string(&path).map_err(|e| {
        format!(
            "golden file {} unreadable ({e}).\n\
             Run `UPDATE_GOLDENS=1 cargo test` to (re)generate, then review and \
             commit the diff.",
            path.display()
        )
    })?;
...
/// Readable first-divergence diff hint: line number, expected versus actual
...
fn diff_hint(rel: &str, expected: &str, actual: &str) -> String {
    ...
    out.push_str(
        "If this change is intentional, run `UPDATE_GOLDENS=1 cargo test`, review \
         the git diff, and commit the updated goldens.\n",
    );
    out
}

/// Assert a schemars-derived request or response schema is a real schema and
/// not the permissive `AnyValue` schema a `Value` handler emits (the "unknown"
/// schema this convention exists to prevent). A real schema carries at least
/// one schema-defining keyword.
pub fn assert_typed_schema(label: &str, schema: &schemars::schema::RootSchema) {
    ...
    assert!(
        has_defining,
        "{label}: schema is the permissive AnyValue/empty schema (no \
         type/properties/$ref/…). The handler is registered with `Value` — give it \
         a typed struct deriving JsonSchema. Got: {value}"
    );
}
