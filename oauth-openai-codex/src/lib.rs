//! OAuth 2.0 Authorization Code + PKCE flow for ChatGPT Plus/Pro subscription
//! auth on the OpenAI Codex API.
//!
...
/// Defaults to 53701; override via `HARNESS_OAUTH_CALLBACK_PORT`.
...
/// Register `oauth::openai-codex::*` iii functions on the bus.
///
/// Functions registered:
/// - `oauth::openai-codex::login` — runs the PKCE flow; URL + progress messages
///   logged via `log::info!`. UI integration can wrap this with an
///   ahead-of-time URL fetch in a follow-up. Returns the resulting
///   [`Credential`] as JSON.
/// - `oauth::openai-codex::refresh` — payload is the existing credential JSON;
///   returns the rotated credential.
/// - `oauth::openai-codex::status` — payload is empty; returns `{ ready: bool }`
...
pub async fn register_with_iii_with_bus(
    ...
    let login_msg = RegisterFunctionMessage::with_id("oauth::openai-codex::login".to_string())
    ...
    let login_fn = iii.register_function((login_msg, |_payload: Value| async move {
        let callbacks = OAuthLoginCallbacks {
            on_open_url: Box::new(|url| {
                log::info!("oauth::openai-codex::login open URL: {url}");
            }),
            on_progress: Box::new(|msg| {
                log::info!("oauth::openai-codex::login progress: {msg}");
            }),
        };
    ...
    let refresh_msg = RegisterFunctionMessage::with_id("oauth::openai-codex::refresh".to_string())
    ...
    let status_msg = RegisterFunctionMessage::with_id("oauth::openai-codex::status".to_string())
...
mod tests {
    ...
    fn authorize_url_includes_required_params() {
        let url = build_authorize_url("CHAL", "STATE", "http://127.0.0.1:53701/callback");
    ...
    fn callback_port_respects_env_var() {
        env::set_var("HARNESS_OAUTH_CALLBACK_PORT", "65001");
