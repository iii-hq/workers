...
export async function analyzeTestCoverage(changedFiles: string[], cwd?: string): Promise<CoverageReport> {
  const root = cwd ?? process.cwd()
  const sourceFiles = changedFiles.filter((f) => SOURCE_EXTENSIONS.has(path.extname(f)) && !TEST_PATTERN.test(f))
