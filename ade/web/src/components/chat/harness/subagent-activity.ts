...
/**
 * What the card can say about a child. There is deliberately no "completed":
 * the harness knows whether the child is still working or has ended its
 * turn, never whether the task came out right.
 */
...
export function displayedSubagentActivity(
  ...
  // `done` means the turn ended — not that the task is complete.
  if (visual === 'completed') return 'ended'
