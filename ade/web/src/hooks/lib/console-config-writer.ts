 ...
 *
 * Every hook that read-modify-writes the entry (trace views, follow turns,
 * span filters) must go through the same queue: two writers
 ...
 * split blinks out for a frame and then is gone from the server. The
 * workspace layout has its own store and writer
 * (`workspace-layout-writer.ts`) and never touches this entry.
 */
