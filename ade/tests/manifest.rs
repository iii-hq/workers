...
//! depends on. Spawns the binary, parses stdout as JSON, and asserts
//! every required field is present and shaped correctly.
...
fn manifest_subcommand_emits_valid_json() {
    let bin = env!("CARGO_BIN_EXE_ade");
    let output = Command::new(bin)
        .arg("--manifest")
        .output()
        .expect("spawn console --manifest");
    ...
    assert!(
        cfg.contains_key("http_port"),
        "default_config.http_port missing"
    );
    assert!(
