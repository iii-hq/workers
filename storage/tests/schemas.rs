//! In-process schema regression: every `storage::*` RPC must register through
//! the typed `RegisterFunction::new_async` so the engine receives
//! auto-generated JSON Schema for tools and docs.
//!
//! This file also owns the `mod e2e;` declaration so Cargo compiles the
