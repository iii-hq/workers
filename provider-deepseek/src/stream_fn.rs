...
async fn run_stream_call(
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            tracing::debug!(provider = "deepseek", error = %e, "provider resolution failed");
            ...
            let _ = send_event(
                ...
                &synthetic_error_event(
                    &llm_router::provider_scaffold::errors::public_error("deepseek", kind),
                    &model,
                    kind,
                ),
            );
    ...
    // catalog is authoritative → id-pattern fallback as a last resort.
    let model_meta = match input.model_meta {
        Some(m) => Some(m),
        None => router_client::models_get(iii, &model).await,
    };
    // Report-and-continue: DeepSeek has no strict json_schema mode, so a
    ...
    {
        warnings.push(
            "response_format schema unsupported: DeepSeek runs json_object mode without schema validation"
                .to_string(),
        );
    }
