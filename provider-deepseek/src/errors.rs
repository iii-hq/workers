...
/// Map a DeepSeek HTTP status + error body to the shared taxonomy
...
pub fn classify(status: Option<u16>, message: &str) -> ErrorKind {
    ...
    match status {
        ...
        // 402 "You have run out of balance" is a billing wall: retry/backoff
        // cannot fix it, only a top-up can.
        Some(402) => ErrorKind::Permanent,
        Some(429) => ErrorKind::RateLimited,
        Some(413) => ErrorKind::ContextOverflow,
        // 503 "server is overloaded" and 500 both say retry after a wait.
...
/// DeepSeek sends OpenAI-style envelopes `{ "error": { "message", "type",
...
mod tests {
    ...
    #[test]
    fn out_of_balance_is_permanent_not_retryable() {
        // 402 is DeepSeek's billing wall; retrying just burns the turn budget.
        assert_eq!(classify(Some(402), ""), ErrorKind::Permanent);
        assert!(!classify(Some(402), "").is_retryable());
        let body = r#"{"error":{"message":"Insufficient Balance","type":"invalid_request_error","code":"insufficient_balance"}}"#;
