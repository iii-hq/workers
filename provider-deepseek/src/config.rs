    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ConfigError::NotConfigured => f.write_str(
                "provider deepseek not configured (no api_key in the llm-router entry \
                 and DEEPSEEK_API_KEY unset)",
            ),
            ConfigError::InvalidApiUrl(u) => write!(
                f,
                "provider deepseek has an invalid endpoint url: {u:?} \
...
/// must agree on it. DeepSeek takes `Authorization: Bearer` for both shapes.
...
) -> Result<DeepseekConfig, ConfigError> {
