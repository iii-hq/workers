 ...
 * harness-internal `pre_generate` hook (see src/functions/inject_guidance.rs)
 * that appends usage guidance to an agent's system prompt, not a call anyone
 * makes on purpose. It keeps the console's default card.
 */
 ...
 * program output, in one of the error messages that quote it by design
 * (error.rs). It is a property of the worker's payloads, not of any one op,
 * so a renderer cannot opt out by omission.
 */
export function createCodeRunnerRenderers(
  host: Host,
): FunctionTriggerRenderer[] {
  return [
    createEvalRenderer(host),
