...
describe('SandboxErrorWire schema', () => {
  ...
  it('isSandboxErrorWire narrows correctly', () => {
    expect(isSandboxErrorWire({ type: 'config', code: 'S100', message: 'no' })).toBe(true)
...
describe('parseSandboxErrorDisplay', () => {
  ...
  it('parses translate function_error envelope with denial details', () => {
    const out = parseSandboxErrorDisplay({
      error: {
        kind: 'function_error',
        message: 'trigger_failed: IIIInvocationError: invocation_failed: handler error',
        details: {
          schema_version: 1,
          status: 'denied',
...
describe('parseDispatchDenial', () => {
  it('extracts the function id and namespace from a bare message', () => {
    const d = parseDispatchDenial("function workflow::start is not permitted by this agent's dispatch policy")
...
describe('lifecycle schemas', () => {
  ...
  it('createRequest + response parse the canonical example', () => {
    ...
    expect(createResponseSchema.safeParse({ sandbox_id: SB, image: 'node' }).success).toBe(true)
  })

  it('stopRequest tolerates missing wait', () => {
    expect(stopRequestSchema.safeParse({ sandbox_id: SB }).success).toBe(true)
    expect(stopResponseSchema.safeParse({ sandbox_id: SB, stopped: true }).success).toBe(true)
  })
...
describe('fs schemas', () => {
  it('fs::ls request + response', () => {
    expect(fsLsRequestSchema.safeParse({ sandbox_id: SB, path: '/' }).success).toBe(true)
    expect(
  ...
  it('fs::stat request + response', () => {
    expect(fsStatRequestSchema.safeParse({ sandbox_id: SB, path: '/a' }).success).toBe(true)
    expect(
  ...
  it('fs::mkdir defaults', () => {
    expect(fsMkdirRequestSchema.safeParse({ sandbox_id: SB, path: '/d' }).success).toBe(true)
    expect(fsMkdirResponseSchema.safeParse({ created: true }).success).toBe(true)
  })
  ...
  it('fs::write accepts UTF-8 content', () => {
    ...
    expect(ok.success).toBe(true)
    expect(fsWriteResponseSchema.safeParse({ bytes_written: 15, path: '/a' }).success).toBe(true)
  })
  ...
  it('fs::rm tolerates default recursive', () => {
    expect(fsRmRequestSchema.safeParse({ sandbox_id: SB, path: '/a' }).success).toBe(true)
    expect(fsRmResponseSchema.safeParse({ removed: true }).success).toBe(true)
  })

  it('fs::mv', () => {
    expect(fsMvRequestSchema.safeParse({ sandbox_id: SB, src: '/a', dst: '/b' }).success).toBe(true)
