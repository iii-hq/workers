...
describe('renderWithHighlight', () => {
  it('returns the line untouched for an empty query', () => {
    expect(renderWithHighlight('keep me', '', { isRegex: false, ignoreCase: false })).toBe('keep me')
  })
  ...
  it('skips zero-width regex matches without dropping real ones', () => {
    ...
    const { text, highlights } = flatten(renderWithHighlight('axxb', 'x*', { isRegex: true, ignoreCase: false }))
    expect(text).toBe('axxb')
    expect(highlights).toEqual(['xx'])
  })

  it('renders no highlights when every regex match is zero-width', () => {
    const { text, highlights } = flatten(renderWithHighlight('bbb', 'a*', { isRegex: true, ignoreCase: false }))
  ...
  it('caps highlighted matches but keeps the unhighlighted tail', () => {
    ...
    const { text, highlights } = flatten(renderWithHighlight(line, 'a', { isRegex: true, ignoreCase: true }))
