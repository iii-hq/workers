/**
 * Best-effort match highlighting for the `fs::grep` view (grep patterns
 * are always regexes on the wire, so the view passes `isRegex: true`).
 * The daemon uses the Rust `regex` crate; JS regex covers the simple
 * patterns agents use and we fall back to substring matching when the
...
export function renderWithHighlight(line: string, query: string, { isRegex, ignoreCase }: HighlightOptions): ReactNode {
...
function highlightRegex(line: string, re: RegExp): ReactNode {
  ...
  for (const hit of line.matchAll(re)) {
    ...
    parts.push(
      <span key={`m:${n}`} className="cr-fam-hl">
        {text}
      </span>,
    )
...
function highlightSubstring(line: string, query: string, ignoreCase: boolean): ReactNode {
  ...
  while (i < line.length) {
    ...
    parts.push(
      <span key={`s:${n++}`} className="cr-fam-hl">
