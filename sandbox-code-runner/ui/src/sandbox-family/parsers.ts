import { z } from 'zod'
import { unwrapEnvelope } from '../lib/payload'
...
export const envShapeSchema = z.union([z.array(z.string()), z.record(z.string(), z.string())])
...
function denialTitle(denial: DenialEnvelopeWire): string {
  switch (denial.denied_by) {
    ...
    default:
      return denial.status === 'denied' ? 'Denied' : 'Trigger failed'
  }
}
...
function denialToInvocation(
  ...
  const title = denialTitle(denial)
  const message = denial.reason ?? fallbackMessage ?? 'The sandbox trigger could not complete.'
  return {
    title,
    message,
 ...
 */
export function parseDispatchDenial(value: unknown): SandboxDispatchDenial | null {
  for (const candidate of collectErrorCandidates(value)) {
    ...
    const namespace = functionId?.includes('::') ? functionId.split('::')[0] : undefined
 ...
 */
export function parseSandboxErrorDisplay(value: unknown): SandboxErrorDisplay | null {
  ...
  if (value && typeof value === 'object' && !Array.isArray(value)) {
    const parsed = functionErrorEnvelopeSchema.safeParse((value as Record<string, unknown>).error)
