...
| [`opengantry`](opengantry/) | Node | OpenGantry governance — scans local `workers/` for iii bundle practices, then runs `gantry::verify` before promote-class calls; `gantry::middleware` on the governed port. |
