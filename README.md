...
| [`discovery`](discovery/) | Rust | One-shot lexical function search — `discovery::search_functions` returns the API reference of only the relevant functions (BM25 + coverage pruning, ≤3 workers/12 contracts), plus a conditional pre-generate search hint injected at most once per session. |
