...
| [`cursor`](cursor/) | Node | Cursor agents through the open `sdk.v1` Bridge protocol, with durable local and cloud turns, raw events on `cursor::events`, and normalized AgentEvent streaming on `agent::events`. |
...
| [`iii-directory`](iii-directory/) | Rust | Engine introspection, workers-registry proxy, filesystem-backed skill + prompt reader, and one-shot lexical function search — `directory::search_functions` returns the API reference of only the relevant functions (BM25 + coverage pruning, installed + installable-from-registry), plus a conditional pre-generate search hint injected at most once per session. |
