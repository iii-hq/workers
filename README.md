...
| [`claude-code`](claude-code/) | Node | Claude Code as an iii worker — `claude::*` runs headless turns, and `claude::terminal::*` runs the same CLI as a TERMINAL on the console (installed on the terminal host, in a workspace equipped with the iii skills). Both mirror raw messages onto `claude::events` and stream AgentEvent frames onto `agent::events`. |
...
| [`provider-merge-gateway`](provider-merge-gateway/) | Rust | Merge Gateway multi-vendor provider behind `llm-router` — `provider::merge-gateway::stream` over the OpenAI-compatible surface, with live model discovery via `refresh_models`, plus `provider::merge-gateway::embed` and `provider::merge-gateway::count_tokens`. |
