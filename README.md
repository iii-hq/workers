...
| [`claude-cli`](claude-cli/) | Node | Claude Code as a TERMINAL on the console — the worker installs the CLI, equips a workspace with the iii skills, opens it in a `shell::pty` session on its own page, and streams the session's hook events onto `agent::events`. |
...
| [`pi-cli`](pi-cli/) | Node | The pi coding agent as a TERMINAL on the console — same shape as `claude-cli`, with the iii activity extension reporting pi's runs onto `agent::events`. |
