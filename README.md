...
| [`aspire-dashboard`](aspire-dashboard/) | Node | Microsoft Aspire Dashboard as an iii worker — starts an OTLP-compatible dashboard, embeds it in the Console, and can configure `iii-observability` to export traces/logs to it, plus an `aspire-dashboard::changed` trigger type so the page updates without polling. |
