...
| [`slides`](slides/) | Node | Slide decks as an iii worker — `slides::*` functions author, store, draft (through `llm-router`), render and export presentations as HTML, PDF and PPTX, with a **Slides** Console page to edit, present and export them and a `deck-designer` agent profile. |
