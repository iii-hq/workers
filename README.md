...
| [`tailscale`](tailscale/) | Rust | Share the local Console over Tailscale Serve (tailnet only, identity headers) or explicitly enabled Funnel (public) — `tailscale::*` status, share, and exact-route stop, plus a Console page with the link as a QR code. |
