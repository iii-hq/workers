...
| [`canvas`](canvas/) | Rust | Diagrams as editable source — `canvas::*` store mermaid text and excalidraw scenes under stable 8-character ids, serve the per-family mermaid syntax primer, and validate source before it lands; the console renders each canvas live in chat and on a canvas page. |
