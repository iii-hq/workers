...
pub struct OpencodeGoConfig {
    ...
    /// <value>`. Populated via [`OpencodeGoConfig::with_credential`] in
    /// P5; the legacy [`OpencodeGoConfig::from_env`] constructor still
...
/// Register `provider::opencode-go::stream` on the iii bus.
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<OpencodeGoConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| {
            OpencodeGoConfig::with_credential(model, cred)
        },
        stream,
    );
    Ok(())
...
mod tests {
    ...
    /// so splitting these caused a race on `OPENCODE_GO_API_KEY`. Coverage is identical
    /// when sequenced.
    #[test]
    fn config_env_resolution() {
        ...
        let cfg = OpencodeGoConfig::from_env("test-model").expect("ok");
    ...
    fn with_credential_api_key() {
        ...
        let cfg = OpencodeGoConfig::with_credential("the-model", &cred).unwrap();
