//! Smoke tests for opencode-go config + Chat Completions request shape.

use provider_opencode_go::OpencodeGoConfig;

#[test]
fn library_exports_register_entry_point() {
    let _ = &provider_opencode_go::register_with_iii;
}
...
fn config_serde_round_trips() {
    ...
    let back: OpencodeGoConfig = serde_json::from_str(&json).expect("deserializes");
...
fn with_credential_api_key_populates_config() {
    ...
    let cfg = OpencodeGoConfig::with_credential("gpt-4o", &cred).expect("builds");
