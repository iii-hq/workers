...
async fn deadlines_model_names_and_oversized_prompts_fail_typed() {
    ...
    let small = client.with_limits(judge_decider::Limits {
    ...
    // Past the tiny vocabulary's 26 one-token labels.
    let many: serde_json::Map<String, Value> =
        (0..27).map(|i| (format!("o{i:02}"), Value::Null)).collect();
    let wide = request(
...
async fn model_listing_describes_the_loaded_model() {
    ...
    assert_eq!(models[0].name, "decider-4b-v2");
    assert!(
