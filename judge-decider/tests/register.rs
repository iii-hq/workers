//! Bus contract of the decider provider over a mocked engine socket.
#[path = "../../judge-typesafe/tests/support/fake_engine.rs"]
mod fake_engine;
mod support;
use iii_sdk::{register_worker, InitOptions};
use judge_decider::{DeciderClient, DeciderConfig};
use serde_json::{json, Value};
use std::{sync::Arc, time::Duration};
use tokio::{sync::mpsc, time::timeout};

async fn invoke(payload: Value) -> (Value, Value) {
    invoke_function("judge-decider::evaluate", payload).await
}
...
async fn invoke_function(function_id: &'static str, payload: Value) -> (Value, Value) {
    invoke_client(
        function_id,
        payload,
        judge_decider::configuration::new_cell(DeciderConfig::default()),
        support::tiny_client(),
    )
    .await
}
...
async fn invoke_client(
    ...
    payload: Value,
    config: judge_decider::SharedConfig,
...
async fn models_and_cancel_are_registered_with_the_provider_ids() {
    ...
    assert_eq!(response["result"]["models"][0]["name"], "decider-4b-v2");
    ...
    let (_, response) =
        invoke_function("judge-decider::cancel", json!({"request_id":"no-caller"})).await;
...
fn provider_ids_follow_the_hub_convention() {
    ...
    use judge_decider::{CANCEL_ID, EVALUATE_ID, MODELS_ID, PROVIDER};
