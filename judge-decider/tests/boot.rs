    ...
    let server = tokio::spawn(async move {
        ...
        while let Some(Ok(frame)) = socket.next().await {
            ...
            if value["type"] == "registerfunction" && value["id"] == "judge-decider::evaluate" {
                socket.send(Message::Text(json!({"type":"invokefunction","invocation_id":"00000000-0000-0000-0000-000000000009","function_id":"judge-decider::evaluate","data":{"timeout_ms":8000,"evaluations":[{"id":"ticket","state":{},"questions":{"urgent":{"type":"noul","instructions":"Is this urgent?"}}}]}}).to_string().into())).await.unwrap();
            }
