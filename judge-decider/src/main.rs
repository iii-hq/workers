//! Boot the decider provider: register on the bus, then fetch and load the GGUF
//! on first use (or at once while the judge hub selects decider).
use clap::Parser;
use iii_llama_runtime::{ModelSlot, IDLE_RELEASE};
use iii_sdk::IIIClient;
use iii_sdk::{register_worker, runtime::WorkerMetadata, InitOptions};
use judge_decider::{configuration, download, engine, register, DeciderClient, PROVIDER};
...
async fn main() -> anyhow::Result<()> {
    ...
    // provider (see `serve`).
    let slot = ModelSlot::new(move || -> anyhow::Result<DeciderClient> {
...
/// Keep the decider model loaded while the judge hub selects this provider (see
/// `ModelSlot::follow_selection`); a hub that does not expose its
/// configuration id pins it for good.
async fn serve(iii: Arc<IIIClient>, slot: Arc<ModelSlot<DeciderClient>>) -> anyhow::Result<()> {
    match iii_config_client::follow(
        &iii,
        "judge",
        "judge-decider::on-judge-config-change",
        "Internal: keep the decider model loaded while the judge hub selects this provider.",
    )
    .await
    {
        ...
        Err(reason) => {
            tracing::warn!(
                reason,
                "judge hub selection unknown; keeping the decider model loaded"
            );
            slot.pin(true);
            Ok(())
        }
