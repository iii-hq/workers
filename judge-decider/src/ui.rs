//! Embed and register the decider console configuration form.
use iii_console_ui::ConsoleUi;
use iii_sdk::IIIClient;
use std::sync::Arc;

pub const PAGE_PATH: &str = "judge-decider/page.js";
pub const STYLES_PATH: &str = "judge-decider/styles.css";
...
mod tests {
    ...
    fn embedded_styles_are_scoped() {
        assert!(
            STYLES_CSS.contains(r#"[data-iii-ui="judge-decider"]"#)
                || STYLES_CSS.contains("[data-iii-ui=judge-decider]"),
            "styles.css must be scoped to the decider worker"
        );
    }
}
