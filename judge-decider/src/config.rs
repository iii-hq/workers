...
impl DeciderConfig {
    pub fn validate(&self) -> Result<(), String> {
        if crate::download::model(&self.model).is_none() {
            ...
            return Err(format!("decider model must be one of {names:?}"));
        }
        ...
        {
            return Err("Invalid decider execution limits".into());
        }
        Ok(())
    }
    ...
    pub fn from_json(value: &Value) -> Result<Self, String> {
        let config: Self = serde_json::from_value(value.clone())
            .map_err(|_| "Invalid decider configuration".to_string())?;
        config.validate()?;
        Ok(config)
    }
    pub fn to_json(&self) -> Value {
        serde_json::to_value(self).expect("decider config serializes")
    }
    pub fn json_schema() -> Value {
        let mut schema =
            serde_json::to_value(schemars::schema_for!(Self)).expect("decider schema serializes");
