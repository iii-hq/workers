...
import { deviceOf, loadedModel, DeciderConfigForm } from './index'
...
async function mount(value: ConfigFormProps['value'], iii = engine(), extra: Partial<ConfigFormProps> = {}) {
  ...
  await act(async () => root!.render(<DeciderConfigForm id="judge-decider" schema={{}} value={value} onChange={onChange} iii={iii} {...extra} />))
...
describe('DeciderConfigForm', () => {
  it('offers the model and reports where it runs', async () => {
    const { container, iii } = await mount({})
    expect(iii.trigger).toHaveBeenCalledWith('judge-decider::models::list', { timeout_ms: 15_000 }, { timeoutMs: 20_000 })
    const select = container.querySelector<HTMLSelectElement>('select[name="model"]')!
    expect([...select.options].map((o) => o.value)).toEqual(['', 'decider-4b-v2'])
    expect(container.querySelector('[data-chip="success"]')?.textContent).toBe('Running decider-4b-v2 on AMD Radeon RX 6900 XT (Vulkan)')
  ...
  it('edits placement and limits, allows 0 GPU layers, preserves unknown fields', async () => {
    ...
    await act(async () => changes.get('model')?.('decider-4b-v2'))
    expect(onChange).toHaveBeenLastCalledWith({ ...value, model: 'decider-4b-v2' })
  })
  ...
  it('maps field errors, shows the rest in a panel, and keeps opaque roots', async () => {
    ...
    expect(container.querySelector('#decider-cfg-context_tokens-error')?.textContent).toBe('too small')
    expect(container.querySelector('[role="alert"][data-variant="alert"]')?.textContent).toContain('root failure')
    const { container: opaque, onChange } = await mount('${DECIDER}')
  ...
  it('focuses the deep-linked control', async () => {
    ...
    expect(document.activeElement).toBe(container.querySelector('#decider-cfg-gpu_layers'))
  })
})
