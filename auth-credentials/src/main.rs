//! Standalone `auth-credentials` worker. Registers `auth::*` (provider credential
//! vault) on the iii engine and runs until Ctrl-C. Uses the in-memory backend by
//! default — replace with an iii-state-backed `CredentialStore` for production.
//!
//! Usage:
//!   iii-auth-credentials [--engine-url <ws>]
...
fn parse_args(args: Vec<String>) -> Result<String> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!("iii-auth-credentials [--engine-url <ws>]");
                std::process::exit(0);
            }
            "--version" | "-V" => {
                println!("iii-auth-credentials {}", env!("CARGO_PKG_VERSION"));
                std::process::exit(0);
            }
...
async fn main() -> Result<()> {
    ...
    log::info!("registered: auth-credentials (5 auth::* fns)");

    log::info!("auth-credentials ready — waiting for requests (Ctrl-C to exit)");
