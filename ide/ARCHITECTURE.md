...
mkdir -p ~/.iii/workers
ln -sfn $(pwd)/target/release/iii-shell ~/.iii/workers/ide
...
`iii trigger compose::add worker=ide` does not currently pull `iii-sandbox` along — run `iii trigger compose::add worker=iii-sandbox` separately before using `shell::exec { target: sandbox }` or any `shell::fs::*` sandbox-target path. Plain host-targeted `shell::exec` works without it.
...
(page `ide` — files/git/search sidebar beside the console's shared Monaco
...
pane key under `data/ide/ui-state/panes/` (resolved against
...
| Engine started but doesn't see the worker | Binary isn't symlinked at `~/.iii/workers/ide`. | `ln -sfn $(pwd)/target/release/iii-shell ~/.iii/workers/ide` |
