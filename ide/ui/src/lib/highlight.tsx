/**
 * Best-effort match highlighting for grep/search-style views. The worker
 * uses the Rust `regex` crate; JS regex covers the simple patterns agents
 * use and we fall back to substring matching when the pattern doesn't
 * compile — or when `isRegex` is false, since the query is then a literal
 * and regex metacharacters must not fire. Ported from the console's
 * sandbox/highlight.tsx.
 */
...
function highlightRegex(line: string, re: RegExp): ReactNode {
  ...
  for (const hit of line.matchAll(re)) {
    ...
    parts.push(
      <span key={`m:${n}`} className="shui-hl">
        {text}
      </span>,
    )
...
function highlightSubstring(
  ...
  while (i < line.length) {
    ...
    parts.push(
      <span key={`s:${n++}`} className="shui-hl">
