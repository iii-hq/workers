...
| `WORKER_BIN_LINK` | `$HOME/.iii/workers/ide` | Symlink the engine reads (registered worker name from `worker-compose.yaml`). Set to `$HOME/.iii/workers/iii-shell` if your engine resolves by binary name instead. |
...
security findings from the `feat/ide-e2e-harness` review. Each case
