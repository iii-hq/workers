...
# Path overrides (defaults assume the harness lives at ide/tests/e2e/ inside
...
RUN_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/ide-e2e.XXXXXX")
...
WORKER_BIN_LINK=${worker_bin_link_override:-"$HOME/.iii/workers/ide"}
if [[ "$SUITE" == jailed ]]; then
...
if [[ "$NO_BUILD" -eq 0 ]]; then
  ...
  (cd "$WORKER_SRC" && cargo build --locked --release --bin ide)
fi
...
(cd "$RUN_ROOT" && III_NAMESPACE=ide-e2e "$WORKER_BIN_TARGET" \
