...
use ide::config::ShellConfig;
use ide::functions;
use ide::functions::types::{ExecBgRequest, ExecRequest, KillRequest, StatusRequest};
use ide::jobs::{self, now_ms, JobHandle, JobRecord, JobStatus};
...
fn fs_host_backend() -> Arc<dyn ide::fs::FsBackend> {
    use std::fmt;

    #[derive(Debug)]
    struct StubChan;
    #[async_trait::async_trait]
    impl ide::fs::host::ChannelMaker for StubChan {
    ...
    Arc::new(ide::fs::host::HostFsBackend::new(
        Arc::new(ide::fs::host::HostFsConfig::default()),
        Arc::new(StubChan),
    ))
}
