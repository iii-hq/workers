...
/// Identify this worker to the engine as `ide` (name, runtime, version, pid)
/// so it appears as `ide` in `engine::workers::list` and the `worker`
...
async fn main() -> Result<()> {
    ...
    // terminal layout) lives under data/ide/ui-state — developer-local
