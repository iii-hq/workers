//! Smoke tests for opencode-zen config + Chat Completions request shape.

use provider_opencode_zen::OpencodeZenConfig;
...
fn config_serde_round_trips() {
    let cfg = OpencodeZenConfig {
        api_key: "sk-test".into(),
        model: "gpt-4o".into(),
        max_tokens: 4096,
    };
    let json = serde_json::to_string(&cfg).expect("serializes");
    let back: OpencodeZenConfig = serde_json::from_str(&json).expect("deserializes");
...
fn with_credential_api_key_populates_config() {
    ...
    let cfg = OpencodeZenConfig::with_credential("gpt-4o", &cred).expect("builds");
