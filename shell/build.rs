//! Build script for the `shell` worker.
//!
...
fn subtree_older_than(root: &Path, ceiling: SystemTime) -> bool {
    ...
    for entry in read.flatten() {
        let path = entry.path();
        // node_modules/dist churn must not force worker rebuilds; only
        // real sources are hashed by mtime here.
        if entry.file_name() == "node_modules" || entry.file_name() == "__tests__" {
            continue;
        }
