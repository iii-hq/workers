//! Hand-rolled golden-file harness for the folded `coder::*` code surface
//! (deliberately no `insta`/snapshot dependency). Goldens live under
//! `shell/tests/golden/` and are committed; any wire-surface change must
...
/// Panicking wrapper around [`check_golden`].
pub fn assert_golden(rel: &str, actual: &str) {
    if let Err(msg) = check_golden(rel, actual) {
        panic!("{msg}");
    }
}
...
fn diff_hint(rel: &str, expected: &str, actual: &str) -> String {
    ...
    // All compared lines matched — one side is a strict prefix of the other.
    if exp_lines.len() != act_lines.len() && first_diff == exp_lines.len().min(act_lines.len()) {
        out.push_str(&format!(
            "(expected has {} lines, actual has {} — divergence is in \
             length, not content)\n",
            exp_lines.len(),
            act_lines.len()
        ));
