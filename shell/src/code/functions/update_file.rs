//! `coder::update-file` — batched line-oriented and content edits across one
//! or more files. Line ops are applied **bottom-up** so earlier ops see the
//! original line numbers; content ops (`str_replace`, regex `replace`) run
//! afterward on the serialized body, in array order. Each file commits
//! atomically via sibling temp + `rename`.
//!
...
//! Content ops:
//!
//! - `{ op: "str_replace", old_str: "...", new_str: "...", replace_all?: bool }`
//!   — replace one exact literal occurrence of `old_str` (no regex). The
//!   preferred op for targeted edits: without `replace_all` the op fails
//!   (C210, nothing written) unless `old_str` occurs exactly once; with
//!   `replace_all: true` every occurrence is replaced (at least one
//!   required).
...
pub enum UpdateOp {
    ...
    /// Replace one exact literal occurrence of `old_str` with `new_str` —
    /// THE PREFERRED OP for targeted edits. Copy `old_str` verbatim from
    /// the file (read it first), including whitespace and indentation; no
    /// regex, no escaping. Fails with C210 (nothing written) when
    /// `old_str` is not found, or occurs more than once without
    /// `replace_all` — include more surrounding lines in `old_str` to
    /// make it unique.
    #[serde(rename = "str_replace")]
    StrReplace {
        /// Exact existing text to replace, copied verbatim from the file.
        old_str: String,
        /// Replacement text (may be empty to delete `old_str`).
        new_str: String,
        /// Replace EVERY occurrence instead of requiring exactly one.
        /// At least one occurrence is still required.
        #[serde(default)]
        replace_all: bool,
    },
...
fn example_update_file_input() -> serde_json::Value {
    serde_json::json!({
        "files": [{
            "path": "src/lib.rs",
            "ops": [
                { "op": "str_replace", "old_str": "fn hello() -> &'static str {\n    \"hallo\"\n}",
...
fn record_line_op_events(
    ...
    for (op_index, op) in line_ops {
        let (pos, delta, first, last) = match op {
            ...
            UpdateOp::Replace { .. } | UpdateOp::StrReplace { .. } => unreachable!("line ops only"),
        };
        if delta != 0 {
...
/// Post-apply match-count rule for one content op: regex `replace` enforces
/// its optional `expect_matches`; `str_replace` enforces found-and-unique
/// (or found-at-all with `replace_all`).
enum CountRule<'a> {
    ExpectMatches {
        pattern: &'a str,
        expect: Option<u64>,
...
/// Apply every content op in `ops` (`str_replace` / regex `replace`, in
/// array order), each on the body produced by all earlier ops, recording
/// mutation events and up to `ECHO_MAX_SITES` echo anchors per op. Match
/// positions are located on each op's OWN input body; earlier replacements
/// within the same op shift later sites through the event list like any
/// other mutation. A `str_replace` desugars into the same machinery: an
/// escaped-literal regex plus a `$$`-escaped replacement, so expansion is
/// verbatim.
fn apply_regex_ops(
    mut bytes: Vec<u8>,
    ops: &[UpdateOp],
    ...
    for (op_index, op) in ops.iter().enumerate() {
        let (re, replacement, count_rule): (regex::Regex, std::borrow::Cow<'_, str>, CountRule) =
            match op {
                UpdateOp::Replace {
                    pattern,
                    replacement,
                    ...
                    (
                        re,
                        std::borrow::Cow::Borrowed(replacement.as_str()),
                        CountRule::ExpectMatches {
                            pattern,
                            expect: *expect_matches,
                        },
                    )
                }
                UpdateOp::StrReplace {
                    ...
                    if old_str.is_empty() {
                        return Err(CoderError::BadInput(
                            "str_replace.old_str must not be empty".into(),
                        ));
                    }
                    let re = regex::RegexBuilder::new(&regex::escape(old_str))
                        .build()
                        .map_err(|e| {
                            CoderError::BadInput(format!("str_replace.old_str too large: {e}"))
                        })?;
                    // `$$` so `Captures::expand` emits `new_str` verbatim
                    // (a bare `$` would be an — always undefined — capture
                    // reference against the escaped-literal pattern).
                    (
                        re,
                        std::borrow::Cow::Owned(new_str.replace('$', "$$")),
                        CountRule::Unique {
                            old_str,
        ...
        match count_rule {
            ...
            CountRule::Unique {
                old_str,
                replace_all,
            } if total == 0 || (!replace_all && total > 1) => {
                return Err(str_replace_mismatch(old_str, total));
            }
            _ => {}
        }
...
/// Prescriptive C210 for a `str_replace` count violation: `old_str` was not
/// found (0 matches — stale or re-typed text) or is ambiguous (>1 match
/// without `replace_all`). Names the corrective next call in both cases.
fn str_replace_mismatch(old_str: &str, actual: u64) -> CoderError {
    let shown = pattern_snippet(old_str);
    if actual == 0 {
        CoderError::BadInput(format!(
            "str_replace old_str {shown} not found in the file — re-read the \
             file (coder::read-file) and copy the exact current text, \
             including whitespace and indentation"
        ))
    } else {
        CoderError::BadInput(format!(
            "str_replace old_str {shown} matched {} — include more \
             surrounding lines in old_str to make it unique, or set \
...
pub fn apply_ops(
    ...
    if ops
        .iter()
        .any(|op| matches!(op, UpdateOp::Replace { .. } | UpdateOp::StrReplace { .. }))
    {
        return Err(CoderError::BadInput(
            "apply_ops does not support content ops".into(),
        ));
    }
...
fn anchor(op: &UpdateOp) -> u32 {
    match op {
        ...
        UpdateOp::Replace { .. } | UpdateOp::StrReplace { .. } => 0,
    }
}
...
fn cover(op: &UpdateOp) -> (u32, u32) {
    match op {
        ...
        UpdateOp::Replace { .. } | UpdateOp::StrReplace { .. } => (0, 0),
    }
}
...
mod tests {
    ...
    fn str_replace(old: &str, new: &str, replace_all: bool) -> UpdateOp {
        UpdateOp::StrReplace {
            old_str: old.into(),
            new_str: new.into(),
            replace_all,
    ...
    fn str_replace_single_occurrence() {
        let out = apply_regex_replaces(
            ...
            &[&str_replace("bar", "qux", false)],
        )
        .unwrap();
        assert_eq!(out, b"foo qux baz");
    }

    #[test]
    fn str_replace_is_literal_not_regex() {
        let out = apply_regex_replaces(
            b"price is $x.*y".to_vec(),
            &[&str_replace("$x.*y", "$z", false)],
        )
        .unwrap();
        assert_eq!(out, b"price is $z");
    }

    #[test]
    fn str_replace_dollar_in_new_str_lands_verbatim() {
        // The classic template-literal collision: `${name}` must survive
        // expansion untouched (regex `replace` requires `$$` for this).
        let out = apply_regex_replaces(
            b"greet('world')".to_vec(),
            &[&str_replace("'world'", "`Hello, ${name}!`", false)],
        )
        .unwrap();
        assert_eq!(out, b"greet(`Hello, ${name}!`)");
    }

    #[test]
    fn str_replace_multiline_old_str() {
        let out = apply_regex_replaces(
            b"fn a() {\n    1\n}\nfn b() {}".to_vec(),
            &[&str_replace(
                "fn a() {\n    1\n}",
        ...
        .unwrap();
        assert_eq!(out, b"fn a() {\n    2\n}\nfn b() {}");
    }

    #[test]
    fn str_replace_not_found_is_c210() {
        let err = apply_regex_replaces(b"foo bar".to_vec(), &[&str_replace("qux", "x", false)])
            .unwrap_err();
        assert_eq!(err.code(), "C210");
    ...
    #[test]
    fn str_replace_ambiguous_without_replace_all_is_c210() {
        let err = apply_regex_replaces(b"foo foo foo".to_vec(), &[&str_replace("foo", "x", false)])
            .unwrap_err();
        assert_eq!(err.code(), "C210");
    ...
    #[test]
    fn str_replace_replace_all_replaces_every_occurrence() {
        let out = apply_regex_replaces(b"foo foo foo".to_vec(), &[&str_replace("foo", "x", true)])
            .unwrap();
        assert_eq!(out, b"x x x");
    }

    #[test]
    fn str_replace_replace_all_still_requires_a_match() {
        let err = apply_regex_replaces(b"foo bar".to_vec(), &[&str_replace("qux", "x", true)])
            .unwrap_err();
        assert_eq!(err.code(), "C210");
    }

    #[test]
    fn str_replace_empty_old_str_rejected() {
        let err =
            apply_regex_replaces(b"foo".to_vec(), &[&str_replace("", "x", false)]).unwrap_err();
        assert_eq!(err.code(), "C210");
        assert!(err.to_string().contains("must not be empty"), "{err}");
    }

    #[test]
    fn str_replace_new_str_may_be_empty() {
        let out = apply_regex_replaces(b"foo bar baz".to_vec(), &[&str_replace(" bar", "", false)])
            .unwrap();
        assert_eq!(out, b"foo baz");
    }
