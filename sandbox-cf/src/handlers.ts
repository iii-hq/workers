import type { CfBridgeClient } from './client.js'
...
export async function doCreate(ctx: HandlerCtx, input: Record<string, unknown>) {
  ...
  try {
    const created = await ctx.client.create(image, idle)
    return {
...
export async function doList(ctx: HandlerCtx, _input: Record<string, unknown>) {
  let sandboxes: Awaited<ReturnType<CfBridgeClient['list']>> = []
  try {
