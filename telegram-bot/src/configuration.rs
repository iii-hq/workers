...
pub const DEFAULT_CONFIG_ID: &str = "telegram-bot";
...
/// Register the bot schema and seed only when the assigned entry has no stored value.
pub async fn register_config(iii: &IIIClient, seed: Option<&WorkerConfig>) -> Result<(), String> {
    ...
    initialization::ensure_with(payload, |function, payload| {
        trigger_configuration_with_retry(iii, function, payload, config_rpc_timeout_ms(seed))
    })
    .await
}
...
mod tests {
    ...
    /// A failed configuration service must not authorize overwriting bot settings.
    #[test]
