...
//! dependency). Goldens live under `tests/golden/` and are committed; any
//! wire-surface change must show up as an explicit, reviewed diff.
//!
    ...
    let expected = fs::read_to_string(&path).map_err(|e| {
        format!(
            "golden file {} unreadable ({e}).\n\
             Run `UPDATE_GOLDENS=1 cargo test` to (re)generate, then review and commit the diff.",
            path.display()
        )
    })?;
    if expected == actual {
...
fn diff_hint(rel: &str, expected: &str, actual: &str) -> String {
    ...
    out.push_str(
        "If this change is intentional, run `UPDATE_GOLDENS=1 cargo test`, review the git diff, \
         and commit the updated goldens.\n",
    );
    out
}

/// Assert a schemars-derived schema is a *real* schema and not the permissive
/// `AnyValue`/empty schema a `Value` handler emits. A real schema carries at
