//! GOLDEN FAMILY A — wire-schema snapshots for all 9 functions.
//!
//! `coder::functions::catalog()` is the single source of truth for each
...
use coder::functions::{catalog, FunctionSpec};
...
fn info_example_round_trips() {
    let _input: coder::functions::info::InfoInput = example_as("coder::info", 0);
}

#[test]
fn read_file_single_example_round_trips_with_window() {
    let input: coder::functions::read_file::ReadFileInput = example_as("coder::read-file", 0);
...
fn read_file_batch_example_round_trips_with_both_target_forms() {
    use coder::functions::read_file::{ReadFileInput, ReadTarget};
...
fn search_example_round_trips() {
    let input: coder::functions::search::SearchInput = example_as("coder::search", 0);
...
fn update_file_example_round_trips_with_three_op_kinds() {
    use coder::functions::update_file::{UpdateFileInput, UpdateOp};
...
fn create_file_example_round_trips_with_relative_and_absolute_entries() {
    let input: coder::functions::create_file::CreateFileInput = example_as("coder::create-file", 0);
...
fn delete_file_example_round_trips() {
    let input: coder::functions::delete_file::DeleteFileInput = example_as("coder::delete-file", 0);
...
fn list_folder_example_round_trips() {
    let input: coder::functions::list_folder::ListFolderInput = example_as("coder::list-folder", 0);
    assert_eq!(input.page_size, Some(50));
}

#[test]
fn tree_example_round_trips() {
    let input: coder::functions::tree::TreeInput = example_as("coder::tree", 0);
