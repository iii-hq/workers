...
//! WebSocket handshake cost dwarfs cucumber's per-scenario overhead, so
//! every scenario shares the same client. When the engine is unreachable
//! `get_or_init` returns `None` and every `@engine` step soft-skips.
