//! GOLDEN FAMILY C — canonicalization parity vectors.
//!
//! CANONICAL CASE MATRIX for the MIRROR-INVARIANT shared between
//! `coder/src/path/mod.rs` and `shell/src/fs/host.rs`
//! (`canonicalize_with_fallback` + `normalize_lexical` implement the same
//! jail-safety algorithm and MUST evolve in lockstep).
//!
//! True cross-crate execution is impossible — coder and shell are
//! separate Cargo workspaces, and a shared crate is deliberately deferred
//! (rule of three). Instead this VECTOR TABLE pins the behavioral
//! contract against coder's `PathResolver`; shell's host.rs points here
//! from its MIRROR-INVARIANT note. When you change either implementation,
//! port the fix to the other file AND extend this matrix with the case
