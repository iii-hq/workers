//! HuggingFace Inference API Chat Completions streaming via provider-base.
...
pub struct HuggingFaceConfig {
    ...
    /// <value>`. Populated via [`HuggingFaceConfig::with_credential`] in
    /// P5; the legacy [`HuggingFaceConfig::from_env`] constructor still
...
/// Register `provider::huggingface::stream` on the iii bus.
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<HuggingFaceConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| {
            HuggingFaceConfig::with_credential(model, cred)
        },
        stream,
    );
    Ok(())
...
mod tests {
    ...
    /// so splitting these caused a race on `HF_TOKEN`. Coverage is identical
    /// when sequenced.
    #[test]
    fn config_env_resolution() {
        ...
        assert!(HuggingFaceConfig::from_env("test-model").is_err());

        std::env::set_var("HF_TOKEN", "test-key");
        let cfg = HuggingFaceConfig::from_env("test-model").expect("ok");
    ...
    fn with_credential_api_key() {
        ...
        let cfg = HuggingFaceConfig::with_credential("the-model", &cred).unwrap();
