...
async fn allow_path_end_to_end() {
    ...
    *exec.response.lock().unwrap() = Some(Ok(json!({ "stdout": "hello\n" })));
    ...
    let resolve_reply =
        handle_resolve(&bus, &exec, STATE_SCOPE, &policy_rules, payload, 2_000).await;
...
async fn cascade_path_end_to_end() {
    ...
    *exec.response.lock().unwrap() = Some(Ok(json!({ "stdout": "" })));
    ...
    // Two pending rows for the SAME argv shape, plus one different argv
    // shape. `cascade_allow_for_session` pushes an exact-pattern Allow
    // rule from the first row's args; only the second row should then
    // verdict as Allow and auto-resolve.
    ...
    // Matching rows are Done(Executed). The different argv stays Pending
    // because `always` added an exact pattern rule, not a function-wide allow.
    let consume = handle_consume(
        &bus,
        STATE_SCOPE,
        json!({ "session_id": "sess_cascade" }),
        3_000,
    )
    .await;
    let entries = consume["entries"].as_array().unwrap();
    ...
    assert_eq!(leftover.len(), 1, "non-matching pending row must remain");
    let leftover_record = Record::from_value(leftover[0].clone()).unwrap();
    assert_eq!(leftover_record.function_call_id, "tc-6");
    assert_eq!(leftover_record.status, Status::Pending);
}
