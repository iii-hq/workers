...
use judge_semif::{CANCEL_ID, EVALUATE_ID, MODELS_ID};
use serde_json::json;

#[tokio::test]
#[ignore = "requires III_ENGINE_BIN; starts an isolated real engine, no network"]
async fn tiny_gguf_answers_through_a_real_engine() {
    let engine = Engine::start("judge_semif").await;
    let provider = connect(&engine.url, "judge-semif").await;
    let consumer = connect(&engine.url, "ticket-consumer").await;
    let client = support::tiny_client();
    judge_semif::register(
        &provider,
        judge_semif::configuration::new_cell(judge_semif::SemifConfig::default()),
    ...
    assert_eq!(evaluated["model"], "qwen3.5-4b");
    ...
    let listed = invoke(&consumer, MODELS_ID, json!({"timeout_ms": 3000}))
        .await
        .unwrap();
    assert_eq!(listed["models"][0]["name"], "qwen3.5-4b", "{listed}");
