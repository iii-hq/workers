//! Verify SemIf's configuration RPC contract against a mock service.
...
async fn register_and_fetch(
    mut stored: Value,
    seed: Option<SemifConfig>,
    ensure_error: Option<Value>,
) -> (Result<SemifConfig, String>, Vec<Value>) {
...
fn assert_ensure_then_fetch(calls: &[Value], candidate: &SemifConfig) {
    ...
    assert_eq!(calls[0]["data"]["metadata"]["ui_form"], "judge-semif");
    assert_eq!(calls[0]["data"]["schema"], SemifConfig::json_schema());
...
async fn missing_ensure_falls_back_to_legacy_get_then_register_seeding() {
    ...
    .await;
    assert_eq!(config.unwrap().to_json(), SemifConfig::default().to_json());
    let ids: Vec<_> = calls
        .iter()
    ...
    assert_eq!(
        ...
        SemifConfig::default().to_json()
    );
    assert_eq!(calls[2]["data"]["metadata"]["ui_form"], "judge-semif");
}
...
async fn other_ensure_errors_are_redacted_even_when_the_message_mentions_missing_ensure() {
    for code in ["ADAPTER_ERROR", "SCHEMA_INVALID", "NOT_FOUND"] {
        ...
        assert_eq!(
            result.unwrap_err(),
            "SemIf configuration registration failed"
        );
