...
async fn check_boot(shutdown_signal: Option<&str>) {
    ...
    let server = tokio::spawn(async move {
        ...
        while let Some(Ok(frame)) = socket.next().await {
            ...
            {
                socket.send(Message::Text(json!({"type":"invokefunction","invocation_id":"00000000-0000-0000-0000-000000000010","function_id":"judge-semif::configuration-id","data":{"_caller_worker_id":"console-test"}}).to_string().into())).await.unwrap();
            }
        }
    });
    let mut worker = Worker(
        Command::new(env!("CARGO_BIN_EXE_judge-semif"))
            .env("III_URL", url)
            .env("III_CONFIG_NAME", "semif-boot-test")
            .env(
                "III_SEMIF_GGUF",
                concat!(
    ...
    timeout(Duration::from_secs(10), async {
        let mut expected_functions = BTreeSet::from([
            "judge-semif::evaluate".to_owned(),
            "judge-semif::models::list".to_owned(),
            "judge-semif::cancel".to_owned(),
            "judge-semif::on-config-change".to_owned(),
        ]);
        if console_ui_enabled {
            expected_functions.insert("judge-semif::ui-content".to_owned());
            expected_functions.insert("judge-semif::configuration-id".to_owned());
        }
            ...
            && evaluated)
        {
            ...
            match value["type"].as_str().unwrap() {
                "invokefunction" if value["function_id"] == "configuration::ensure" => {
                    ...
                    assert_eq!(value["data"]["id"], "semif-boot-test");
                    assert_eq!(value["data"]["metadata"]["ui_form"], "judge-semif");
                    assert_eq!(value["data"]["initial_value"]["model"], "qwen3.5-4b");
                    config = true;
                }
                ...
                "registertrigger" if value["trigger_type"] == "configuration" => {
                    assert_eq!(value["function_id"], "judge-semif::on-config-change");
                    assert_eq!(
                        value["config"],
                        json!({
                            "configuration_id": "semif-boot-test",
                ...
                "registertrigger" if value["trigger_type"] == "console:script" => {
                    ...
                    assert_eq!(value["function_id"], "judge-semif::ui-content");
                    assert_eq!(value["config"]["path"], "judge-semif/page.js");
                    script = true;
                }
                "registertrigger" if value["trigger_type"] == "console:style" => {
                    assert!(console_ui_enabled);
                    assert_eq!(value["function_id"], "judge-semif::ui-content");
                    assert_eq!(value["config"]["path"], "judge-semif/styles.css");
                    style = true;
                }
                "invocationresult" if value["function_id"] == "judge-semif::evaluate" => {
                    assert_eq!(value["result"]["status"], "ok", "{value}");
                    assert_eq!(value["result"]["model"], "qwen3.5-4b");
                    assert!(
                ...
                "invocationresult" if value["function_id"] == "judge-semif::configuration-id" => {
                    assert!(console_ui_enabled);
                    assert_eq!(value["result"], json!({"id":"semif-boot-test"}));
                    assert!(value.get("error").is_none());
                    identity = true;
                }
                _ => {}
    ...
    if let Some(signal) = shutdown_signal {
        timeout(Duration::from_secs(10), async {
            while let Some(line) = logs.recv().await {
                if line.contains("judge-semif ready") {
                    return;
                }
            }
