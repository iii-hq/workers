...
pub fn register(iii: &IIIClient, config: SharedConfig, slot: Arc<ModelSlot<SemifClient>>) {
    ...
    iii.register_function(crate::MODELS_ID, registration.request_format(request_schema).description("Describe the SemIf model, loading it if needed (name, pinned revision, device, context window); performs no inference.").metadata(json!({ "internal": true })));
