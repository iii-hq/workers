...
The same worker also runs Claude Code as a **terminal on the console**: `claude::terminal::*` installs the CLI on the terminal host, equips a workspace with the iii skills, and opens Claude in a `shell::pty` session on its own page — always Claude, never a shell. Both halves share one login and one events stream, so a headless turn and a typed turn look the same in the console. Its sibling [`pi-cli`](https://github.com/iii-hq/workers/tree/main/pi-cli) is the terminal half for the pi agent.
