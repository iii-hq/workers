...
//!    session as `timed_out`, so a previously queued approval can no
//!    longer dispatch the inner function after the user clicked stop.
...
pub async fn execute(iii: III, payload: Value) -> Result<Value, IIIError> {
    ...
    //    its inner function after the user clicked stop. The cause is
    //    logged here — approval-gate no longer persists a reason string
    //    on the swept records (status: "timed_out" is self-describing).
    if let Err(e) = iii
        .trigger(TriggerRequest {
        ...
        .await
    {
        ...
        tracing::info!(%session_id, cause = "run_stopped", "run::stop: swept pending approvals");
    }
