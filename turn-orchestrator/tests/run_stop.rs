...
async fn run_stop_invokes_all_primitives_on_happy_path() {
    ...
    // The legacy `reason` payload field is gone — approval-gate's
    // timed_out status is self-describing. run::stop now logs the
    // cause locally instead of forwarding it on the wire.
    assert!(
        sweep_calls[0].get("reason").is_none(),
        "approval::sweep_session payload must not carry a reason: {:?}",
        sweep_calls[0]
    );
}
