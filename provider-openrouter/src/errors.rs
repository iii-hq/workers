...
//!
//! OpenRouter has documented, gateway-level status semantics that differ from
//! a single vendor's: 402 is an insufficient-credits billing wall, 403 is a
//! moderation flag on the input (NOT an auth failure), 408 is the gateway's
//! own timeout, and 502/503 mean the chosen upstream is down or no provider
//! is available — retryable through the router. The error envelope is
//! `{"error": {"code": <number>, "message", "metadata"?}}`, with raw upstream
//! errors preserved under `metadata.raw`.
use iii_sdk::errors::Error;
use llm_router::types::events::ErrorKind;
use serde_json::Value;

/// Map an OpenRouter HTTP status + error body to the shared taxonomy.
...
/// The OpenRouter envelope: `{"error": {"code": <number>, "message"}}` with
/// the HTTP status repeated as the numeric `code`. Upstream vendors' own
/// OpenAI-style string codes can also surface through compatibility proxies;
...
mod tests {
    ...
    fn numeric_envelope_codes_are_honored() {
        let body = r#"{"error":{"code":402,"message":"Insufficient credits. Add more using https://openrouter.ai/settings/credits"}}"#;
