...
//!
//! OpenRouter normalizes vendor reasoning streams into a `reasoning` string
//! on the delta; some OpenAI-compatible gateways reachable via `api_url`
//! still use the older `reasoning_content` field. Both are surfaced as a
//! Thinking block (ThinkingStart/Delta/End) preceding the answer's Text
//! block. With `usage: {include: true}` the final usage chunk also carries
//! `cost` — the actual billed USD (cache discounts included) — which lands on
//! `usage.cost_usd`; the router's catalog-pricing fill keeps a
...
fn build_content(state: &PartialState) -> Vec<ContentBlock> {
    let mut out = Vec::new();
    // Thinking precedes the answer text (reasoning streams first). OpenRouter
...
/// The reasoning text on a delta: OpenRouter's normalized `reasoning` string,
/// falling back to the older `reasoning_content` used by some gateways.
...
pub fn handle_chunk(
    ...
    // Mid-stream error envelope (OpenRouter relays provider failures as an
    ...
    if let Some(finish) = choice.get("finish_reason").and_then(Value::as_str) {
        ...
        if finish == "content_filter" {
            state.warnings.push(
                "openrouter filtered the completion (finish_reason: content_filter)".to_string(),
            );
        }
        close_open_block(state, model, &mut events);
    }
...
mod tests {
    ...
    fn run(chunks: &[Value]) -> (PartialState, Vec<AssistantMessageEvent>) {
        ...
        let mut events = Vec::new();
        for c in chunks {
            events.extend(handle_chunk(c, &mut state, "openrouter/vendor/model-x"));
        }
        (state, events)
    }
    ...
    fn synthetic_error_event_shape() {
        let ev = synthetic_error_event("boom", "openrouter/vendor/model-x", ErrorKind::RateLimited);
        match ev {
            AssistantMessageEvent::Error { error } => {
