//! OpenRouter Chat Completions streaming via provider-base.
...
pub struct OpenRouterConfig {
    ...
    /// <value>`. Populated via [`OpenRouterConfig::with_credential`] in
    /// P5; the legacy [`OpenRouterConfig::from_env`] constructor still
...
pub async fn stream(
    ...
    .with_extra_header("HTTP-Referer", "https://iii.dev")
    .with_extra_header("X-Title", "iii-harness");
...
/// Register `provider::openrouter::stream` on the iii bus.
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<OpenRouterConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| {
            OpenRouterConfig::with_credential(model, cred)
        },
        stream,
    );
    Ok(())
...
mod tests {
    ...
    fn config_from_env_behavior() {
        ...
        assert!(OpenRouterConfig::from_env("test-model").is_err());
        std::env::set_var(ENV_VAR, "test-key");
        let cfg = OpenRouterConfig::from_env("test-model").unwrap();
    ...
    fn with_credential_api_key() {
        ...
        let cfg = OpenRouterConfig::with_credential("the-model", &cred).unwrap();
