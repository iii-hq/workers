...
const STUB_SSE: &str = "HTTP/1.1 200 OK\r\ncontent-type: text/event-stream\r\nconnection: close\r\n\r\ndata: {\"choices\":[{\"index\":0,\"delta\":{\"role\":\"assistant\",\"content\":\"\"}}]}\n\ndata: {\"choices\":[{\"index\":0,\"delta\":{\"content\":\"Hello\"}}]}\n\ndata: {\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\"}]}\n\ndata: {\"choices\":[],\"usage\":{\"prompt_tokens\":12,\"completion_tokens\":2,\"cost\":0.00014,\"prompt_tokens_details\":{\"cached_tokens\":4}}}\n\ndata: [DONE]\n\n";

const STUB_401: &str = "HTTP/1.1 401 Unauthorized\r\ncontent-type: application/json\r\nconnection: close\r\n\r\n{\"error\":{\"code\":401,\"message\":\"No auth credentials found\"}}";

// OpenRouter-shaped listing. vendor-a/agentic: full metadata, admitted.
// vendor-b/bare: sparse but tools+text, admitted with conservative defaults.
// vendor-c/chatty: no tools support → filtered. vendor-d/painter: image
// output → filtered.
const STUB_MODELS: &str = "HTTP/1.1 200 OK\r\ncontent-type: application/json\r\nconnection: close\r\n\r\n{\"data\":[\
{\"id\":\"vendor-a/agentic\",\"name\":\"Vendor A: Agentic\",\"context_length\":200000,\
...
{\"id\":\"vendor-b/bare\",\"supported_parameters\":[\"tools\"],\
\"architecture\":{\"output_modalities\":[\"text\"]}},\
{\"id\":\"vendor-c/chatty\",\"supported_parameters\":[\"temperature\"],\
\"architecture\":{\"output_modalities\":[\"text\"]}},\
{\"id\":\"vendor-d/painter\",\"supported_parameters\":[\"tools\"],\
...
async fn boot_stack(engine_url: &str) -> (IIIClient, IIIClient) {
    ...
    loop {
        ...
        let registered = list["providers"]
            .as_array()
            .is_some_and(|p| p.iter().any(|x| x["id"] == "openrouter"));
        if registered {
            break;
        }
        assert!(
...
async fn refresh_and_wait(router_iii: &IIIClient, provider_iii: &IIIClient, expect_id: &str) {
    let res = call(
        provider_iii,
        "provider::openrouter::refresh_models",
        json!({}),
    )
    .await
...
async fn provider_registers_with_persisted_token_and_live_only_catalog() {
    ...
    // until live discovery can run (models come from GET /api/v1/models only).
    let list = call(
        &router_iii,
        "router::models::list",
        json!({ "provider": "openrouter" }),
    ...
    let token = call(
        &provider_iii,
        "state::get",
        json!({ "scope": "provider-openrouter", "key": "registration_token" }),
    )
    .await
    .unwrap();
    assert!(
...
#[tokio::test(flavor = "multi_thread")]
async fn chat_streams_end_to_end_with_billed_cost_passthrough() {
    let engine = engine_or_skip!();
    ...
    refresh_and_wait(&router_iii, &provider_iii, "openrouter/vendor-a/agentic").await;
    ...
    let res = consumer
        .trigger(TriggerRequest {
            ...
            payload: json!({
                ...
                "model": "openrouter/vendor-a/agentic",
    ...
    // usage accounting: the upstream's billed cost rides through verbatim —
    // the router's catalog-pricing fill keeps a provider-supplied value.
    assert_eq!(res["usage"]["cost_usd"], 0.00014, "cost passthrough: {res}");
...
async fn refresh_models_reconciles_admitted_live_catalog_with_metadata() {
    ...
    // exactly the admitted live list under the openrouter/ prefix:
    // tools+text models stay, tool-less and non-text models are gone.
    assert!(ids.contains(&"openrouter/vendor-a/agentic"), "got {ids:?}");
    assert!(
        ids.contains(&"openrouter/vendor-b/bare"),
        "sparse tools+text model still routes: {ids:?}"
    );
    assert!(
        !ids.contains(&"openrouter/vendor-c/chatty"),
        "tool-less model should be filtered: {ids:?}"
    );
    assert!(
        !ids.contains(&"openrouter/vendor-d/painter"),
        "image generator should be filtered: {ids:?}"
    );
    ...
    let known = models
        .iter()
        .find(|m| m["id"] == "openrouter/vendor-a/agentic")
        .unwrap();
    assert_eq!(known["display_name"], "Vendor A: Agentic");
    assert_eq!(known["context_window"], 200_000);
    assert_eq!(known["max_output_tokens"], 64_000);
    assert_eq!(known["supports_vision"], true);
    assert_eq!(known["supports_thinking"], true);
    assert_eq!(known["supports_structured_output"], true);
    // per-token strings converted to USD per MTok
    assert_eq!(known["pricing"]["input"], 3.0);
    assert_eq!(known["pricing"]["output"], 15.0);
    let unknown = models
        .iter()
        .find(|m| m["id"] == "openrouter/vendor-b/bare")
        .unwrap();
