//! Wire-schema snapshots for the `provider::openrouter::*` functions.
//!
//! `provider_openrouter::surface::catalog()` is the single source of truth for each
...
use provider_openrouter::surface::{catalog, FunctionSpec};
...
fn catalog_lists_all_functions_in_registration_order() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "provider::openrouter::abort",
            "provider::openrouter::refresh_models",
            "provider::openrouter::on_router_ready",
        ]
    );
}
