...
pub async fn register_config(iii: &IIIClient, seed: Option<&QueueConfig>) -> Result<(), String> {
    ...
    // The candidate (seed, else the durable packaged default) is forwarded
...
/// Read the assigned entry without treating unrelated failures as permission to seed.
    ...
    for attempt in 1..=CONFIG_RETRIES {
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                if matches!(&err, iii_sdk::errors::Error::Remote { code, .. } if code == "function_not_found" || code == "NOT_FOUND")
                {
                    return Err(last_err);
                }
...
mod tests {
    ...
    /// Failed configuration lookups must not silently replace the selected queue adapter.
    #[test]
