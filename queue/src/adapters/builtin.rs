...
impl QueueAdapter for BuiltinAdapter {
    async fn enqueue(
        ...
        // bare topic name.
        let topic_subscriptions = self.topic_subscriptions.lock().await;
        match topic_subscriptions.get(topic) {
    ...
    async fn unsubscribe(&self, topic: &str, id: &str) {
        ...
        let Some(sub) = subscriptions.remove(&key) else {
        ...
        let queue_name = internal_queue_name(topic, id);
        if let Err(error) = self.store.purge(&queue_name).await {
            tracing::error!(topic = %topic, id = %id, %error, "failed to purge unsubscribed queue");
        }
        drop(subscriptions);
    ...
    loop {
        ...
        let next = tokio::select! {
            ...
            result = async {
                ...
                let job = store.dequeue(&cfg.queue_name).await;
                Some((permit, job))
            } => result,
        };
        let Some((permit, job)) = next else {
            break;
        };
...
mod tests {
    ...
    async fn unsubscribe_stops_delivery() {
        ...
        assert_eq!(store.topic_stats(&queue_name).await, TopicStats::default());
        assert!(!store.list_topics().await.contains(&queue_name));
    ...
    #[tokio::test]
    async fn unsubscribe_joins_inflight_delivery_before_purge_and_rearm() {
        let store: Arc<dyn QueueStore> = Arc::new(InMemoryStore::new());
        ...
        adapter
            .subscribe("demo", "sub1", "backend", None, None, None)
            .await;
        adapter.enqueue("demo", json!("old"), None, None).await;
        ...
        adapter
            .subscribe("demo", "sub1", "backend", None, None, None)
            .await;
        adapter.enqueue("demo", json!("new"), None, None).await;
        wait_until(|| {
            let invoker = invoker.clone();
            async move { invoker.completed.lock().await.as_slice() == [json!("new")] }
        })
        .await;
    ...
    #[tokio::test]
    async fn publish_unsubscribe_and_same_id_rearm_are_serialized() {
        ...
        assert!(
            adapter.topic_subscriptions.try_lock().is_err(),
            "publish must retain the routing lock until its private writes finish"
        );
        ...
        wait_until(|| {
            let adapter = adapter.clone();
            async move { adapter.subscriptions.try_lock().is_err() }
        })
        .await;
        let rearming = {
            let adapter = adapter.clone();
        ...
        assert!(!unsubscribing.is_finished());
        assert!(!rearming.is_finished());

        store.release.notify_one();
        tokio::time::timeout(Duration::from_secs(1), async {
            publishing.await.unwrap();
            unsubscribing.await.unwrap();
            rearming.await.unwrap();
        })
        .await
        .expect("publish, retirement, and rearm should not deadlock");

        adapter.enqueue("demo", json!("fresh"), None, None).await;
        wait_until(|| {
            let invoker = invoker.clone();
            async move {
                invoker
                    ...
                    .await
                    .iter()
                    .any(|(_, payload)| payload == &json!("fresh"))
            }
        })
        .await;
        assert_eq!(
            invoker.calls().await,
            vec![("backend".to_string(), json!("fresh"))]
        );
