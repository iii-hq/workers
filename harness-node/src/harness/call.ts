 ...
 *
 * The Rust harness exposes `harness::call` both as an HTTP route and as a
 * bus function. The browser-facing pattern (see
 * `workers/harness/web/src/App.tsx:317-340`) routes every chat turn
 ...
 * `instrumentSdk` Proxy in `runtime/worker.ts`, so the inner call's
 * `harness.run::start` span ALREADY gets the IDs from its own payload.
 * Adding `harness::call` matches the harness/web pattern explicitly so
 * console/web can use the same wrapper without depending on whether the
 * inner function happens to be auto-wrapped. It also gives us one well-
 * defined ingestion point for browser-originated calls that keeps the
...
/** Upper bound for a single harness::call. Mirrors the Rust constant. */
  ...
  iii.registerFunction(
    ...
    async (input: unknown) => {
      ...
      if (typeof functionId !== 'string' || functionId.length === 0) {
        throw new Error('harness::call: missing function_id');
      }
      const inner =
