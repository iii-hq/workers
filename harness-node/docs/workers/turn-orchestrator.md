...
- **State trigger** on `scope: agent` (no condition function — fires on every write to `session/<sid>/turn_state`) → `turn::on_record_written`. Registered in [src/turn-orchestrator/on-record-written.ts](harness-node/src/turn-orchestrator/on-record-written.ts). Emits a `turn_state_changed` event to `agent::events` on every `turn_state` write, carrying the full new (and prior) record so the UI can derive pending approvals from state rather than from a signal event.
...
| [src/turn-orchestrator/on-record-written.ts](harness-node/src/turn-orchestrator/on-record-written.ts) | State-trigger adapter on `agent / session/<sid>/turn_state` that emits `turn_state_changed` to `agent::events` for the subscribed UI. Carries the full new (and prior) `TurnStateRecord` so the console can derive pending approvals from state rather than from a signal event. |
