//! Kimi for Coding (Moonshot) Chat Completions streaming via provider-base.
...
const PROVIDER_NAME: &str = "kimi-coding";

#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct KimiCodingConfig {
    ...
    /// <value>`. Populated via [`KimiCodingConfig::with_credential`] in
    /// P5; the legacy [`KimiCodingConfig::from_env`] constructor still
...
/// Register `provider::kimi-coding::stream` on the iii bus.
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<KimiCodingConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| {
            KimiCodingConfig::with_credential(model, cred)
        },
        stream,
    );
    Ok(())
...
mod tests {
    ...
    /// so splitting these caused a race on `MOONSHOT_API_KEY`. Coverage is identical
    /// when sequenced.
    #[test]
    fn config_env_resolution() {
        let prev = std::env::var("MOONSHOT_API_KEY").ok();

        std::env::remove_var("MOONSHOT_API_KEY");
        assert!(KimiCodingConfig::from_env("test-model").is_err());

        std::env::set_var("MOONSHOT_API_KEY", "test-key");
        let cfg = KimiCodingConfig::from_env("test-model").expect("ok");
    ...
    fn with_credential_api_key() {
        ...
        let cfg = KimiCodingConfig::with_credential("the-model", &cred).unwrap();
