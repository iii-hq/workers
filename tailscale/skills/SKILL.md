...
- `tailscale::share::stop` — stop one route by mode, HTTPS port, and path: `funnel` removes public access and keeps the tailnet-only route, `serve` removes the route entirely.
