//! Wire-schema snapshots for the `provider::elevenlabs::*` functions.
//!
//! `provider_elevenlabs::surface::catalog()` is the single source of truth for each
...
use provider_elevenlabs::surface::{catalog, FunctionSpec};
...
fn catalog_lists_all_functions_in_registration_order() {
    ...
    assert_eq!(
        ids,
        vec![
            "provider::elevenlabs::transcribe",
            "provider::elevenlabs::speak",
            "provider::elevenlabs::refresh_models",
            "provider::elevenlabs::on_router_ready",
        ]
    );
}
