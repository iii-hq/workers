//! `provider-elevenlabs` binary entry.
//!
//! The worker keeps no operator settings of its own: the credential and
//! `api_url` arrive per request from llm-router's resolve step. `--config`
//! is still accepted per the binary-worker CLI contract; keys found there
//! are warned about instead of silently dropped.

use clap::Parser;
use iii_sdk::runtime::WorkerMetadata;
use iii_sdk::{register_worker, InitOptions};
use provider_elevenlabs::register::register_provider;

#[derive(Parser, Debug)]
#[command(
    name = "provider-elevenlabs",
    about = "ElevenLabs speech provider worker behind llm-router."
)]
struct Cli {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.manifest {
        println!(
            "{}",
            serde_json::to_string_pretty(&provider_elevenlabs::manifest::build_manifest())?
        );
        return Ok(());
    }
    ...
    if let Ok(contents) = std::fs::read_to_string(&cli.config) {
        if has_config_keys(&contents) {
            tracing::warn!(
                path = %cli.config,
                "provider-elevenlabs takes no file-based config; configure the provider \
    ...
    let iii = register_worker(
        ...
        InitOptions {
            metadata: Some(WorkerMetadata {
                ...
                name: "provider-elevenlabs".to_string(),
    ...
    tracing::info!(url = %cli.url, "provider-elevenlabs registered");
