//! Fireworks Chat Completions streaming via provider-base.
...
pub struct FireworksConfig {
    ...
    /// <value>`. Populated via [`FireworksConfig::with_credential`] in
    /// P5; the legacy [`FireworksConfig::from_env`] constructor still
...
/// Register `provider::fireworks::stream` on the iii bus.
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<FireworksConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| {
            FireworksConfig::with_credential(model, cred)
        },
        stream,
    );
    Ok(())
...
mod tests {
    ...
    fn config_from_env_behavior() {
        ...
        assert!(FireworksConfig::from_env("test-model").is_err());
        std::env::set_var(ENV_VAR, "test-key");
        let cfg = FireworksConfig::from_env("test-model").unwrap();
    ...
    fn with_credential_api_key() {
        ...
        let cfg = FireworksConfig::with_credential("the-model", &cred).unwrap();
