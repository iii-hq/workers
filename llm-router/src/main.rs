//! `llm-router` binary entry.
//!
//! Operator config — providers, credentials, model routing — lives in the
//! engine's `llm-router` configuration entry (spec § Configuration), not in
//! a config file. `--config` is still accepted per the binary-worker CLI
//! contract (the engine passes it when an operator sets a config block);
...
#[command(
    name = "llm-router",
    about = "One front door + provider protocol in front of every LLM provider."
)]
struct Cli {
    /// Accepted for the standard worker CLI contract; llm-router reads its
    /// operator config from the engine's `llm-router` configuration entry.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(contents) = std::fs::read_to_string(&cli.config) {
        if has_config_keys(&contents) {
            tracing::warn!(
                path = %cli.config,
                "llm-router takes no file-based config; set the engine's `llm-router` \
                 configuration entry instead — ignoring this file's keys"
            );
        }
    }
...
mod tests {
    ...
    fn real_keys_are_detected() {
        assert!(has_config_keys("providers: {}\n"));
        assert!(has_config_keys("# comment\nmax_tokens: 8192\n"));
    }
}
