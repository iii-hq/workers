//! Wire-schema snapshots for the `router::*` functions.
//!
//! `llm_router::surface::catalog()` is the single source of truth for each
...
//! compared against `tests/golden/schemas/<id>.json` (`::` maps to `.` in
//! filenames).
//!
...
use llm_router::surface::{catalog, FunctionSpec};
...
/// The catalog must cover exactly the 13 registered functions, in registration
