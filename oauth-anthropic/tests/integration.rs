//! Integration coverage for oauth-anthropic.
//!
//! These tests exercise the public API as a black box: the authorize URL
//! builder via the `oauth_anthropic::testing` re-exports, and the wiring
//! path via an in-memory [`oauth_anthropic::testing::CredentialBus`] that
...
/// In-memory [`oauth_anthropic::testing::CredentialBus`] for tests.
///
/// Records every `set_token`, `record_function`, and `record_trigger` call
/// the production code makes against the bus.
struct InMemoryBus {
...
impl oauth_anthropic::testing::CredentialBus for InMemoryBus {
    fn set_token<'a>(
        &'a self,
        provider: &'a str,
        cred: &'a Credential,
...
fn library_exports_register_entry_point() {
    let _ = &oauth_anthropic::register_with_iii;
}

#[test]
fn authorize_url_includes_required_pkce_params() {
    let url = oauth_anthropic::testing::build_authorize_url(
        "TEST_CHALLENGE",
        "TEST_STATE",
...
async fn register_with_iii_records_expected_function_ids() {
    ...
    let bus_dyn: Arc<dyn oauth_anthropic::testing::CredentialBus> = bus.clone();

    let refs = oauth_anthropic::testing::register_with_iii_with_bus(&iii, bus_dyn)
        .await
    ...
    let ids: Vec<String> = bus
        ...
        .map(|m| m.id.clone())
        .collect();
    assert!(ids.contains(&"oauth::anthropic::login".to_string()));
    assert!(ids.contains(&"oauth::anthropic::refresh".to_string()));
    assert!(ids.contains(&"oauth::anthropic::status".to_string()));
    assert_eq!(ids.len(), 3);

    // Triggers: oauth-anthropic registers no triggers today.
    assert!(bus.recorded_triggers().is_empty());
    // Token storage path: handlers return the credential to the caller; no
    // bus.set_token call happens during register.
