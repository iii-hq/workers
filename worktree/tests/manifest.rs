//! `--manifest` subprocess contract: valid JSON with the five fields the
//! registry publish pipeline requires.
...
fn manifest_subcommand_emits_valid_json() {
    let bin = env!("CARGO_BIN_EXE_worktree");
    let output = Command::new(bin)
        .arg("--manifest")
        .output()
        .expect("spawn worktree --manifest");
    ...
    assert_eq!(manifest["default_config"]["land_queue"], "worktree-land");
    assert!(
