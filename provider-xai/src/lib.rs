...
pub struct XaiConfig {
    ...
    /// <value>`. Populated via [`XaiConfig::with_credential`] in P5; the
    /// legacy [`XaiConfig::from_env`] constructor still reads the env
    /// var directly.
    pub api_key: String,
    pub model: String,
    pub max_tokens: u32,
...
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<XaiConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| XaiConfig::with_credential(model, cred),
        stream,
    );
    Ok(())
}
...
mod tests {
    ...
    fn with_credential_api_key() {
        ...
        let cfg = XaiConfig::with_credential("the-model", &cred).unwrap();
