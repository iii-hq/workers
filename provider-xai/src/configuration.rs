...
pub const DEFAULT_CONFIG_ID: &str = "provider-xai";
...
pub async fn register_config(iii: &IIIClient) -> Result<(), String> {
    ...
    // The candidate default is forwarded unconditionally: `configuration::ensure`
    // installs it atomically ONLY against an absent/null entry, so a stored
    // operator/Compose override is preserved without a client-side
...
/// Refresh the applied provider settings from the assigned configuration entry.
