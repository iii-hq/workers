...
/// Last-wins merge: standard xAI reports usage once (final include_usage
/// chunk); xAI-compatible servers that report per-chunk report cumulative
...
pub fn handle_chunk(
    ...
    if let Some(finish) = choice.get("finish_reason").and_then(Value::as_str) {
        ...
        if finish == "content_filter" {
            state
                .warnings
                .push("xai filtered the completion (finish_reason: content_filter)".to_string());
        }
        close_open_block(state, model, &mut events);
    }
    events
...
mod tests {
    ...
    fn run(chunks: &[Value]) -> (PartialState, Vec<AssistantMessageEvent>) {
        ...
        let mut events = Vec::new();
        for c in chunks {
            events.extend(handle_chunk(c, &mut state, "grok-test"));
        }
        (state, events)
    }
    ...
    fn synthetic_error_event_shape() {
        let ev = synthetic_error_event("boom", "grok-test", ErrorKind::RateLimited);
        match ev {
            AssistantMessageEvent::Error { error } => {
                assert_eq!(error.error_kind, Some(ErrorKind::RateLimited));
                assert_eq!(error.stop_reason, StopReason::Error);
                assert_eq!(error.provider, "xai");
            }
