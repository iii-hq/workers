...
#[command(
    name = "provider-xai",
    about = "xAI Chat Completions provider worker behind llm-router."
)]
struct Cli {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.manifest {
        println!(
            "{}",
            serde_json::to_string_pretty(&provider_xai::manifest::build_manifest())?
        );
        return Ok(());
    }
    ...
    if let Ok(contents) = std::fs::read_to_string(&cli.config) {
        if has_config_keys(&contents) {
            tracing::warn!(
                path = %cli.config,
                "provider-xai takes no file-based config; configure the provider \
    ...
    tracing::info!(url = %cli.url, "provider-xai registered");
