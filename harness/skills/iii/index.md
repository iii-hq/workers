...
When one fits, install it: `compose::add { worker: '<worker>' }` — it declares the worker (plus its registry dependencies) in the compose file, pins resolved versions, and waits until newly declared workers are ready.
...
//   → empty? check compose::status for a stopped 'iii-directory' and start it:
// compose::up { container: 'iii-directory' }
//   → not declared at all? install it first:
// compose::add { worker: 'iii-directory' }
```

**3. Build a worker.** Only when steps 1 and 2 both come up empty. Author it with the SDK (below), then deploy it. Discover the deployment/runtime surface the same way as any other capability — `directory::registry::workers::list` / `::info` and its skill — rather than assuming a worker name. (`compose::add` installs registry packages; for un-published code that lives elsewhere, run it via a runtime/sandbox worker or declare it by hand in the compose file.)
...
## Worker lifecycle — `compose::*` ops

Install, run, and remove workers through the Compose daemon supervising the stack. Fetch exact request/response shapes from the daemon rather than trusting this list:

```jsonc
// compose::schema { function_id: 'compose::add' }  → request/response JSON Schemas;
//                                                    omit function_id for every compose::* op
```

| Op | Does |
|---|---|
| `compose::add` | Declare worker(s) + registry dependencies in the compose file, pin resolved versions, start them, and wait until ready. Leaves already-running workers in place. |
| `compose::remove` | Remove one declared worker and dependency references to it, stop only that worker, then reconcile. |
| `compose::update` | Move one declared package worker to a requested or latest version, then restart the project. |
| `compose::up` | Start the project, or one container and its dependencies. Repeated calls leave ready containers running. |
| `compose::down` | Stop the project, or one container and its dependents, in reverse dependency order. |
| `compose::restart` | Restart the whole project, or one named container without changing its dependency graph. |
| `compose::status` | Project namespace, state directory, daemon pid, and current state of every declared container. |
| `compose::list` | Every project loaded by this compose daemon. |
| `compose::validate` | Validate a worker-compose.yaml offline without loading it. |
| `compose::schema` | JSON Schemas for every op. |

- The harness routes `compose::*` calls to its supervising Compose daemon and scopes them to its own compose file — do not add `namespace` or `file` yourself.
- Reach for `compose::status` before any other op when you don't already know what is declared and running.
...
| `compose::status` | Declared workers and their current state, whether WS-connected or not. |
...
`engine::workers::list` only sees WS-connected workers; some daemon-managed providers serve traffic without opening an engine WS, so merge it with `compose::status` by container name for the full picture.
