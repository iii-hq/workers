...
pub struct AgentQualitySubjectV1 {
...
#[derive(Debug, Clone)]
pub struct ResolvedAgentQualitySubjectV1 {
    pub subject_id: String,
...
impl ResolvedAgentQualitySubjectV1 {
...
pub fn load(path: &Path) -> Result<ResolvedAgentQualitySubjectV1, EvalError> {
    let subject_bytes = std::fs::read(path)
        .map_err(|error| EvalError::setup(format!("read subject {}: {error}", path.display())))?;
    let subject: AgentQualitySubjectV1 = serde_json::from_slice(&subject_bytes)
    ...
    Ok(ResolvedAgentQualitySubjectV1 {
