...
use harness_agent_quality::{
    compare_runs, comparison_dimension, run_suite, subject, AgentQualityLimitsV1,
    AgentQualityRunReportV1, LimitsArgs, ResolvedAgentQualitySubjectV1, SuiteRunConfig,
};
...
) -> Result<AgentQualityRunReportV1> {
    tracing::info!(
        variant,
        run = repetition + 1,
        total_runs = cli.runs,
