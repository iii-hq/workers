...
use crate::limits::{limit_failures, AgentQualityLimitsV1};
use crate::report::{AgentQualityScenarioReportV1, ScenarioObservationV1};
...
pub async fn run(
    ...
    spec: ScenarioSpec,
) -> AgentQualityScenarioReportV1 {
    let started = Instant::now();
    let session_id = format!("aq_{}", Uuid::new_v4().simple());
    let mut report =
        AgentQualityScenarioReportV1::new(spec.id, session_id.clone(), spec.prompt.clone());
...
fn send_request(
    ...
    SendRequest {
        ...
        idempotency_key: Some(format!("agent-quality:{run_id}:{}:send", spec.id)),
        session: Some(SessionInit {
            title: Some(format!("Agent quality: {}", spec.id)),
            metadata: Some(json!({
                "agent_quality_run_id": run_id,
                "agent_quality_scenario": spec.id,
            })),
        }),
