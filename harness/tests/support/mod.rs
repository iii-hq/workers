//! Hand-rolled golden-file harness (no snapshot dependency). Goldens live
//! under `tests/golden/` and are committed; any wire-surface change must show
//! up as an explicit, reviewed diff.
//!
//! - `cargo test` compares actual output against the committed goldens.
//! - `UPDATE_GOLDENS=1 cargo test` regenerates them; review the git diff, then
//!   commit alongside the change that caused them.
...
/// Assert a schema is a real typed schema, not the permissive `AnyValue`
/// schema a `Value` handler emits (SOP §7).
pub fn assert_typed_schema(label: &str, schema: &schemars::schema::RootSchema) {
    ...
    assert!(
        has_defining,
        "{label}: schema is the permissive AnyValue/empty schema. The handler is registered with \
         `Value` — give it a typed struct deriving JsonSchema. Got: {value}"
    );
}
