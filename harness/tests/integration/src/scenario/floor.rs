...
/// Terminal status `completed` with no pending calls or queued messages.
/// `children` is deliberately NOT a failure: spawns are fire-and-forget (the
/// call checkpoint settles `Done` at spawn time), so a terminal turn that
/// spawned a leaf legitimately lists it — bookkeeping, not pending work.
fn terminal_failure(run: &RunEvidence) -> Option<String> {
    ...
    if status == Some("completed") && pending == 0 && queued == 0 {
        return None;
    }
    Some(format!(
        "floor: terminal status must be completed with nothing pending; got status {status:?}, \
         {pending} pending call(s), {queued} queued message(s)"
    ))
}
