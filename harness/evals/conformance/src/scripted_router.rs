...
impl ScriptedRouter {
    ...
    pub async fn start(ws_url: &str, mut script: RouterScriptV1) -> anyhow::Result<Self> {
        ...
        let client = Client::connect(ws_url, "conformance-scripted-router");
    ...
    fn register_all(&self) {
...

        {
            ...
            iii.register_function(
                ...
                .description("Scripted conformance chat: replays fixture generations.")
        ...
        }
        {
            ...
            iii.register_function(
                ...
                .description("Scripted conformance abort: first abort of a live call wins.")
                .metadata(json!({ "internal": true })),
            );
        }
        {
            ...
            iii.register_function(
                ...
                .description("Scripted conformance catalog: the single fixture model."),
            );
        }
        {
            let state = self.state.clone();
            iii.register_function(
                ...
                .description("Scripted conformance catalog lookup."),
            );
        }
        {
            let state = self.state.clone();
            iii.register_function(
                ...
                .description("Scripted conformance capability check."),
            );
        }
        iii.register_function(
            ...
            .description("Scripted conformance system prompt: defers to the harness built-in."),
        );
    }
...
async fn chat(state: Arc<Mutex<State>>, address: String, input: Value) -> Result<Value, Error> {
    ...
    let (generation, writer_ref, request_id) = {
        ...
        let Some(generation) = generation else {
            ...
            return Err(Error::Handler(
                "conformance/unexpected_call: router::chat after script exhausted".into(),
            ));
        };
        ...
        if !failed.is_empty() {
            ...
            return Err(Error::Handler(format!(
                "conformance/match_failed: generation {}: {detail}",
                generation.ordinal
            )));
        }
        ...
        let writer_ref: StreamChannelRef =
            serde_json::from_value(input.get("writer_ref").cloned().unwrap_or(Value::Null))
                .map_err(|e| Error::Handler(format!("conformance/writer_ref: {e}")))?;
        let request_id = input
            .get("request_id")
            .and_then(Value::as_str)
            .unwrap_or_default()
    ...
    serde_json::to_value(&generation.response)
        .map_err(|e| Error::Handler(format!("conformance/response_serialize: {e}")))
}
...
async fn stream_frames(
    ...
    for frame in &generation.frames {
        let text = serde_json::to_string(frame)
            .map_err(|e| Error::Handler(format!("conformance/frame_serialize: {e}")))?;
