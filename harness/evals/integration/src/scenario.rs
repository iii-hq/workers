...
use crate::types::scenario::{Classification, IntegrationResultV1, IntegrationScenarioV1};
...
impl ScenarioRunner<'_> {
    ...
    async fn run_armed(
        &mut self,
        stack: &mut Stack,
        scenario: &IntegrationScenarioV1,
        ...
        let client = Client::connect(&stack.ws_url, "integration-runner");
        ...
        if let Some(http_port) = stack.console_http_port {
                ...
                crate::console_recording::ConsoleRecording::wait_http_ready(http_port, 30_000).await
            {
                ...
                let chrome = std::path::PathBuf::from(
                    std::env::var("INTEGRATION_CHROME")
    ...
    async fn arm(&self, scenario: &IntegrationScenarioV1, client: &Client) -> anyhow::Result<()> {
        let configure = json!({
            "schema_version": "1",
            "run_id": self.run_id,
        ...
        let response = client
            .call("integration-recorder::configure", configure)
            .await
    ...
    fn expand_fixture(&self) -> anyhow::Result<(IntegrationScenarioV1, RouterScriptV1, String)> {
        ...
        let scenario: IntegrationScenarioV1 = serde_json::from_value(scenario_value)?;
