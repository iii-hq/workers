...
`hook-fanout::reply_handler` — delivers every reply frame into the
...
| [src/hook-fanout/publish-collect.ts](harness/src/hook-fanout/publish-collect.ts) | `execute` (installs the collector, publishes, awaits the exit reason, applies merge_rule, builds the response) + `handleStreamReply` (stream-trigger callback that delivers replies into pending collectors). |
