/**
 * `provider::anthropic::refresh_models` — re-pull the upstream model list
 * and register each into the iii models catalog. Returns `{ registered }`.
...
export const FUNCTION_ID = 'provider::anthropic::refresh_models';
  ...
  iii.registerFunction(
    ...
    async (): Promise<RefreshResult> => {
      try {
        ...
        logger.warn('provider::anthropic::refresh_models failed', { err: String(err) });
        return { registered: [] };
      }
    },
    {
