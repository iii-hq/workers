...
pub struct SpawnOptions {
    ...
    /// Subset against the parent policy — a child inherits the parent's deny
    /// globs and any legacy explicit allow ceiling, so it can narrow but never
    /// escalate. An `ask`-mode child is further capped at the configured
    /// default policy.
    #[serde(default, skip_serializing_if = "Option::is_none")]
