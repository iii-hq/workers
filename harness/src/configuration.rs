...
/// read-then-`register` seed, which could clobber a stored override. This is
/// the seed/schema initialization path ONLY — the deliberate legacy-migration
/// `configuration::register`/`set` writes below stay untouched.
...
mod tests {
    ...
    /// Preserve operator settings when configuration lookup fails for another reason.
    #[test]
