...
async fn basic_get_returns_body_and_status() {
    ...
    let out = execute_fetch(
        payload(json!({ "url": format!("{}/hi", server.uri()) })),
        &WebConfig::default(),
    )
    .await;
...
async fn json_mode_autoparses_when_not_truncated() {
    ...
    let out = execute_fetch(
        payload(json!({ "url": format!("{}/j", server.uri()), "response_format": "json" })),
        &WebConfig::default(),
    )
    .await;
...
async fn json_mode_sets_parse_error_on_bad_json() {
    ...
    let out = execute_fetch(
        payload(json!({ "url": format!("{}/bad", server.uri()), "response_format": "json" })),
        &WebConfig::default(),
    )
    .await;
...
async fn byte_cap_truncates() {
    ...
    let out = execute_fetch(
        payload(json!({ "url": format!("{}/big", server.uri()), "max_bytes": 10 })),
        &WebConfig::default(),
    )
    .await;
...
async fn redirect_followed_and_chain_recorded() {
    ...
    let out = execute_fetch(
        payload(json!({ "url": format!("{}/from", server.uri()) })),
        &WebConfig::default(),
    )
    .await;
...
async fn too_many_redirects() {
    ...
    let out = execute_fetch(
        payload(json!({ "url": format!("{}/loop", server.uri()) })),
        &cfg,
    )
    .await;
...
async fn three_xx_without_location_returns_as_success() {
    ...
    let out = execute_fetch(
        payload(json!({ "url": format!("{}/nolctn", server.uri()) })),
        &WebConfig::default(),
    )
    .await;
    assert_eq!(out["ok"], true);
...
async fn page_mode_markdown_transforms_html() {
    ...
    let out = execute_fetch(
        payload(json!({ "url": format!("{}/page", server.uri()), "format": "markdown" })),
        &WebConfig::default(),
    )
    .await;
...
async fn image_envelope_deserializes_to_content_blocks() {
    ...
    let out = execute_fetch(
        payload(json!({ "url": format!("{}/img", server.uri()), "format": "markdown" })),
        &WebConfig::default(),
    )
    .await;
...
async fn blocked_host_for_metadata_ip() {
    let out = execute_fetch(
        payload(json!({ "url": "http://169.254.169.254/latest/meta-data/" })),
        &WebConfig::default(),
    )
    .await;
