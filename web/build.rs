//! Build the web worker's injectable console assets and forward TARGET.
...
fn main() {
    ...
    if std::env::var_os("SKIP_UI_BUILD").is_some() {
        for asset in &assets {
            assert!(
                asset.exists(),
                "SKIP_UI_BUILD set but {} is missing; build web/ui first",
                asset.display()
            );
        }
        return;
    ...
    let build = Command::new(&pnpm)
        ...
        .current_dir(&ui_dir)
        .status()
        .unwrap_or_else(|e| panic!("failed to build web UI in {}: {e}", ui_dir.display()));
    assert!(build.success(), "web UI build failed with {build}");
    for asset in &assets {
        assert!(
            asset.exists(),
...
fn locate_pnpm() -> PathBuf {
    ...
    panic!("pnpm not found on PATH; set PNPM or build web/ui manually");
}
