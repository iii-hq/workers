...
#[derive(Parser, Debug)]
#[command(name = "iii-sandbox-daytona")]
struct Cli {
    /// Path to config.yaml.
...
async fn main() -> Result<()> {
    ...
    let client = Arc::new(DaytonaClient::new(cfg.api_base.clone(), api_key));
    ...
    tracing::info!("sandbox-daytona registered, awaiting invocations");
