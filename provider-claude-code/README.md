...
whatever its size (Anthropic applies its per-model token minimum over the
whole prefix and silently skips a short one), so the per-session tail no
longer invalidates the shared entry. That boundary
