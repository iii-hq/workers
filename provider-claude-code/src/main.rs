//! `provider-claude-code` binary entry.
//!
...
use provider_claude_code::register::register_provider;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.manifest {
        println!(
            "{}",
            serde_json::to_string_pretty(&provider_claude_code::manifest::build_manifest())?
        );
        return Ok(());
    }
    ...
    if let Ok(contents) = std::fs::read_to_string(&cli.config) {
        if has_config_keys(&contents) {
            tracing::warn!(
                path = %cli.config,
                "provider-claude-code takes no file-based config; configure the provider \
    ...
    tracing::info!(url = %cli.url, "provider-claude-code registered");
