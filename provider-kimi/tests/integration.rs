...
// kimi-k2-0905-preview: known family (metadata). moonshot-v1-128k: known.
// kimi-future-9000: unknown kimi family (conservative defaults). The embedding
// id and the non-kimi gpt id must both be filtered out by the chat-family rule.
const STUB_MODELS: &str = "HTTP/1.1 200 OK\r\ncontent-type: application/json\r\nconnection: close\r\n\r\n{\"data\":[{\"id\":\"kimi-k2-0905-preview\",\"object\":\"model\"},{\"id\":\"moonshot-v1-128k\",\"object\":\"model\"},{\"id\":\"kimi-future-9000\",\"object\":\"model\"},{\"id\":\"moonshot-embedding-v1\",\"object\":\"model\"},{\"id\":\"gpt-5.2\",\"object\":\"model\"}]}";
...
async fn boot_stack(engine_url: &str) -> (IIIClient, IIIClient) {
    ...
    loop {
        ...
        let registered = list["providers"]
            .as_array()
            .is_some_and(|p| p.iter().any(|x| x["id"] == "kimi"));
        if registered {
            break;
        }
        assert!(
...
async fn refresh_and_wait(router_iii: &IIIClient, provider_iii: &IIIClient, expect_id: &str) {
    let res = call(provider_iii, "provider::kimi::refresh_models", json!({}))
        .await
...
async fn chat_streams_end_to_end_with_cost_fill() {
    ...
    refresh_and_wait(&router_iii, &provider_iii, "kimi-k2-0905-preview").await;
...
async fn refresh_models_reconciles_filtered_live_catalog() {
    ...
    // exactly the filtered live list: kimi/moonshot chat families stay,
    // embedding and non-kimi ids are gone.
    assert!(ids.contains(&"kimi-k2-0905-preview"), "got {ids:?}");
    assert!(ids.contains(&"moonshot-v1-128k"), "got {ids:?}");
    assert!(
        ids.contains(&"kimi-future-9000"),
        "unknown kimi family still routes: {ids:?}"
    );
    assert!(
        !ids.contains(&"moonshot-embedding-v1"),
        "embedding model should be filtered: {ids:?}"
    );
    assert!(
        !ids.contains(&"gpt-5.2"),
        "non-kimi model should be filtered: {ids:?}"
    );
