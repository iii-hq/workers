...
/// Build the Chat Completions `messages` array. The hard requirement Moonshot
/// enforces (more strictly than OpenAI): an assistant message with `tool_calls`
/// MUST be immediately followed by one `role:"tool"` message per `tool_call_id`.
///
    ...
    while i < n {
        match &messages[i] {
            ...
            AgentMessage::Assistant(a) => {
                ...
                // Moonshot rejects an assistant message with neither content nor
...
mod tests {
    ...
    fn empty_assistant_turn_is_omitted() {
        // thinking-only turn → no content, no tool_calls → Moonshot would
    ...
    fn reused_tool_call_id_pairs_positionally_not_globally() {
        ...
        // dangling → Moonshot rejects. Positional pairing must give the second
