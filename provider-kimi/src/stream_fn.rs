//! The `provider::kimi::stream` iii function (spec § Provider stream
...
async fn run_stream_call(
        ...
        Err(e) => {
            ...
            let _ = send_event(
                ...
                &synthetic_error_event(
                    &llm_router::provider_scaffold::errors::public_error("kimi", kind),
                    &model,
                    kind,
                ),
            );
    ...
    // Moonshot has no reasoning_effort knob — thinking is implicit per model
    // (Kimi K2 Thinking / kimi-thinking-preview) and streamed back as
    // `reasoning_content` (see sse.rs). thinking_level is therefore advisory
    // here; the model alone decides whether it thinks.

    // Moonshot supports only json_object mode, not strict json_schema. A
    // requested schema is mapped to json_object (request.rs) and reported here.
    if input
        .response_format
        .as_ref()
    ...
    {
        warnings.push(
            "response_format json_schema degraded to json_object: Moonshot supports json_object mode only"
                .to_string(),
        );
    }
