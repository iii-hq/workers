...
export function ConsolePage() {
  return (
    ...
    >
      <SequencePlayer
        title="console ↔ the stack — one governed turn"
        lanes={LANES}
        steps={STEPS}
        width={940}
      />
      ...
      <div className="grid grid-cols-1 @2xl:grid-cols-3 gap-4">
        <Cell title="live catalogs" bodyClassName="max-w-none">
          the model picker reads the router's capability catalog; the composer's
          @-mentions search every function registered on the engine — both
          update as workers come and go. the ui never ships a hardcoded list.
        </Cell>
        <Cell title="approvals where you read" bodyClassName="max-w-none">
          held calls appear as cards in the transcript with their arguments,
          grouped when consecutive. approve and deny are one click — the
          parked turn resumes instantly.
        </Cell>
        <Cell title="context, visible" bodyClassName="max-w-none">
          a usage meter tracks the window with warn and danger thresholds and
          nudges compaction — the same budgeting the loop runs is legible to
          the operator.
        </Cell>
      </div>
        ...
        <SpecSheet title="what the console binds" meta="reactive surface" defaultOpen>
          <div className="flex flex-col">
            ...
            <SpecRow name="harness::turn_completed" type="outcomes">
              toasts on failures, auto-titling after first turns, anything
              that reacts to results rather than text.
            </SpecRow>
          </div>
        </SpecSheet>
        ...
        <SpecSheet title="observability — every hop is a span" meta="opentelemetry">
          <div className="flex flex-col">
            ...
            <SpecRow name="metadata propagates" type="end-to-end">
              consumer-supplied tracing metadata rides through every nested
              call, so one conversation stitches into one trace.
            </SpecRow>
          </div>
        </SpecSheet>
      </div>
