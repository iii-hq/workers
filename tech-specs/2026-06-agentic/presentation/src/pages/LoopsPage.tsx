    ...
    >
      ...
      <div className="grid grid-cols-1 @4xl:grid-cols-2 gap-4 items-start">
        <CodeBlock title="the entire loop — one worker file">
          iii.<K>registerFunction</K>(<S>"reports::daily"</S>, <K>async</K> ()
          {' '}
            <M>{'=>'}</M> <M>{'{'}</M>
          {'\n'}
          {'  '}
          ...
          {'        '}functions: <M>{'{'}</M>
          {'\n'}
          {'          '}allow: [<S>"database::*"</S>, <S>"search::web"</S>,{' '}
          <S>"harness::spawn"</S>]{'\n'}
          {'        '}
          <M>{'}'}</M>
          {'\n'}
          ...
          {'\n\n'}
          {'  '}
          <K>await</K> iii.<K>trigger</K>(<M>{'{'}</M> function_id:{' '}
          <S>"email::send"</S>, payload: render(run.result) <M>{'}'}</M>);
          {'\n'}
          <M>{'}'}</M>);
          {'\n\n'}
          ...
          {'  '}config: <M>{'{'}</M> expression: <S>"0 0 9 * * *"</S>{' '}
          <M>{'}'}</M>,{'\n'}
          <M>{'}'}</M>);
        </CodeBlock>
          ...
          <SpecSheet title="the output contract" meta="typed deliverables" defaultOpen>
            <div className="flex flex-col">
              ...
              <SpecRow name='{ type: "json", schema }' type="automation grade">
                the turn must produce json matching the schema. delivered
                provider-natively when the model supports structured output,
                or through a synthetic submit_result call when it does not —
                callers never see the difference.
              </SpecRow>
              <SpecRow name="validation retries" type="bounded">
                a miss appends the validation errors as a nudge and regenerates
                — a bounded number of times, then an explicit result_error.
              </SpecRow>
            </div>
          </SpecSheet>
          ...
          <SpecSheet title="chains that know when to stop" meta="turn_completed">
            <div className="flex flex-col">
              <SpecRow name="event-driven chains" type="completed → send → …">
                bind harness::turn_completed and start the next turn from the
                handler — the supported way to build multi-stage pipelines.
              </SpecRow>
              <SpecRow name="termination is yours" type="by design">
                max_turns bounds one turn, not a chain. carry a hop counter in
                session metadata or a budget sibling — the spec is explicit
                about who owns the brake.
              </SpecRow>
            </div>
          </SpecSheet>
