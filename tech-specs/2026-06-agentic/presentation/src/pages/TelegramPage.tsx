import { SequencePlayer, type SeqLane, type SeqStep } from '@/components/diagrams/SequencePlayer'
...
export function TelegramPage() {
  return (
    ...
    >
      <SequencePlayer
        title="telegram ↔ the stack — one conversation"
        lanes={LANES}
        steps={STEPS}
        width={1000}
      />
      ...
      <div className="grid grid-cols-1 @4xl:grid-cols-2 gap-4 items-start">
        <CodeBlock title="the whole reactive render path">
          ...
          {'\n'}
          iii.<K>registerFunction</K>(<S>"tg::on-message-updated"</S>,{' '}
          <K>async</K> (evt) <M>{'=>'}</M> <M>{'{'}</M>
          {'\n'}
          {'  '}
          <K>const</K> chatId = <K>await</K> chatIdForSession(evt.session_id);
          {'\n'}
          {'  '}
          <K>if</K> (!shouldEdit(evt.revision)) <K>return</K>;{' '}
          <C>// ~1/s throttle</C>
          {'\n'}
          {'  '}
        ...
        <div className="flex flex-col gap-4">
          <Cell title="what the worker owns" bodyClassName="max-w-none">
            telegram ux only: the command fsm, the inline keyboard, the chat ↔
            session map, edit throttling. roughly a hundred lines — everything
            else is the stack's job.
          </Cell>
          <Cell title="what it never reimplements" bodyClassName="max-w-none">
            streaming, retries, context budgeting, provider routing, crash
            recovery, cancellation semantics, model catalogs. five workers
            already solved those — for every consumer at once.
          </Cell>
        </div>
      </div>
        ...
        <SpecSheet title="worker surface — functions & bindings" meta="2 + 4">
          <div className="flex flex-col">
            ...
            <SpecRow name="harness::turn_completed → tg::on-turn-completed" type="binding">
              drains the local queue, or posts a single final message when
              live edits are not wanted.
            </SpecRow>
          </div>
        </SpecSheet>
          ...
          <SpecSheet title="steering vs strict fifo" meta="a product choice">
            <div className="flex flex-col">
              <SpecRow name="steering (default)" type="harness-native">
                messages sent mid-turn fold into the running turn — the model
                treats them as added context. zero extra worker code.
              </SpecRow>
              <SpecRow name="strict fifo" type="worker-local queue">
                one question, one answer: the worker checks harness::status
                and queues locally, draining on turn_completed. the stack
                supports both; the product decides.
              </SpecRow>
            </div>
          </SpecSheet>
