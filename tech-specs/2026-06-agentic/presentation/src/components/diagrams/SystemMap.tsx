...
const EDGES: MapEdge[] = [
  ...
  { id: 'run', from: 'third-party', to: 'harness', d: 'M 210 326 C 300 318, 350 250, 425 238', label: 'harness::run', lx: 296, ly: 288, dur: 2.2 },
  { id: 'direct', from: 'third-party', to: 'llm-router', d: 'M 210 352 C 460 450, 720 410, 798 212', label: 'router::chat — no loop needed', lx: 560, ly: 428, dur: 3 },
  { id: 'spawn', from: 'harness', to: 'harness', d: 'M 520 168 C 520 118, 620 118, 620 168', label: 'harness::spawn → child sessions', lx: 575, ly: 110, dur: 2.4 },
]
...
export function SystemMap({ selected, onSelect }: SystemMapProps) {
  ...
  return (
    ...
    >
      <defs>
        <marker id="arr-faint" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
          <path d="M 0 0 L 8 4 L 0 8 z" className="fill-ink-ghost" />
        </marker>
        <marker id="arr-accent" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
      ...
      {NODES.map((node) => {
        ...
        return (
          ...
          >
            ...
            />
            {isSelected ? (
              <rect x={node.x} y={node.y} width={3} height={node.h} className="fill-accent" />
            ) : null}
            <text
              x={node.x + node.w / 2}
              y={node.y + node.h / 2 - 4}
              ...
              fontWeight={600}
              className={cn(
                isSelected || isConnected || node.kind === 'core' ? 'fill-ink' : 'fill-ink-faint',
              )}
            >
              {node.title}
            </text>
...
export function MapDatasheet({
  ...
  return (
    <aside
      className={cn(
        'border border-rule bg-bg flex flex-col min-w-0 min-h-0 overflow-hidden',
        className,
      )}
    >
          ...
          <ul className="flex flex-col gap-y-2.5">
            {info.functions.map((fn) => (
              <li key={fn.id} className="min-w-0">
                <FnChip tone="ink" className="max-w-full overflow-hidden">{fn.id}</FnChip>
