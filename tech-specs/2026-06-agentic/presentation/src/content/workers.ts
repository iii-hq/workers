...
export const WORKERS: Record<string, WorkerInfo> = {
  ...
  'llm-router': {
    ...
    functions: [
      ...
      { id: 'router::models::list / get / supports', desc: 'the live capability catalog, populated by provider discovery.' },
      { id: 'router::provider::register / resolve', desc: 'providers self-declare; credentials resolve centrally per request.' },
    ],
    emits: [],
    notes: [
  ...
  'approval-gate': {
    ...
    notes: [
      'the inbox is ephemeral: records exist only while a call is held. the transcript is the audit trail.',
    ],
  },
  chat: {
    id: 'chat',
  ...
  'telegram-bot': {
    ...
    standalone:
      'same triangle as the chat, different surface — webhooks dedupe with an idempotency key.',
    functions: [
  ...
  'third-party': {
    ...
    standalone:
      'each core worker is independently useful; a consumer composes exactly the subset it needs.',
    functions: [
  ...
  substrate: {
    ...
    notes: [
      'fail-closed: with no allow-list, every call is refused. deployments opt functions in deliberately.',
    ],
  },
}
