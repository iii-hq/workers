          ...
          {INSTALL_LINES.slice(0, count).map((line, i) =>
            line.kind === 'cmd' ? (
              <div key={i} className="flex items-center gap-x-2">
                ...
                {line.comment ? (
                  <span className="text-ink-ghost text-[12px]">{line.comment}</span>
                ) : null}
              </div>
            ) : (
      ...
      <div className="grid grid-cols-1 @4xl:grid-cols-2 gap-4 items-start">
        ...
        <div className="flex flex-col gap-4 min-w-0">
          <CodeBlock title="step 2 — send (any language, any worker)">
            ...
            {'    '}options: <M>{'{'}</M> functions: <M>{'{'}</M> allow: [
            <S>"shell::*"</S>] <M>{'}'}</M> <M>{'}'}</M>
            {'\n'}
            {'  '}
            <M>{'}'}</M>
      ...
      <div className="mt-6 grid grid-cols-1 @2xl:grid-cols-3 gap-px bg-rule border border-rule">
        {[
          ...
          <div key={cell.title} className="bg-bg p-6">
            <div className="font-mono text-[15px] font-semibold lowercase text-ink mb-2">
              {cell.title}
            </div>
            <p className="font-mono text-[12.5px] leading-[1.7] text-ink-faint lowercase">
              {cell.body}
            </p>
          </div>
        ))}
