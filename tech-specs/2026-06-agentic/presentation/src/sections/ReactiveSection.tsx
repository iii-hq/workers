import { EventFanOut } from '@/components/diagrams/EventFanOut'
          ...
          <div className="border border-rule bg-bg">
            ...
            <div className="px-4 py-4">
              ...
              <p className="mt-4 font-mono text-[12px] leading-[1.65] text-ink-faint lowercase max-w-[58ch]">
                every one of these is a trigger type a function can be bound
                to — and that function can call{' '}
                <span className="text-ink">harness::run</span> with a goal. a
                webhook, a schedule, a state change, or another agent
                finishing: any event in the system can set an agent in motion
                toward an outcome.
              </p>
            </div>
          </div>
          ...
          <SpecSheet title="events vs hooks — know or act" meta="two surfaces">
            <div className="flex flex-col">
              <SpecRow name="need to know? bind an event" type="async, observe-only">
                turn_started / turn_completed and the six session events fire
                at the boundaries — cheap, unordered, at-least-once;
                reconciled by revision.
              </SpecRow>
              ...
              <SpecRow name="hooks are operator code" type="never model-reachable">
                bindings are a worker-plane surface with per-binding timeout
                and an explicit failure policy — a dead hook can never brick
                the loop silently.
              </SpecRow>
            </div>
          </SpecSheet>
