...
export function SubstrateSection() {
  ...
  const allowedCount = useMemo(
    () => REGISTRY.filter((fn) => isAllowed(fn, policy)).length,
    [policy],
  )
      ...
      <div className="grid grid-cols-1 @4xl:grid-cols-[minmax(0,5fr)_minmax(0,7fr)] gap-6 items-start">
        <div className="flex flex-col gap-4 min-w-0">
          <CodeBlock title="the model sees one surface">
            ...
            {'\n'}
            <K>agent_trigger</K> <M>{'{'}</M> function:{' '}
            <S>"engine::functions::list"</S> <M>{'}'}</M>
          </CodeBlock>
          <StatusPanel
            variant="info"
        ...
        <SpecSheet title="exposure modes" meta="per turn">
          <div className="flex flex-col">
            <SpecRow name='expose: "agent_trigger"' type="default">
              one generic schema, zero per-function token cost. discovery
              happens at runtime through the registry — right for broad,
              exploratory agents.
            </SpecRow>
            <SpecRow name='expose: "native"' type="narrow agents">
              the harness expands the allow globs against the registry at turn
              start and attaches one concrete schema per function — models
              follow exact schemas more reliably, the right mode for sub-agents
              with small fixed surfaces.
            </SpecRow>
        ...
        <SpecSheet title="provenance — calls cannot be laundered" meta="security">
          <div className="flex flex-col">
            <SpecRow name="agent provenance" type="propagated">
              every call made on behalf of a model carries a mark, and the
              engine propagates it through nested calls — a function the agent
              reaches cannot re-trigger its way around policy.
            </SpecRow>
            <SpecRow name="permission rules" type="deployment-level">
              rules apply to any call whose chain carries the mark;
              worker-initiated and human-initiated calls pass untouched.
            </SpecRow>
            <SpecRow name="sub-agents inherit" type="narrow, never escalate">
              a child's policy is the parent's intersected with the request —
              depth, fan-out, and turn budgets bound the whole tree.
            </SpecRow>
          </div>
        </SpecSheet>
      </div>
