...
const LEGEND = [
  ...
  { swatch: <span className="inline-block size-3 border border-dashed border-ink-faint bg-bg" />, label: 'optional sibling' },
        ...
        <SpecSheet title="the wiring, in words" meta="8 edges">
          <div className="flex flex-col">
            ...
            <SpecRow name="harness → session::append / update_message" type="persist + stream">
              the loop writes every delta into the session store; each write
              emits an event consumers already listen to.
            </SpecRow>
            <SpecRow name="harness → context::assemble" type="fit the window">
              raw history in, model-ready context out — pruned, compacted,
              budgeted for the exact target model.
            </SpecRow>
            <SpecRow name="harness → router::chat" type="generate">
              one front door for every provider; the stream relays back over a
              channel frame by frame.
            </SpecRow>
            <SpecRow name="harness → agent_trigger" type="dispatch">
              the model asks for work; the harness runs it against any allowed
              function on the bus — fail-closed.
            </SpecRow>
            <SpecRow name="approval-gate ⇆ harness" type="hold / release">
              a held call parks the turn; a human decision releases or answers
              it. the loop never changes.
            </SpecRow>
            <SpecRow name="harness → harness::spawn" type="sub-agents">
              children are ordinary sessions on the same queue — parallel by
              construction.
            </SpecRow>
            <SpecRow name="session-manager → consumers" type="reactive surface">
              session created, message added / updated, status changed — bind
              once, render live everywhere.
            </SpecRow>
          </div>
        </SpecSheet>
        ...
        <SpecSheet title="design principles" meta="from the spec">
          <div className="flex flex-col">
            <SpecRow name="standalone first">
              every core worker is independently installable and has a coherent
              purpose by itself. cross-worker calls are explicit bus calls,
              never imports.
            </SpecRow>
            <SpecRow name="the harness is thin">
              it sequences the other three plus function dispatch. anything
              that grows real logic — approvals, budgets, scheduling — becomes
              a sibling, never harness code.
            </SpecRow>
            <SpecRow name="llm-router is consumer-agnostic">
              it never assumes a loop, a session, or a ui. it streams into a
              caller-supplied channel and returns.
            </SpecRow>
            <SpecRow name="context-manager owns no storage">
              message arrays in, results out; the caller persists. reusable by
              any harness or ai feature.
            </SpecRow>
            <SpecRow name="session-manager is the single reactive surface">
              consumers bind its trigger types and render live — they never
              chase status and never need to know about the provider or the
              loop.
            </SpecRow>
          </div>
        </SpecSheet>
