        ...
        <SpecSheet title="steering — messages mid-turn" meta="concurrency">
          <div className="flex flex-col">
            <SpecRow name="one turn per session" type="atomic check-and-set">
              two concurrent sends create exactly one turn — the loser of the
              race merges instead of forking.
            </SpecRow>
            <SpecRow name="merged sends fold in" type="watermark">
              a message that lands mid-turn is appended and picked up by the
              running loop's steering check — the model sees it in the same
              turn, like a colleague adding context.
            </SpecRow>
            <SpecRow name="never silently dropped" type="double-check">
              the merge path re-reads the turn record after appending; if the
              turn just ended, a fresh turn starts for the message.
            </SpecRow>
          </div>
        </SpecSheet>
        ...
        <SpecSheet title="the streaming contract" meta="typed frames">
          <div className="flex flex-col">
            <SpecRow name="AssistantMessageEvent" type="discriminated union">
              start, text_delta, thinking_delta, functioncall_delta, usage,
              stop, done, error — providers stream one shape, every consumer
              reads one shape.
            </SpecRow>
            <SpecRow name="partial accumulator" type="on every frame">
              non-terminal frames carry the assembled message so far — a ui
              can render from any frame it joins on.
            </SpecRow>
            <SpecRow name="revision numbers" type="last-write-wins">
              events echo a revision per entry; consumers reconcile snapshots
              in any arrival order.
            </SpecRow>
            <SpecRow name="usage + cost" type="per stream">
              token splits (including cache reads) and usd cost are attached
              by the router from its catalog pricing.
            </SpecRow>
          </div>
        </SpecSheet>
      </div>
