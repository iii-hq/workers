      ...
      <div className="flex flex-wrap items-center gap-y-2 border border-rule bg-bg px-3.5 py-2.5 mb-4">
        {FLOW.map((stage, i) => (
          <span key={stage.id} className="flex items-center">
            ...
            {i < FLOW.length - 1 ? (
              <span aria-hidden className="mx-2 h-px w-4 bg-rule inline-block" />
            ) : null}
          </span>
        ))}
      </div>
        ...
        <div className="border border-rule bg-bg flex flex-col">
          ...
          <div className="px-4 pb-4 mt-auto flex flex-col gap-y-1.5">
            <div className="font-mono text-[11.5px] text-ink-faint lowercase">
              <span className="text-ink">policy:</span> allow ["shell::*"] —
              matched
            </div>
            <div className="font-mono text-[11.5px] text-ink-faint lowercase">
              <span className="text-ink">hook:</span> approval::gate →{' '}
              <span className="text-warn">hold</span> — destructive command,
              needs a human
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div className="px-4 py-3.5 flex flex-col gap-y-2">
            ...
            <div className="font-mono text-[11px] text-ink-ghost lowercase">
              expires in 30 min — unresolved holds settle as errors, never
              park forever
            </div>
          </div>
        ...
        <div className="border border-rule bg-bg flex flex-col">
          ...
          {decision === 'pending' ? (
            <div className="px-4 py-3.5 flex flex-col gap-y-2.5">
              <div className="font-mono text-[12.5px] text-ink lowercase">
                turn t_004 — parked
              </div>
              <p className="font-mono text-[11.5px] leading-[1.65] text-ink-faint lowercase">
                the call checkpointed as pending and the step ended cleanly. no
                queue entry is held open, nothing times out, nothing blocks —
                durable execution makes waiting free, whether the human answers
                in seconds or tomorrow.
              </p>
            </div>
          ) : decision === 'approved' ? (
            <div className="px-4 py-3.5 flex flex-col gap-y-2.5">
              <div className="font-mono text-[12.5px] text-accent lowercase">
                released — the harness executes
              </div>
              <p className="font-mono text-[11.5px] leading-[1.65] text-ink-faint lowercase">
                approval::resolve releases the call back into the dispatch
                pipeline: remaining hooks run, the function executes with the
                original call's provenance, the result is appended, and the
                turn re-enqueues. the approver never re-implements dispatch.
              </p>
            ...
            <div className="px-4 py-3.5 flex flex-col gap-y-2.5">
              ...
              <p className="font-mono text-[11.5px] leading-[1.65] text-ink-faint lowercase">
                the denial is appended as the call's result under the same
                deterministic id an execution would have used. the model reads
                the reason in-transcript and proposes another way — no crash,
                no dangling state.
              </p>
              <pre className="font-mono text-[11px] leading-[1.55] text-ink-faint border border-rule-2 px-3 py-2 overflow-x-auto">
        ...
        <SpecSheet title="why this shape wins" meta="policy as a sibling">
          <div className="flex flex-col">
            <SpecRow name="the loop ships mechanics only" type="hold + resolve">
              the harness knows how to park and resume; it never knows who may
              approve what. swap the policy worker without touching the loop.
            </SpecRow>
            <SpecRow name="notify anywhere" type="reactive">
              approval::pending_created is a trigger type — bind a function
              and route holds to a console inbox, a chat channel, a pager.
            </SpecRow>
            <SpecRow name="per-session modes" type="manual / auto / full">
              tighten or relax per conversation; standing "approve always"
              grants are honoured in every mode.
            </SpecRow>
            <SpecRow name="crashed gate ≠ open gate" type="fail closed">
              a hook that dies or times out resolves by its failure policy —
              for approval that means deny, never wave-through.
            </SpecRow>
          </div>
        </SpecSheet>
        ...
        <SpecSheet title="the audit story" meta="who did what">
          <div className="flex flex-col">
            <SpecRow name="the transcript is the trail" type="function_result">
              every held call ends as a result entry — approved, denied, or
              expired — attributable to its turn and session.
            </SpecRow>
            <SpecRow name="hook mutations annotate" type="origin metadata">
              rewrites at the hooks can attach annotations to the entries they
              touch, so audits see what actually ran.
            </SpecRow>
            <SpecRow name="bindings are enumerable" type="engine registry">
              the effective hook set is a registry read — no hidden
              middleware, no config drift.
            </SpecRow>
          </div>
        </SpecSheet>
      </div>
