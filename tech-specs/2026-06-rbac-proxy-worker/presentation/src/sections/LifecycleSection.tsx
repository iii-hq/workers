...
export function LifecycleSection() {
  return (
    ...
    >
      <SequencePlayer
        title="one connection, upgrade to rewritten result"
        lanes={SEQ_LANES}
        steps={SEQ_STEPS}
      />
        ...
        <SpecSheet title="what the proxy preserves" meta="wire fidelity">
          <div className="flex flex-col">
            <SpecRow name="invocation_id" type="byte-for-byte">
              echoed through every rewrite, or the sdk pending-call map never
              resolves.
            </SpecRow>
            ...
            <SpecRow name="unrecognised frames" type="pass-through">
              anything the proxy cannot classify is forwarded as-is; protocol
              additions degrade to relay, never breakage.
            </SpecRow>
          </div>
        </SpecSheet>
        ...
        <SpecSheet title="on a denied call" meta="no upstream hop">
          <div className="flex flex-col">
            <SpecRow name="synthesized result" type="InvocationResult">
              the proxy answers FORBIDDEN itself; the frame never reaches the
              engine.
            </SpecRow>
            <SpecRow name="void actions" type="fabricated id">
              a denied void call still gets a reply with a fabricated
              invocation_id, matching the engine deny path exactly.
            </SpecRow>
            <SpecRow name="engine:: calls" type="bypass middleware">
              connection setup, channels, and the discovery overrides are never
