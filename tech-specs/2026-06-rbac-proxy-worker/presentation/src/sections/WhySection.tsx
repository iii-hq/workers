        ...
        {PAIN_CARDS.map((card) => (
          <div key={card.n} className="bg-bg p-6 @3xl:p-7 min-w-0">
            <div className="flex items-baseline gap-x-2.5 mb-3">
              <span className="font-mono text-[11px] text-alert tabular-nums">
                {card.n}
              </span>
              <span className="font-mono text-[15px] font-semibold text-ink lowercase">
                {card.title}
              </span>
            </div>
            <p className="font-mono text-[13px] leading-[1.7] text-ink-faint lowercase max-w-[46ch]">
              {card.body}
            </p>
          </div>
        ))}
      ...
      <div className="mt-8">
        ...
        >
          <div className="overflow-x-auto">
            <div className="min-w-[560px] flex flex-col">
              ...
              {DISCOVERY_LEAK.map((row) => (
                ...
                >
                  <FnChip tone={row.sessionAware ? 'ink' : 'alert'}>
                    {row.fn}
                  </FnChip>
                  <span
                    className={`font-mono text-[12px] ${
                      row.sessionAware ? 'text-accent' : 'text-alert'
                    }`}
                  >
          ...
          <p className="mt-4 font-mono text-[12px] leading-[1.7] text-ink-faint lowercase">
            and even the two session-aware functions filter against an in-process
            session a remote proxy never receives. so a gated worker can read
            schemas, owners, and host metadata for functions it can never invoke.
          </p>
        </SpecSheet>
      </div>
    </Section>
