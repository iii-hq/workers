...
import { C, CodeBlock, M, S } from '@/components/schematic/CodeBlock'
...
export function AccessSection() {
  ...
  const profile =
    ACCESS_PROFILES.find((p) => p.id === profileId) ?? ACCESS_PROFILES[0]

  const verdicts = useMemo(
    () => ACCESS_CATALOG.map((fn) => ({ fn, v: resolveAccess(fn, profile) })),
    [profile],
  )
  const callable = verdicts.filter((x) => x.v.allowed).length
  const pickedVerdict = picked
    ? verdicts.find((x) => x.fn.id === picked)
    : undefined
      ...
      <div className="border border-rule bg-bg">
        <div className="flex flex-wrap items-center justify-between gap-3 bg-panel px-3.5 py-2.5 border-b border-rule">
          ...
          <span className="font-mono text-[12px] uppercase tracking-[0.06em] text-ink-faint">
            <span className="text-accent text-[15px] font-semibold tabular-nums">
              {callable}
            </span>{' '}
            ...
            return (
              ...
              >
                <FnChip
                  tone={tone}
                  className={
                    v.allowed ? '' : 'line-through opacity-55 decoration-alert/70'
                  }
                >
                  {fn.id}
                </FnChip>
        ...
        <div className="border-t border-rule px-4 py-3 min-h-[58px] flex items-center">
          {pickedVerdict ? (
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
              <FnChip tone={pickedVerdict.v.allowed ? 'accent' : 'alert'}>
                {pickedVerdict.fn.id}
              </FnChip>
              <span
        ...
        <SpecSheet title="the resolution order" meta="forbidden wins" defaultOpen>
          <div className="flex flex-col">
            <SpecRow name="1 · forbidden_functions" type="deny">
              the hard deny. a per-user or per-role denylist expose cannot
              override.
            </SpecRow>
            ...
            <SpecRow name="5 · otherwise" type="deny">
              an empty expose is not deny-everything; it is allow only the carve-out
              plus whatever the auth result allowed.
            </SpecRow>
          </div>
        </SpecSheet>
      </div>
