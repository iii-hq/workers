...
import { C, CodeBlock, K, M, S } from '@/components/schematic/CodeBlock'
        ...
        <div className="border border-rule bg-bg">
          ...
          <div className="flex flex-col">
            {FAIL_RULES.map((r) => (
              <div
                key={r.fn}
                className="px-4 py-3.5 border-b border-rule-2 last:border-b-0"
              >
                <div className="flex flex-wrap items-baseline gap-x-2">
                  <span className="font-mono text-[13px] font-semibold text-ink">
                    {r.fn}
                  </span>
                  <span className="font-mono text-[11.5px] text-ink-ghost lowercase">
                    {r.when}
                  </span>
                </div>
                <div className="mt-1 font-mono text-[12.5px] leading-[1.6] text-alert lowercase">
                  {r.then}
                </div>
              </div>
            ))}
            <div className="px-4 py-3.5 font-mono text-[12px] leading-[1.7] text-ink-faint lowercase">
              the only safe default for a security boundary: a broken policy
              function denies, it does not open the door.
            </div>
          </div>
        </div>
      </div>
