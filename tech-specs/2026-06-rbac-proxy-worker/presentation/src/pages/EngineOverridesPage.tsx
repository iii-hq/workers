...
import { C, CodeBlock, K, M } from '@/components/schematic/CodeBlock'
...
export function EngineOverridesPage() {
  return (
    ...
    >
      <StepReveal
        title="engine::workers::list, filtered client-side"
          ...
          <div className="min-w-[680px] flex flex-col">
            ...
            {OVERRIDE_TABLE.map((row) => (
              ...
              >
                ...
                <span className="font-mono text-[12px] leading-[1.5] text-ink-faint lowercase">
                  {row.rewrite}
                </span>
                <span className="font-mono text-[12px] leading-[1.5] text-ink-ghost lowercase">
                  {row.policy}
                </span>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 @4xl:grid-cols-2 gap-4">
        <CodeBlock title="the two caches, over the control connection">
          ...
          {'\n'}
          <K>registered_trigger_id</K> <M>→</M> <M>{'{'}</M> trigger_type, function_id{' '}
          <M>{'}'}</M>
          {'\n'}
        ...
        <SpecSheet title="cache-miss semantics" meta="not uniformly fail-closed" defaultOpen>
          <div className="flex flex-col">
            <SpecRow name="wildcard filter" type="allow on cold cache">
              match(&quot;api::*&quot;) needs only the id string, never the catalog,
              so a never-seen api::new is correctly allowed on a miss.
            </SpecRow>
            <SpecRow name="metadata filter" type="fail closed on miss">
              needs the cached metadata, so a freshly-registered metadata-gated
              function is briefly invisible until the catalog refreshes, never
              wrongly exposed.
            </SpecRow>
            <SpecRow name="refresh" type="ttl + trigger">
              lazy on a short ttl, proactively on engine::functions-available,
              mirroring console&apos;s function-list cache.
            </SpecRow>
          </div>
        </SpecSheet>
      </div>
      ...
      <SpecSheet title="worker-internals leak policy" meta="expose_worker_internals">
        <div className="flex flex-col">
          <SpecRow name="false (default)" type="strip">
            drop ip_address + isolation from WorkerSummary, and additionally pid +
            internal + latest_metrics from WorkerDetailEnvelope. the caller still
            sees name, version, status, recomputed function_count, connected_at_ms.
          </SpecRow>
          <SpecRow name="true" type="pass through">
            single-tenant or operator-trusted deployments where the full picture
            is wanted.
          </SpecRow>
          <SpecRow name="per-session" type="optional">
            key the decision off AuthResult.context (e.g. context.admin === true)
            in a small policy hook instead of the global knob.
          </SpecRow>
        </div>
      </SpecSheet>
      ...
      <SpecSheet title="prefix in results" meta="strip own namespace">
        <p className="font-mono text-[13px] leading-[1.7] text-ink-faint lowercase">
          a session&apos;s own functions live in the registry as {'{prefix}::{id}'}{' '}
          and surface that way in discovery. the worker registered them bare and
          must see them bare, so the proxy strips its own session prefix from every
          id and worker name in a result. a foreign id is shown canonically. this
          is symmetric with the dispatch-path strip: the worker only ever sees foo,
          never tenant1::foo.
        </p>
      </SpecSheet>
    </PageShell>
