  ...
  return (
    ...
    >
        ...
        <div className="border border-rule bg-bg flex flex-col">
          {RESOLUTION_RULES.map((rule) => (
            ...
            >
              ...
              <span className="font-mono text-[13px] text-ink lowercase flex-1 min-w-0">
                {rule.text}
              </span>
              <span
      ...
      <div className="grid grid-cols-1 @4xl:grid-cols-2 gap-4">
        ...
        <SpecSheet title="middleware" meta="its return is the result" defaultOpen>
          ...
          <p className="mt-3 font-mono text-[12px] leading-[1.7] text-ink-faint lowercase">
            whatever the middleware returns is returned to the caller. if it wants
            the target to actually run, it must invoke it itself. engine:: calls
      ...
      <div>
        ...
        <p className="mt-3 font-mono text-[12px] leading-[1.7] text-ink-faint lowercase">
          each hook sees the bare id plus the session context, before prefix
          resolution. it returns mapped fields to allow, or throws to deny.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 @4xl:grid-cols-2 gap-px bg-rule border border-rule">
          {DIVERGENCES.map((d) => (
            <div key={d.title} className="bg-bg p-5 min-w-0">
              <div className="font-mono text-[14px] font-semibold text-ink lowercase mb-2.5">
                {d.title}
              </div>
