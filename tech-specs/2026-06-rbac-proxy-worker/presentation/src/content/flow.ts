...
export const OVERRIDE_STAGES: RevealStage[] = [
  ...
  },
  {
    ...
    tone: 'alert',
    caption: "the engine answers with the full result. it filtered against the proxy's own session, which is unrestricted on the internal listener.",
    rows: [
      { k: 'workers', v: '12' },
  ...
  },
  {
    label: 'filter',
    tone: 'warn',
    caption: 'the proxy runs the same vendored is_function_allowed over every function id in the result: drop zero-access workers, recompute counts, strip the prefix and worker internals.',
    rows: [
  ...
  },
  {
    label: 'downstream',
    tone: 'accent',
    caption: 'the worker receives a result showing exactly the surface it can invoke. discovery and invocation can never disagree.',
    rows: [
      { k: 'workers', v: '3' },
      { k: 'leaked', v: 'none' },
    ],
...
export const OVERRIDE_TABLE: OverrideRow[] = [
  ...
  },
  {
    ...
    rewrite: 'resolve each worker via the catalog cache; drop zero-access workers; recompute function_count; strip internals',
    policy: 'filtered list',
  },
  {
