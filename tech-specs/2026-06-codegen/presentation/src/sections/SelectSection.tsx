    ...
    >
      <div className="flex flex-col gap-5 @3xl:flex-row @3xl:items-center @3xl:justify-between">
        ...
        <div className="flex items-center gap-x-3 font-mono text-[12px] lowercase text-ink-faint">
          <span className="text-ink-ghost uppercase tracking-[0.1em] text-[10px]">
            functions:
          </span>
      ...
      <div className="mt-6 grid grid-cols-1 @lg:grid-cols-2 @4xl:grid-cols-3 gap-px bg-rule border border-rule">
        {CATALOG.map((fn) => {
          ...
          return (
            <div
              key={fn.id}
              className="bg-bg px-4 py-3 flex items-center justify-between gap-x-3 min-w-0"
            >
              <FnChip
                tone={on ? 'ink' : 'ghost'}
                className={on ? '' : 'line-through opacity-50'}
              >
                {fn.id}
              </FnChip>
              <span
      ...
      <p className="mt-5 font-mono text-[12px] leading-[1.6] text-ink-faint lowercase max-w-[72ch]">
        selection is the union of the workers, functions, and triggers lists. an
        empty list contributes nothing; only <span className="text-ink">*</span>{' '}
        means all. engine-internal handlers stay hidden unless a glob names them.
      </p>
    </Section>
  )
}
