/**
 * Settings live in the built-in `configuration` worker under the `claude-cli`
...
const CONFIG_ID = 'claude-cli';
const CONFIG_FN_ID = 'claude-cli::on-config-change';
const TIMEOUT_MS = 5_000;

export type Config = {
  /** Path to the `claude` binary; empty resolves it on the terminal host. */
  executable: string;
  /** Extra argv for every session. */
  args: string[];
  /** Directory Claude starts in; empty means `<primary shell root>/claude-cli`. */
  workspace_dir: string;
  /** Install Claude Code (vendor installer) when the binary is missing. */
  ...
  return {
    ...
    properties: {
      executable: {
        type: 'string',
        description:
          'Path to the claude binary on the terminal host. Empty: resolve `claude` on PATH there, installing it first when auto_install is on.',
      },
      args: {
        type: 'array',
        items: { type: 'string' },
        description: 'Extra argv appended to every Claude session.',
      },
      workspace_dir: {
        type: 'string',
        description:
          "Directory Claude starts in. Empty: `claude-cli` under the shell worker's primary root. Must be reachable by the shell worker — it owns the terminal.",
      },
      auto_install: {
        type: 'boolean',
        description: 'Install Claude Code from https://claude.ai/install.sh when it is missing.',
      },
      setup_workspace: {
        type: 'boolean',
        description:
...
export async function registerConfig(iii: IIIClient): Promise<void> {
  await iii.trigger({
    ...
    payload: {
      ...
      name: 'Claude CLI',
      description:
        'Claude Code terminal worker: the claude binary path and argv, the workspace directory, the install/setup toggles, and the stream its turns land on.',
      schema: jsonSchema(),
...
export async function bindConfigTrigger(
  ...
  iii.registerFunction(
    ...
    {
      description: 'Internal: reload claude-cli configuration when it changes.',
