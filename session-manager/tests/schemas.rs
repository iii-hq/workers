//! Wire-schema snapshots for every `session::*` function this worker registers.
//!
//! `session_manager::surface::catalog()` is the single source of truth for each
...
//! registration, from the same input/output structs). Each entry is serialized
//! to pretty JSON and compared against `tests/golden/schemas/<id>.json` (`::`
//! maps to `.` in filenames).
//!
...
use session_manager::surface::{catalog, FunctionSpec};
...
/// order (store protocol, then session::*, then the configuration handlers —
/// kept in lockstep with the boot sequence in `main.rs`).
#[test]
fn catalog_lists_all_functions_in_registration_order() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "session::store::put-attachment",
            "session::store::get-attachment",
            "session::store::list-attachments",
            "session::store::delete-attachment",
            "session::store::delete-attachments",
