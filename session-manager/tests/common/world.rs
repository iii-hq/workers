...
pub async fn dispatch(deps: &Deps, function: &str, payload: Value) -> Result<Value, String> {
    match function {
        ...
        "session::set_meta" => out(set_meta::handle(deps, parse(payload)?).await),
        "session::set_status" => out(set_status::handle(deps, parse(payload)?).await),
        "session::delete" => out(delete::handle(deps, parse(payload)?).await),
        "session::append" => out(append::handle(deps, parse(payload)?).await),
        "session::append_many" => out(append_many::handle(deps, parse(payload)?).await),
        "session::update_message" => out(update_message::handle(deps, parse(payload)?).await),
        "session::messages" => out(messages::handle(deps, parse(payload)?).await),
        "session::get_message" => out(get_message::handle(deps, parse(payload)?).await),
        "session::fork" => out(fork::handle(deps, parse(payload)?).await),
        "session::set_active_leaf" => out(set_active_leaf::handle(deps, parse(payload)?).await),
