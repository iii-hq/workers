...
| `workers/harness/src/cases-native-capture.ts` | `capture: native` cases for postgres (LISTEN/NOTIFY, `pg_native_db`) and sqlite (changelog + fs watch, `sqlite_native_db`) — cross-client delivery, no double-fire on own writes, table-less binding rejection |
