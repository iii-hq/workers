...
import { Runner, type RunnerMode } from './runner.ts';
...
// `HARNESS_MODE` controls which case groups run. Validated here so a typo in
// run-tests.sh becomes "ran the full suite" rather than silently disabling
// the bypass repros that CI relies on.
...
const runner = new Runner({ iii, reportPath: REPORT_PATH, filterDriver: FILTER, mode: MODE });
