...
export function PlanTree({ plan }: { plan: ExplainResult }) {
  ...
  if (!plan.root) {
    return (
      <p className="db-msg">The driver returned no plan for this statement.</p>
    )
  }
...
function Node({
  ...
  const share =
    rootCost > 0 ? Math.min(1, (node.cost_total ?? 0) / rootCost) : 0
  const worst = mine.some((w) => w.severity === 'high')
    ? 'high'
    : mine.length
      ? 'warn'
      : null
      ...
      >
        ...
        <span className="db-plan-metrics">
          {node.rows_actual != null && node.rows_estimated != null ? (
            ...
            <Metric
              label="Rows"
              value={fmt(node.rows_estimated)}
              title="Estimated"
            />
          ) : null}
          {node.loops != null && node.loops > 1 ? (
            <Metric label="Loops" value={fmt(node.loops)} />
          ) : null}
          {node.time_ms != null ? (
            <Metric label="Time" value={`${node.time_ms.toFixed(2)}ms`} />
          ) : null}
          {node.cost_total != null ? (
            <Metric label="Cost" value={fmt(node.cost_total)} />
          ) : null}
        </span>

        <span
          className="db-track plan"
          title={`${(share * 100).toFixed(1)}% of total cost`}
        >
          <span
