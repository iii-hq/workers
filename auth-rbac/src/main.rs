...
fn spawn_skill_register(iii: Arc<iii_sdk::III>) {
    tokio::spawn(async move {
        register_skill_with_retry(&iii, auth_rbac::SKILL_ID, auth_rbac::SKILL_MD).await;
        for (id, body) in auth_rbac::SUB_SKILLS {
...
async fn unregister_skill(iii: &Arc<iii_sdk::III>) {
    for (id, _) in auth_rbac::SUB_SKILLS {
        let _ = iii
            .trigger(TriggerRequest {
    ...
    let _ = iii
        .trigger(TriggerRequest {
            function_id: "skills::unregister".into(),
            payload: json!({ "id": auth_rbac::SKILL_ID }),
            action: None,
            timeout_ms: Some(2_000),
        })
        .await;
