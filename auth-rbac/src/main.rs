//! Standalone `auth-rbac` worker. Registers `auth::rbac::*` (workspaces, API keys,
//! HMAC verify, role grants) on the iii engine and runs until Ctrl-C.
//!
//! Usage:
//!   iii-auth-rbac [--engine-url <ws>]
//!
//! Required env: `AUTH_HMAC_SECRET` — base secret used to derive HMACs.
...
fn parse_args(args: Vec<String>) -> Result<String> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!("iii-auth-rbac [--engine-url <ws>]");
                std::process::exit(0);
            }
            "--version" | "-V" => {
                println!("iii-auth-rbac {}", env!("CARGO_PKG_VERSION"));
                std::process::exit(0);
            }
...
async fn main() -> Result<()> {
    ...
    let cfg = auth_rbac::AuthRbacConfig::from_env()
        .context("auth-rbac config from env failed (set AUTH_HMAC_SECRET)")?;
    let _refs = auth_rbac::register_with_iii(&iii, cfg)
        .await
        .context("auth-rbac register failed")?;
    log::info!("registered: auth-rbac (9 auth::rbac::* fns)");

    log::info!("auth-rbac ready — waiting for requests (Ctrl-C to exit)");
