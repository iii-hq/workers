//! Integration coverage for oauth-google-antigravity.
//!
...
impl oauth_google_antigravity::testing::CredentialBus for InMemoryBus {
    fn set_token<'a>(
        &'a self,
        provider: &'a str,
        cred: &'a Credential,
...
fn authorize_url_includes_google_offline_params() {
    let url = oauth_google_antigravity::testing::build_authorize_url(
        "TEST_CHALLENGE",
        "TEST_STATE",
        "http://127.0.0.1:53703/callback",
    );
...
async fn register_with_iii_records_expected_function_ids() {
    ...
    let bus_dyn: Arc<dyn oauth_google_antigravity::testing::CredentialBus> = bus.clone();

    let refs = oauth_google_antigravity::testing::register_with_iii_with_bus(&iii, bus_dyn)
        .await
    ...
    let ids: Vec<String> = bus
        ...
        .map(|m| m.id.clone())
        .collect();
    assert!(ids.contains(&"oauth::google-antigravity::login".to_string()));
    assert!(ids.contains(&"oauth::google-antigravity::refresh".to_string()));
    assert!(ids.contains(&"oauth::google-antigravity::status".to_string()));
    assert_eq!(ids.len(), 3);
