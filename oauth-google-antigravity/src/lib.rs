//! OAuth 2.0 Authorization Code + PKCE flow for the Google Antigravity
//! multi-model gateway.
//!
//! Antigravity reuses Google's installed-app OAuth shape, with the same
//! `client_id` as the Cloud Code Assist flow. The exchange runs against
//! Google's standard OAuth token endpoint:
//!   1. Generate a PKCE verifier/challenge pair (SHA-256, base64-url, no padding).
//!   2. Spawn a localhost HTTP listener on `127.0.0.1:53703/callback`.
//!   3. Build the authorize URL and hand it to the caller via `on_open_url`.
//!   4. Wait for the callback, exchange `code` for tokens.
...
/// Defaults to `127.0.0.1`; override via `HARNESS_OAUTH_ANTIGRAVITY_CALLBACK_HOST`.
pub fn callback_host() -> String {
    env::var("HARNESS_OAUTH_ANTIGRAVITY_CALLBACK_HOST").unwrap_or_else(|_| DEFAULT_HOST.to_string())
}

/// Returns the loopback port the callback listener will bind to.
/// Defaults to 53703; override via `HARNESS_OAUTH_ANTIGRAVITY_CALLBACK_PORT`.
pub fn callback_port() -> u16 {
    env::var("HARNESS_OAUTH_ANTIGRAVITY_CALLBACK_PORT")
        .ok()
...
/// Internal bus surface for the oauth-google-antigravity register handlers.
#[allow(dead_code)]
...
/// Register `oauth::google-antigravity::*` iii functions on the bus.
///
/// Functions registered:
/// - `oauth::google-antigravity::login` — runs the PKCE flow; URL + progress messages
///   logged via `log::info!`. UI integration can wrap this with an
///   ahead-of-time URL fetch in a follow-up. Returns the resulting
///   [`Credential`] as JSON.
/// - `oauth::google-antigravity::refresh` — payload is the existing credential JSON;
///   returns the rotated credential.
/// - `oauth::google-antigravity::status` — payload is empty; returns `{ ready: bool }`
...
pub async fn register_with_iii_with_bus(
    ...
    let login_msg =
        RegisterFunctionMessage::with_id("oauth::google-antigravity::login".to_string())
    ...
    let login_fn = iii.register_function((login_msg, |_payload: Value| async move {
        let callbacks = OAuthLoginCallbacks {
            on_open_url: Box::new(|url| {
                log::info!("oauth::google-antigravity::login open URL: {url}");
            }),
            on_progress: Box::new(|msg| {
                log::info!("oauth::google-antigravity::login progress: {msg}");
            }),
        };
    ...
    let refresh_msg =
        RegisterFunctionMessage::with_id("oauth::google-antigravity::refresh".to_string())
    ...
    let status_msg =
        RegisterFunctionMessage::with_id("oauth::google-antigravity::status".to_string())
...
mod tests {
    ...
    fn pkce_challenge_is_sha256_of_verifier() {
        let verifier = "test-verifier-antigravity";
        let challenge = pkce_challenge(verifier);
    ...
    fn authorize_url_includes_cloud_platform_scope() {
        let url = build_authorize_url("CHAL", "STATE", "http://127.0.0.1:53703/callback");
    ...
    fn callback_port_respects_env_var() {
        env::set_var("HARNESS_OAUTH_ANTIGRAVITY_CALLBACK_PORT", "65002");
        assert_eq!(callback_port(), 65002);
        env::remove_var("HARNESS_OAUTH_ANTIGRAVITY_CALLBACK_PORT");
        assert_eq!(callback_port(), DEFAULT_PORT);
    }

    #[test]
    fn parse_code_validates_state() {
        let line = "GET /callback?code=AGCODE&state=GOOD HTTP/1.1\r\n";
        let code = parse_code_from_request_line(line, "GOOD").unwrap();
        assert_eq!(code, "AGCODE");
