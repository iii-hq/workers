//! The Cua computer-server backend. computer-server runs inside the desktop
//! guest and speaks a tiny request/response protocol: each call is a JSON
//! frame `{"command": <name>, "params": {...}}` and the reply is
...
pub struct ComputerServerClient {
    endpoint: String,
    socket: Mutex<Option<Socket>>,
    command_timeout: Duration,
}

impl ComputerServerClient {
    /// Connect to a computer-server endpoint. `endpoint` may be a full ws/wss
...
/// `return_code`/`stderr` composite computer-server uses for shell failures.
...
impl Backend for ComputerServerClient {
    ...
    async fn screenshot(&self) -> Result<Shot, String> {
        ...
        if bytes.is_empty() {
            return Err("screenshot: backend returned an empty image".to_string());
        }
        Ok(Shot::new(bytes))
    }
