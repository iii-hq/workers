    ...
    iii.register_function(
        ...
        RegisterFunction::new_async(move |req: observe::ObserveInput| {
            ...
            async move {
                ...
                let shot = backend.screenshot().await.map_err(Error::Handler)?;
                let accessibility = if req.include_a11y.unwrap_or(false) {
                    match backend.accessibility_tree().await {
            ...
            async move {
                ...
                let detail = match req.action.as_str() {
                    "click" | "left_click" => {
                        ...
                        backend.left_click(x, y).await.map_err(Error::Handler)?;
                        format!("left click at ({x}, {y})")
                    }
                    "right_click" => {
                        let (x, y) = require_xy(&req)?;
                        backend.right_click(x, y).await.map_err(Error::Handler)?;
                        format!("right click at ({x}, {y})")
                    }
                    "double_click" => {
                        let (x, y) = require_xy(&req)?;
                        backend.double_click(x, y).await.map_err(Error::Handler)?;
                        format!("double click at ({x}, {y})")
                    }
                    "move" | "move_cursor" => {
                        let (x, y) = require_xy(&req)?;
                        backend.move_cursor(x, y).await.map_err(Error::Handler)?;
                        format!("move to ({x}, {y})")
                    }
                    "scroll" => {
                        ...
                        backend.scroll(x, y, sx, sy).await.map_err(Error::Handler)?;
                    ...
                    "type" => {
                        ...
                        backend.type_text(text).await.map_err(Error::Handler)?;
                    ...
                    "press" | "hotkey" | "key" => {
                        ...
                        backend.keypress(keys).await.map_err(Error::Handler)?;
