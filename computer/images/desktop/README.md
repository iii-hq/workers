...
A minimal Linux desktop for **sandbox-backed** computer-use sessions. The
`computer` worker boots this inside an [`iii-sandbox`](https://workers.iii.dev/workers/iii-sandbox)
microVM and drives it entirely through iii primitives (`sandbox::exec` +
`sandbox::fs`), with no computer-server and no host socket.

## Why a sandbox backend
