//! Cerebras Chat Completions streaming via provider-base.
...
pub struct CerebrasConfig {
    ...
    /// <value>`. Populated via [`CerebrasConfig::with_credential`] in
    /// P5; the legacy [`CerebrasConfig::from_env`] constructor still
...
pub async fn register_with_iii(iii: &iii_sdk::III) -> anyhow::Result<()> {
    provider_base::register_provider_complete::<CerebrasConfig, _, _, _, _>(
        iii,
        PROVIDER_NAME,
        |model: &str, cred: &auth_credentials::Credential| {
            CerebrasConfig::with_credential(model, cred)
        },
        stream,
    );
    Ok(())
...
mod tests {
    ...
    fn config_from_env_behavior() {
        ...
        assert!(CerebrasConfig::from_env("test-model").is_err());
        std::env::set_var(ENV_VAR, "test-key");
        let cfg = CerebrasConfig::from_env("test-model").unwrap();
    ...
    fn with_credential_api_key() {
        ...
        let cfg = CerebrasConfig::with_credential("the-model", &cred).unwrap();
