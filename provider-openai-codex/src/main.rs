...
use provider_openai_codex::register::register_provider;

#[derive(Parser, Debug)]
#[command(
    name = "provider-openai-codex",
    about = "OpenAI Codex (ChatGPT subscription) Responses provider worker behind llm-router."
)]
struct Cli {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.manifest {
        println!(
            "{}",
            serde_json::to_string_pretty(&provider_openai_codex::manifest::build_manifest())?
        );
        return Ok(());
    }
    ...
    let iii = register_worker(
        ...
        InitOptions {
            metadata: Some(WorkerMetadata {
                ...
                name: "provider-openai-codex".to_string(),
