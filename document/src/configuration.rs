...
pub const CONFIG_ID: &str = "document";
...
const CONFIG_FN_ID: &str = "document::on-config-change";
...
pub async fn register_config(iii: &IIIClient, seed: Option<&WorkerConfig>) -> Result<(), String> {
    let mut payload = json!({
        "id": config_id(),
        "name": "Document",
        "description": "Limits for converting documents to markdown: the size ceiling on an \
...
/// Require the authoritative entry instead of treating an unavailable service as defaults.
...
pub fn register_config_trigger(iii: &IIIClient, cell: ConfigCell) -> Result<(), Error> {
    ...
    iii.register_function(
        ...
        .description(
            "Internal: hot-reload the document worker from the authoritative configuration when \
...
/// `document::on-config-change` is a bus function, so trusting a `new_value` in
...
async fn on_config_change(iii: &IIIClient, cell: &ConfigCell) {
    ...
    tracing::info!("document configuration reloaded");
}
...
mod tests {
    ...
    fn a_missing_entry_is_not_retried() {
        assert!(is_not_found(
            "remote error (NOT_FOUND): configuration 'document' not found"
        ));
