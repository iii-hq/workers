//! Injectable console UI for the document worker.
//!
//! Ships two assets into any running console:
//!
//! - `document/page.js` (`console:script`) — a page that takes a document and
//!   shows what the agent sees: the detected format, the markdown, and the
//!   images the file carries.
//! - `document/styles.css` (`console:style`) — the stylesheet, every rule
//!   scoped under `[data-iii-ui="document"]`.
//!
//! The registration machinery (content function `document::ui-content`, one
//! Message-path trigger per asset, the `III_DOCUMENT_UI_WATCH` hot-reload
...
pub const PAGE_PATH: &str = "document/page.js";
pub const STYLES_PATH: &str = "document/styles.css";
...
fn console_ui() -> ConsoleUi {
    ConsoleUi::new("document")
        .script(PAGE_PATH, PAGE_JS)
        .style(STYLES_PATH, STYLES_CSS)
}

/// Register the document worker's console UI. Call after the functions it
/// drives.
...
mod tests {
    ...
    fn embedded_styles_are_scoped() {
        // esbuild prints the attribute selector unquoted ([data-iii-ui=document]).
        assert!(
            STYLES_CSS.contains(r#"[data-iii-ui="document"]"#)
                || STYLES_CSS.contains("[data-iii-ui=document]"),
